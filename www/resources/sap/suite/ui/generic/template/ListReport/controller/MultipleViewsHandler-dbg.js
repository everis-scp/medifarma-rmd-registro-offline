sap.ui.define([
	...
	function getMethods(oState, oController, oTemplateUtils) {
		...
		function getPresentationControl() {
			var oSettings = oTemplateUtils.oComponentUtils.getSettings();
			if (oSettings.quickVariantSelection) {
				return oState.oSmartTable;
			} else if (oSettings.quickVariantSelectionX) {
				var sId = StableIdHelper.getStableId({type: "ListReportTable", subType: "SmartTable", "sQuickVariantKey": oGenericMultipleViewsHandler.getSelectedKey()});
		...
		return {
			...
			getMode: fnGetMode,
			getPresentationControl: getPresentationControl
		};
	}
