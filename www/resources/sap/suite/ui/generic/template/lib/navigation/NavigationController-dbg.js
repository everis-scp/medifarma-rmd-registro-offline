...
sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
					...
					if (sComponentId === oCurrentIdentity.treeNode.componentId){
						...
						for (var sRouteName in oTemplateContract.mRoutingTree) {
							if (!mAncestors[sRouteName]) {
								...
								oTemplateContract.oNavigationHost.unbindElement(oOtherTreeNode.specificModelName);
								// Temporary solution (AncestorFeature)
								if (oOtherTreeNode.contextTargets) {
									oOtherTreeNode.contextTargets.forEach(function(oContextTarget) {
										oContextTarget.unbindElement();
									});
								}
							}
						}
		...
		function fnSetPlaceholderForIdentity(oIdentity){
			...
			placeholderHelper.setPlaceholder(oIdentity, oFlexibleColumnLayoutHandler, oTemplatePrivateGlobalModel, oRoutingTree);
		}
		...
		function fnPrepareHostView(oTreeNode, isComponentPreloaded){
			...
			var oPlaceholderInfo = bEnablePlaceholder && oTreeNode.behaviour && oTreeNode.behaviour.getPlaceholderInfo();
		...
		function fnHandleRouteMatchedImpl() {
			...
			if (oActivationInfo.bIsBack && !oRoutingOptions) { // the Browser Back button scenario. In all other scenarios we have checked beforehand
		...
		function isBackLeavingTheMainObject(){
			if (oHistoryState) {
				var aHistoricalEntires = oHistoryState.getData().historicalEntries;
				var oCurrentIdentity = getCurrentIdentity();
				if (oCurrentIdentity.treeNode.level === 0){
					return false; 
				}
				...
				var oCurrentMainObjectNode = oTemplateContract.oApplicationProxy.getAncestralNode(oCurrentIdentity.treeNode,1);
				var oPreviousNode = oTemplateContract.mRoutingTree[aHistoricalEntires[1].sRouteName];
				var oPreviousMainObjectNode = oTemplateContract.oApplicationProxy.getAncestralNode(oPreviousNode,1);
				return oCurrentMainObjectNode !== oPreviousMainObjectNode;
		...
		oNavigationControllerProxy.isBackLeavingTheMainObject = isBackLeavingTheMainObject;
		...
		oTemplateContract.oBusyHelper.setBusy((getParsedShellHashFromFLP() || Promise.resolve()).then(function(oResults){
			...
			if (oParsedUrl && oParsedUrl["sap-ui-xx-placeholder"]){
				var bPlaceholderValueFromURL = oParsedUrl["sap-ui-xx-placeholder"][0] === 'true' ? true : false ;
				oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/enablePlaceholder",bPlaceholderValueFromURL);	
			} 
			bEnablePlaceholder = oTemplateContract.oTemplatePrivateGlobalModel.getProperty("/generic/enablePlaceholder");
			if (!bEnablePlaceholder) {
