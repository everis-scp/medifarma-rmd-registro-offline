	...
	function getMethods(oController, oTemplateUtils, oConfiguration) {
			...
			if (iRequest > 1){ // refresh needed
				if (bIsSmartTable){
					if (oConfiguration.refreshModelOnTableRefresh) {
						var bIsInvalidating = oTemplateUtils.oCommonUtils.refreshModel(oCurrentViewMeta.smartControl);
						if (bIsInvalidating){
							fnRefreshSiblingControls(oCurrentViewMeta.smartControl);
						}
					}
		...
		function fnRefreshSiblingControls(oSmartControl){
			if (oImplementingHelper.refreshSiblingControls) {
				oImplementingHelper.refreshSiblingControls(oSmartControl);
			}			
		}
