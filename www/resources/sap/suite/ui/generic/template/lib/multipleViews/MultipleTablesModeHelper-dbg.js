...
], function(BaseObject, Device, Filter, testableHelper, controlStateWrapperFactory, extend) {
	"use strict";
	/*
	...
	function getMethods(oController, oTemplateUtils, oConfiguration) {
		...
		function fnGetImplementingHelperAttributes(oViewMeta) {
			...
			return {
				...
				controlStateWrapper: controlStateWrapperFactory.getControlStateWrapper(oViewMeta.smartControl)
			};
		}
		...
		function fnGetSelectedKeyAndRestoreFromIappState(oState) {
			...
			if (oState.controlStates) {
				...
				for (var sKey in  mSwitchingKeyToViewMeta) {
					mSwitchingKeyToViewMeta[sKey].implementingHelperAttributes.controlStateWrapper.setState(oState.controlStates && oState.controlStates[mSwitchingKeyToViewMeta[sKey].smartControl.getId()]);
				}
			} else {
		...
		function fnGetContentForIappState(sSelectedKey) {
			var sKey, oTmpTable, mControlStates = Object.create(null);
			for (sKey in mSwitchingKeyToViewMeta) {
				oTmpTable = mSwitchingKeyToViewMeta[sKey].smartControl;
				mControlStates[oTmpTable.getId()] = mSwitchingKeyToViewMeta[sKey].implementingHelperAttributes.controlStateWrapper.getState();
			}
			var oRet = {
		...
		function fnRefreshSiblingControls(oSmartControl){
			...
			for (var sKey in  mSwitchingKeyToViewMeta) {
				var oViewMeta = mSwitchingKeyToViewMeta[sKey];
				var oControl = oViewMeta.smartControl;
				if (oControl !== oSmartControl && oControl.getEntitySet() === sEntitySet && oViewMeta.implementingHelperAttributes.dirtyState < 2){
