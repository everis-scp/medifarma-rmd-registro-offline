sap.ui.define(["sap/ui/base/Object",
	...
	SelectionVariant, Controller, FeLogger, MessageUtils, extend, isEmptyObject, genericAppLibrary, placeholderHelper) {
	...
	function getMethods(oController, oComponentUtils, oServices, oCommonUtils) {
		...
		function getPaginatorInfo(oTable, oRow, oState){
			var oControlBindingInfo = oCommonUtils.getTableOrListBindingInfo(oTable);
		...
		*/
		...
		function fnNavigateIntent(oOutbound, oContext, oSmartFilterBar, oSmartControl, oState, bDisplayOrCreateNavigation) {
			var oNavigationHandler = oServices.oApplication.getNavigationHandler();
			...
			oCommonUtils.removePropertiesFromNavigationContext(oSelectionVariant, oSmartControl, oState);
		...
		function fnNavigateIntentManifest(oEventSource, oContext, oSmartFilterBar, oSmartChart, bDisplayOrCreateNavigation) {
			...
			if (controlHelper.isSmartChart(oEventSource)){
				...
				oSmartControl = oCommonUtils.getOwnerControl(oEventSource) && oCommonUtils.getOwnerControl(oEventSource).getParent();
			}
			fnNavigateIntent(oOutbound, oContext, oSmartFilterBar, oSmartControl, Object.create(null), bDisplayOrCreateNavigation);
		}
		...
		function fnOnSemanticObjectLinkNavigationPressed(oEventSource, oEventParameters){
			...
			}, Function.prototype, undefined, (oComponentUtils.isDraftEnabled() && "NavigateExternal") || undefined);
		}
		...
		function onListNavigate(oEventOrSource, oState, oBindingContext, bIsProgrammatic) {
			...
			if (bIsProgrammatic){
				...
				if (oEventSource.data("CrossNavigation") || (!oEventSource.data("CrossNavigation") && !oComponentUtils.isDraftEnabled())){
					oCommonUtils.fnProcessDataLossOrDraftDiscardConfirmation(onListNavigateImpl.bind(null, oEventSource, oEventParameters, oState, oBindingContext, bReplace), Function.prototype, undefined, (oComponentUtils.isDraftEnabled() && "NavigateExternal") || undefined);
				} else {
...
//			}
		...
		function onDataFieldWithIntentBasedNavigation(oEvent, oState) {
			...
			oCommonUtils.fnProcessDataLossOrDraftDiscardConfirmation(function() {
				...
				fnNavigateIntent(oOutbound, oContext, oState.oSmartFilterbar || undefined, oState.oSmartTable || oEventSource, oState);
			}, Function.prototype, oState, (oComponentUtils.isDraftEnabled() && "NavigateExternal") || undefined);
		}
		...
		function onInlineDataFieldForIntentBasedNavigation(oEventSource, oState){
			oCommonUtils.fnProcessDataLossOrDraftDiscardConfirmation(function() {
				...
				var oContext = oEventSource.getParent().getBindingContext();
				fnNavigateIntent(oOutbound, oContext, oState.oSmartFilterbar, oState.oSmartTable || oEventSource, oState);
			}, Function.prototype, oState, (oComponentUtils.isDraftEnabled() && "NavigateExternal") || undefined);
		}
