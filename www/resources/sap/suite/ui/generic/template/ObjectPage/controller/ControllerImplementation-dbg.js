sap.ui.define([
	...
	"sap/suite/ui/generic/template/genericUtilities/controlStateWrapperFactory"
		...
		ExtensionAPI, AnnotationHelper, ResponsiveTable, DynamicSideContent, ShareUtils, Selectors, FeLogger, MultipleViewsHandler,
			...
			getFirstEditableInput, Focusable, FeError, CreateWithDialogHandler, FlexRuntimeInfoAPI, Device, placeholderHelper, controlStateWrapperFactory) {
	"use strict";
		...
		getMethods: function(oViewProxy, oTemplateUtils, oController) {
				...
				return oTemplateUtils.oCommonUtils.getDialogFragmentAsync(
					"sap.suite.ui.generic.template.ObjectPage.view.fragments.RelatedAppsSheet", {
						buttonPressed: function(oEvent) {
							...
							}, Function.prototype, undefined, (oTemplateUtils.oComponentUtils.isDraftEnabled() && "NavigateExternal") || undefined);
						}
					}, "buttons");
			}
			...
			*/
			...
			function getGroupTitle(sMsgId, aControlIds) {
				...
				return aGroupers.length ? aGroupers[0].getTitle() : oTemplateUtils.oCommonUtils.getText("GENERIC_MESSAGE_GROUP_NAME");
			}
			...
			oMessageSorter.fnCompare = function (oMsgObj1, oMsgObj2) {
				...
				if (oPlacementInfo1.groupers.length === oPlacementInfo2.groupers.length && oPlacementInfo1.groupers.every(function(oControl, i){
					return oPlacementInfo2.groupers[i] === oControl;
				})){ // messages belong to the same group
				...
				var iMaxPathLength = Math.max(oPlacementInfo1.pathToControlId.length, oPlacementInfo2.pathToControlId.length);
			...
			function fnGetFocusableElementOnObjectPage(oObjectPage, sMode) {
				if (sMode === "Display") {
					...
					var fnGetFocusableElementOnSection = function (oSection) {
						...
						var oFirstFocusableElement = getFirstEditableInput(oSectionDomRef);
						if (oFirstFocusableElement && oFirstFocusableElement.type !== "checkbox") {
							return oFirstFocusableElement;
						}
						// In case if getFirstEditableInput method returns a checkbox control, as this is not a focusable control
					...
					var sNextSectionId, oFocusableElement;
					for (var i = 0; i < aSections.length; i++) {
						if (aSections[i].getVisible() && ((aSections[i].getId() === sSectionSelected) || sNextSectionId)) {
							...
							oFocusableElement = fnGetFocusableElementOnSection(aSections[i]);
			...
			function fnInitSmartTableInfoObject(oSmartTableInfoObject, aCategories, oSmartTable){
				...
				oSmartTableInfoObject.oControlStateWrapper = controlStateWrapperFactory.getControlStateWrapper(oSmartTable);
				oSmartTableInfoObject.oControlStateWrapper.attachStateChanged(oViewProxy.getCurrentState);
			...
			function fnInitSmartChartInfoObject(oSmartChartInfoObject, aCategories, oSmartChart){
				...
				aCategories.push("smartControl");
				oSmartChartInfoObject.oControlStateWrapper = controlStateWrapperFactory.getControlStateWrapper(oSmartChart);
				oSmartChartInfoObject.oControlStateWrapper.attachStateChanged(oViewProxy.getCurrentState);
				//TODO: Do we still need this block of code
			...
			function onSearchObjectPage(oEvent) {
				...
				oTemplateUtils.oCommonUtils.refreshSmartTable(oSearchFieldInfoObject.table);
			}
			...
			function addEntryImpl(oPredefinedValues, oButton) {
				...
				oTemplateUtils.oCommonUtils.fnProcessDataLossOrDraftDiscardConfirmation(function() {
					oTemplateUtils.oCommonEventHandlers.addEntry(oButton, false);
				}, Function.prototype, oBase.state, (oTemplateUtils.oComponentUtils.isDraftEnabled() && "NavigateExternal") || undefined);
			}
			...
			oViewProxy.getCurrentState = function() {
				...
				var bPageOrPagination = oController.getOwnerComponent().getAppComponent().getStatePreservationMode() === "persistence";
			...
			var oControllerImplementation = {
				onInit: function() {
					...
					oTemplateUtils.oComponentUtils.attach(oController, "PageDataLoaded", function(oEvent) {
						...
						placeholderHelper.resetPlaceHolders(oTemplateUtils);
					});
					...
					function fnInitializeInfoObjects(oBlock){
						if (oBlock.tableSettings){
							var oSmartTable = oController.byId(StableIdHelper.getStableId({type: "ObjectPageTable", subType: "SmartTable", sFacet: oBlock.additionalData.facetId}));
							if (oSmartTable){
					...
					onBeforeSemanticObjectLinkNavigationCallback: function() {
						return new Promise(function(resolve) {
							oTemplateUtils.oCommonUtils.fnProcessDataLossOrDraftDiscardConfirmation(function () {
								...
								resolve(false);
							}, undefined, (oTemplateUtils.oComponentUtils.isDraftEnabled() && "NavigateExternal") || undefined);
						});
					},
