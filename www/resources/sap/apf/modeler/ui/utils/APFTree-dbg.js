		...
		_getObjectNodesArray : function(sObjectType, configIndexInTree, categoryIndexInConfig, stepIndexInCategory) {
			...
			switch (sObjectType) {
				...
				case constants.configurationObjectTypes.FACETFILTER:
		            ...
		            var filtersLength = this.getItems()[parseInt(index,10)+1].getItemNodeContext().children.length;
		            if(this.getItems()[parseInt(index,10)+1].getItemNodeContext().context.sPath === "/aConfigDetails/"+configIndexInTree+"/configData/0"){
		            for(var i = 1; i < filtersLength; i++) {
		               aObjectArray.push(this.getItems()[parseInt(index,10)+1+i]);
		             }
		            }
				...
				case constants.configurationObjectTypes.CATEGORY:
					...
					if(this.getItems()[parseInt(index,10)+1].getItemNodeContext().context.sPath === "/aConfigDetails/"+configIndexInTree+"/configData/0"){
		            var filtersLength = this.getItems()[parseInt(index,10)+1].getItemNodeContext().children.length;
		            var categoryIndex = parseInt(index,10)+2+filtersLength;
		            var categoryLength = this.getItems()[parseInt(categoryIndex,10)].getItemNodeContext().children.length;
		            for(var i = 1; i < categoryLength; i++) {
		               aObjectArray.push(this.getItems()[parseInt(categoryIndex,10)+1+i]);
		             }
		            }
					this.expandToLevel(1);
					break;
