...
sap.ui.define(["sap/ui/mdc/filterbar/IFilterContainer", "sap/m/HeaderContainer", "sap/m/FlexBox"], function(
	IFilterContainer,
	HeaderContainer,
	FlexBox
) {
	...
	VisualFilterContainer.prototype.init = function() {
		...
		this.oLayout = new FlexBox({
			direction: "Column", // TODO: for phone this should be Column Reverse. Check with V2 or UX
	...
	VisualFilterContainer.prototype.insertFilterField = function(oControl, iIndex) {
		...
		var bFieldHasVisualFilter = false;
		var oVisualFilters = this.aVisualFilterFields;
		var _oVisualFilter;
		this.aAllFilterFields.forEach(function(oFilterField) {
			if (oFilterField.getContent()[1].getId() === oControl.getContent()[1].getId()) {
				var sFilterFieldId = oControl.getContent()[1].getId();
				if (oVisualFilters[sFilterFieldId]) {
					_oVisualFilter = oVisualFilters[sFilterFieldId];
					oFilterField.getContent().forEach(function(oInnerControl) {
						if (oInnerControl.isA("sap.ui.mdc.FilterField") && oInnerControl.data("hasVisualFilter") === "true") {
							// set aggregation content to render visual filter field
							oInnerControl.setContent(_oVisualFilter);
							bFieldHasVisualFilter = true;
						}
					});
				}
			}
