...
import { Draft, singletonPathVisitor, UI } from "sap/fe/core/converters/helpers/BindingHelper";
...
function getOperationAvailableMap(lineItemAnnotation: LineItem | undefined): string {
					...
					if (actionTarget?.parameters?.length) {
						const bindingParameterFullName = actionTarget.parameters[0].fullyQualifiedName,
							...
							targetExpression = annotationExpression(
								...
								[],
								undefined,
								(path: string) => (path ? path.substring(bindingParameter.length + 1) : "")
