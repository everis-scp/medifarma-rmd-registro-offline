...
sap.ui.define(
	...
	function(
		...
		var TableHelper = {
			 ...
			 * @returns {bool} `true` if this is a static action, else otherwise
			 * @private
			 * @ui5-restricted
			 */
			...
			addOperationAvailableFieldsToSelectQuery: function(aLineItemCollection, oContext) {
				...
				if (selectedFieldsArray.length) {
					...
					var sEntitySet = oContext.context.getPath().split("/@")[0],
						sEntityType = oContext.context.getModel().getObject(sEntitySet).$Type,
						oHeaderInfo = oContext.context.getModel().getObject("/" + sEntityType + "@com.sap.vocabularies.UI.v1.HeaderInfo");

					if (oHeaderInfo && oHeaderInfo.Title && oHeaderInfo.Title.Value && oHeaderInfo.Title.Value.$Path) {
						selectedFieldsArray.push(oHeaderInfo.Title.Value.$Path);
					}
				}
			 ...
			 * @returns {string} The select query having the selected fields from the parameters passed
			 */
			create$Select: function(
				...
				function pushField(sField) {
					if (sField && !aSelectedFields.includes(sField)) {
			 ...
			 * @param {boolean} bEnableAnalytics A boolean to specify is this is an analytical table or not
			 ...
			 * @returns {*} An expression to determine if the datafield for action should be enabled or not
			 */
			...
			pressEventForDeleteButton: function(oThis, sEntitySetName) {
				...
				return CommonHelper.generateFunction(
					"._editFlow.deleteMultipleDocuments",
					sDeletableContexts,
					CommonHelper.objectToString(oParams)
				);
			},
