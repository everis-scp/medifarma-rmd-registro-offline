	...
	function(
		...
		var oFilterUtils = {
					...
					if (oFilterControl.isA("sap.ui.mdc.FilterBar") && sEntitySetPath !== sFilterEntityPath) {
						...
						aFilterBarSelectionFieldsAnnotation.forEach(function(oFilterBarSelectionFieldAnnotation) {
							...
							if (!mapSelectionFields[sPath]) {
								var oFilterField = ListReportConverter.getFilterField(
									sPath,
									oConverterContext,
			...
			getNotApplicableFiltersForTable: function(oFilterBar, oTable) {
				var sTargetEntityTypePath = oTable.data("entityType"),
					aNotApplicable = [],
				...
				if (mConditions && (!bIsFilterBarEntityType || bIsAnalyticalTable)) {
					...
					var aTargetProperties = bIsFilterBarEntityType
						...
						mTableAggregates = CommonHelper.parseCustomData(DelegateUtil.getCustomData(oTable, "aggregates") || {}),
