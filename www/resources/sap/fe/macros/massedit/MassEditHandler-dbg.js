...
sap.ui.define(
	...
	function(
		 ...
		 * @param {boolean} bPropertyNullable The property value can be set to null or not
		 */
		function setDefaultValuesToDialog(aValues, oTable, sPropertyPath, oDefaultValues, bPropertyNullable) {
					...
					for (var i = 0; i < oSelectedContext.length; i++) {
						...
						if (!oDistinctValueMap[sPropertyKey] && aFinalPath[aPropertyPaths[aPropertyPaths.length - 1]]) {
							...
							aFinalPath[aPropertyPaths[aPropertyPaths.length - 1]].push({
								...
								"textInfo": oTextInfo,
								"fieldRequiredInfo": { "isFieldRequired": oData.isFieldRequired, "relatedUnitField": oData.unitProperty }
							});
						...
						if (oData.unitProperty && oDataObject[oData.unitProperty]) {
							...
							if (!oDistinctValueMap[sPropertyName]) {
								...
								aUnitData[oData.unitProperty].push({
									...
									"textInfo": oTextInfo,
									"fieldRequiredInfo": { "isFieldRequired": oData.isFieldRequired, "relatedUnitField": false }
								});
			...
			aDataArray.forEach(function(oData) {
				if (oData.dataProperty.indexOf("/") > -1) {
					...
					if (!sMultiLevelPropPathValue) {
						...
						setDefaultValuesToDialog(sMultiLevelPropPathValue, oTable, oData.dataProperty, oDefaultValues, oData.nullable);
					}
				} else if (aValues[oData.dataProperty] && aValues[oData.dataProperty].length) {
					setDefaultValuesToDialog(aValues[oData.dataProperty], oTable, oData.dataProperty, oDefaultValues, oData.nullable);
				}
				if (aUnitData[oData.unitProperty] && aUnitData[oData.unitProperty].length) {
					setDefaultValuesToDialog(aUnitData[oData.unitProperty], oTable, oData.unitProperty, oDefaultValues, oData.nullable);
				} else if (
					...
					if (oData.dataProperty) {
						aValues[oData.dataProperty].push({ text: oDefaultValues.leaveBlankValue, key: "Empty/" + oData.dataProperty });
					}
					if (oData.unitProperty) {
						aUnitData[oData.unitProperty].push({ text: oDefaultValues.leaveBlankValue, key: "Empty/" + oData.dataProperty });
					}
				}
		...
		function openMassEditDialog(oTable, oController) {
			var sFragmentName = "sap/fe/macros/massedit/MassEditDialog",
				...
				oDefaultValues = {
					...
					"massEditTitle": oResourceBundle.getText("C_MASS_EDIT_DIALOG_TITLE"),
					"saveButtonText": oResourceBundle.getText("C_MASS_EDIT_SAVE_BUTTON_TEXT"),
					...
					var sDataProperty = oColumn && oColumn.getDataProperty(),
						aRealtedColumnInfo =
							...
							oTable.data("columns").customData.filter(function(oColumnInfo) {
								return oColumnInfo.name === sDataProperty;
							});
					aTableFields.push({
					...
					.then(function(oFragment) {
						return Fragment.load({ definition: oFragment }).then(function(oDialogContent) {
							var oDialog = new Dialog({
								...
								beginButton: {
									text: oDefaultValues.saveButtonText,
									type: "Emphasized",
