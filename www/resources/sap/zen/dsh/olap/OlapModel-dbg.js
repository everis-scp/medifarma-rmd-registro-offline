    ...
    var OlapModel = Model.extend(
      "sap.zen.dsh.olap.OlapModel", {
        constructor: function (mSettings) {
          ...
          var oFireflyReady = Promise.resolve(null).then(
            function(){
          ...
          that.bindList = function (sPath, oContext, aSorters, aFilters, mParameters) {
            if (
              sPath.match(/^(\/)?dataProvider\/(.)*\/Grid\/renderGrid/)
            ){
          ...
          function _getObject(sPath, oContext) {
            var oNode = oContext instanceof Context ? _getObject(
              oContext.getPath()
            ) : oModelData;
            if (sPath) {
          ...
          that.openVariableSelector = function (sVar) {
            return Promise.resolve(null).then(function () {
              ...
              return oDP.openVariableSelector(sVar);
            });
          };
