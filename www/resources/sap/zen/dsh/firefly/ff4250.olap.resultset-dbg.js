...
oFF.RsDefStructureMemberProperties.create = function(placeholderAliasMappings, minimumDrillStateMap, unsatisfiedRequiredDimensionNames)
{
...
oFF.RsMinimumDrillStateValidationContext.create = function(axisType, tupleIndex)
{
	var result = new oFF.RsMinimumDrillStateValidationContext();
	result.setupValidationContext(axisType, tupleIndex);
	return result;
};
oFF.RsMinimumDrillStateValidationContext.prototype.m_axisType = null;
oFF.RsMinimumDrillStateValidationContext.prototype.m_tupleIndex = 0;
...
oFF.RsMinimumDrillStateValidationContext.prototype.setupValidationContext = function(axisType, tupleIndex)
{
...
oFF.RsMinimumDrillStateValidationContext.prototype.getTupleIndex = function()
{
...
oFF.ResultSet.prototype.getDataCell = function(column, row)
{
	...
	if (this.getSession().hasFeature(oFF.FeatureToggleOlap.ASSERT_RESULTSET_BOUNDARIES))
	{
...
oFF.ResultSetContainer.prototype.processExecution = function(syncType, listener, customIdentifier)
{
	...
	var preQueries = this.isSerializedRemotePreQuery() ? null : this.getPreQueries(queryModel);
...
oFF.ResultSetContainer.prototype.executeRuntime = function(syncType, listener, customIdentifier)
{
	...
	if (this.getSyncState() === oFF.SyncState.OUT_OF_SYNC)
	{
		...
		for (var i = 0; i < size; i++)
		{
			...
			_function.getRequest().setRequestStructure(queries.get(i));
...
{
	var preQueries = this.isSerializedRemotePreQuery() ? null : this.getPreQueries(this.getQueryModel());
	if (oFF.XCollectionUtils.hasElements(preQueries))
	{
		var size = preQueries.size();
		for (var i = 0; i < size; i++)
		...
		if (queryModel.isBlendingModel())
		{
			...
			if (oFF.notNull(blendingDefinition))
			{
				...
				while (sourcesIterator.hasNext())
				{
					...
					{
						result.addAll(this.getPreQueries(sourceModel));
					}
				}
			}
		}
...
oFF.ResultSetContainer.prototype.processSynchronization = function(syncType)
{
	...
	if (oFF.isNull(this.m_rsDefQueryModel))
	{
		...
		if (oFF.notNull(queryModel))
		{
			...
			isIgnoreAutoVariableSubmit = queryModel.isIgnoreAutoVariableSubmit();
		}
		this.m_rsDefQueryModel = oFF.RsDefQueryModel.create(queryManager, rowsAxis, columnsAxis, clusteringDimension, isIgnoreAutoVariableSubmit);
	}
	if (this.hasErrors())
	{
		return false;
...
oFF.RsCursorResultSet.prototype.addMeasureWithPlaceholderIdAndAliasName = function(placeholderAliasMapping, dimension)
{
	...
	while (placeholderMappingsIter.hasNext())
	{
		...
		if (dimensionType === oFF.DimensionType.ACCOUNT || dimensionType === oFF.DimensionType.MEASURE_STRUCTURE)
		{
			dimension.addMeasure(oFF.RsDefStructureMember.create(placeholderMappingId, aliasName, null, null));
		}
	}
};
...
oFF.RsCursorResultSet.prototype.createRuntimeRsDefStructureMembersForDimension = function(dimension, runtimeStructureMemberProperties)
{
	...
	while (memberNamesIter.hasNext())
	{
		...
		dimension.addMeasure(oFF.RsDefStructureMember.create(memberName, aliasName, minimumDrillState, unsatisfiedRequiredDimensionNames));
	}
};
...
oFF.RsCursorResultSet.prototype.collectRequiredDimensionsForAxis = function(axisType, reqDimensionNamesMap)
{
	...
	for (var i = 0; i < rsDimensions.size(); ++i)
	{
		...
		if (dimensionType === oFF.DimensionType.ACCOUNT || dimensionType === oFF.DimensionType.MEASURE_STRUCTURE)
		{
			...
			var iterator = rsDimension.getAllStructureMembers().getIterator();
			while (iterator.hasNext())
			{
				var rsDefStructureMember = iterator.next();
				var allAndUnsatisfiedRequiredDimensionNames = rsDefStructureMember.getNestedAllAndUnsatisfiedRequiredDimensionNames(null, null);
				var nestedRequiredDimensionNames = allAndUnsatisfiedRequiredDimensionNames.getFirstObject();
				if (oFF.notNull(nestedRequiredDimensionNames) && !nestedRequiredDimensionNames.isEmpty())
				{
					if (oFF.isNull(validationContext))
					{
						validationContext = oFF.RsMinimumDrillStateValidationContext.create(axisType, i);
					}
					validationContext.addRequiredDimensionNames(rsDefStructureMember.getName(), nestedRequiredDimensionNames);
				}
				var nestedUnsatisfiedRequiredDimensionNames = allAndUnsatisfiedRequiredDimensionNames.getSecondObject();
				if (oFF.notNull(nestedUnsatisfiedRequiredDimensionNames) && !nestedUnsatisfiedRequiredDimensionNames.isEmpty())
				{
					if (oFF.isNull(validationContext))
					{
						validationContext = oFF.RsMinimumDrillStateValidationContext.create(axisType, i);
					}
					validationContext.addUnsatisfiedRequiredDimensionNames(rsDefStructureMember.getName(), nestedUnsatisfiedRequiredDimensionNames);
				}
			}
...
oFF.RsCursorResultSet.prototype.validateCellMinimumDrillState = function(column, row)
{
	...
	if (requiredDimensionNamesByMemberNameMap.isEmpty())
	{
		return result;
	}
	...
	while (iterator.hasNext())
	{
		...
		var memberName = this.getDimensionMemberName(column, row, validationContext.getAxisType(), validationContext.getTupleIndex());
		var unsatisfiedRequiredDimensionNames = validationContext.getUnsatisfiedRequiredDimensionNames(memberName);
		if (oFF.notNull(unsatisfiedRequiredDimensionNames) && !unsatisfiedRequiredDimensionNames.isEmpty())
		{
			result.addMissingRequiredDimensionNames(dimensionName, memberName, unsatisfiedRequiredDimensionNames);
		}
		var requiredDimensionNames = validationContext.getRequiredDimensionNames(memberName);
		if (oFF.notNull(requiredDimensionNames) && !requiredDimensionNames.isEmpty())
		{
			result.addRequiredDimensionNames(dimensionName, memberName, requiredDimensionNames);
...
oFF.RsDefField.prototype.isHierarchyPathField = oFF.noSupport;
...
oFF.RsDefDimension.prototype.setupExt = function(context, dimension, rsDefAxis)
{
	...
	if (oFF.notNull(dimension))
	{
		...
		{
			var structureMembers = dimension.getAllStructureMembers();
			for (var i = 0; i < structureMembers.size(); i++)
			{
				...
				if (oFF.notNull(structureMember))
				{
					...
					var nestedUnsatisfiedRequiredDimensionNames = null;
					var nestedMinimumDrillState = null;
					if (context.getSession().hasFeature(oFF.FeatureToggleOlap.SFX_MINIMUM_DRILL_STATE))
					{
						var allAndUnsatisfiedRequiredDimensionNames = structureMember.getNestedAllAndUnsatisfiedRequiredDimensionNames(context, null);
						nestedMinimumDrillState = allAndUnsatisfiedRequiredDimensionNames.getFirstObject();
						nestedUnsatisfiedRequiredDimensionNames = allAndUnsatisfiedRequiredDimensionNames.getSecondObject();
					}
					var rsStructureMember = oFF.RsDefStructureMember.create(placeholderName, structureMember.getAliasName(), nestedMinimumDrillState, nestedUnsatisfiedRequiredDimensionNames);
...
oFF.RsDefQueryModel.create = function(queryManager, rowsAxis, columnsAxis, dimension, isIgnoreAutoVariableSubmit)
{
	var object = new oFF.RsDefQueryModel();
	object.setupExt(queryManager, rowsAxis, columnsAxis, dimension, isIgnoreAutoVariableSubmit);
	return object;
};
oFF.RsDefQueryModel.prototype.m_rows = null;
oFF.RsDefQueryModel.prototype.m_cols = null;
oFF.RsDefQueryModel.prototype.m_isIgnoreAutoVariableSubmit = false;
oFF.RsDefQueryModel.prototype.m_queryManager = null;
oFF.RsDefQueryModel.prototype.setupExt = function(queryManager, rowsAxis, columnsAxis, dimension, isIgnoreAutoVariableSubmit)
{
	...
	this.m_isIgnoreAutoVariableSubmit = isIgnoreAutoVariableSubmit;
	if (oFF.notNull(dimension))
	{
...
oFF.RsDefQueryModel.prototype.isIgnoreAutoVariableSubmit = function()
{
	return this.m_isIgnoreAutoVariableSubmit;
};
...
oFF.RsDefStructureMember.create = function(name, aliasName, nestedMinimumDrillState, nestedUnsatisfiedRequiredDimensionNames)
{
	...
	rsDefStructureMember.m_nestedRequiredDimensionNames = nestedMinimumDrillState;
	rsDefStructureMember.m_nestedUnsatisfiedRequiredDimensionNames = nestedUnsatisfiedRequiredDimensionNames;
	return rsDefStructureMember;
};
oFF.RsDefStructureMember.prototype.m_aliasName = null;
oFF.RsDefStructureMember.prototype.m_nestedRequiredDimensionNames = null;
oFF.RsDefStructureMember.prototype.m_nestedUnsatisfiedRequiredDimensionNames = null;
oFF.RsDefStructureMember.prototype.releaseObject = function()
{
	this.m_aliasName = null;
	this.m_nestedRequiredDimensionNames = oFF.XObjectExt.release(this.m_nestedRequiredDimensionNames);
	this.m_nestedUnsatisfiedRequiredDimensionNames = oFF.XObjectExt.release(this.m_nestedUnsatisfiedRequiredDimensionNames);
	oFF.RsDefModelComponent.prototype.releaseObject.call( this );
};
...
oFF.RsDefStructureMember.prototype.getNestedAllAndUnsatisfiedRequiredDimensionNames = function(context, messageManager)
{
	return oFF.XPair.create(this.m_nestedRequiredDimensionNames, this.m_nestedUnsatisfiedRequiredDimensionNames);
};
