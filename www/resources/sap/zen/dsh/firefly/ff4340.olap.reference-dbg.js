...
oFF.RsHiChartVizUtilsSimple = {
	...
	ploLineBuilder:function(yAxis, Value, belowColor, aboveColor, color, text, styleStructure)
	{
		...
		if (oFF.notNull(belowColor))
		{
			...
			var interimColorA = oFF.UiColor.create(255, 255, 255, 0);
			interimColorA.setColorByString(belowColor);
			interimColorA.setAlpha(0.2);
			plotBandsProp.putString(oFF.VizDefConstants.K_COLOR, interimColorA.getRgbaColor());
		...
		if (oFF.notNull(aboveColor))
		{
			...
			var interimColorB = oFF.UiColor.create(255, 255, 255, 0);
			interimColorB.setColorByString(aboveColor);
			interimColorB.setAlpha(0.2);
			plotBandsProp1.putString(oFF.VizDefConstants.K_COLOR, interimColorB.getRgbaColor());
...
{
	return new oFF.RsSacTableContextResolver();
};
oFF.RsSacTableContextResolver.prototype.m_modelJson = null;
oFF.RsSacTableContextResolver.prototype.m_resultSet = null;
oFF.RsSacTableContextResolver.prototype.updateModel = function(classicResultSet, modelJson)
{
	this.m_modelJson = modelJson;
	this.m_resultSet = classicResultSet;
};
oFF.RsSacTableContextResolver.prototype.getCell = function(column, row)
{
	var rowStructure = this.m_modelJson.getListByKey(oFF.SacTableConstants.TD_L_ROWS).getStructureAt(row);
	...
	var cellStructure = rowStructure.getListByKey(oFF.SacTableConstants.R_L_CELLS).getStructureAt(column);
...
{
	return row <= this.getDataRegionCornerRow() - (this.hasRowDimensions() ? 1 : 0);
};
...
oFF.RsSacTableContextResolver.prototype.getColumnTuple = function(column)
{
	...
	var columnAxis = this.m_resultSet.getColumnsAxis();
	if (correctedColumn > -1 && correctedColumn < columnAxis.getTuplesCount())
	{
		return columnAxis.getTupleAt(correctedColumn);
	}
	return null;
};
oFF.RsSacTableContextResolver.prototype.getColumnTuples = function(columnMin, columnMax)
{
	...
	var columnAxis = this.m_resultSet.getColumnsAxis();
	if (correctedColumnMin > -1 && correctedColumnMax < columnAxis.getTuplesCount())
	{
		for (var i = correctedColumnMin; i <= correctedColumnMax; i++)
		{
			result.add(columnAxis.getTupleAt(i));
		}
	}
	return result;
};
oFF.RsSacTableContextResolver.prototype.getRowTuple = function(row)
{
	...
	var rowAxis = this.m_resultSet.getRowsAxis();
	if (correctedRow > -1 && correctedRow < rowAxis.getTuplesCount())
	{
		return rowAxis.getTupleAt(correctedRow);
	}
	return null;
};
oFF.RsSacTableContextResolver.prototype.getRowTuples = function(rowMin, rowMax)
{
	...
	var rowAxis = this.m_resultSet.getRowsAxis();
	if (correctedRowMin > -1 && correctedRowMax < rowAxis.getTuplesCount())
	{
		for (var i = correctedRowMin; i <= correctedRowMax; i++)
		{
			result.add(rowAxis.getTupleAt(i));
		}
	}
	return result;
};
oFF.RsSacTableContextResolver.prototype.getColumnTupleElement = function(column, row)
{
	var correctedColumn = column - this.getDataRegionCornerCol() - 1;
	var columnAxis = this.m_resultSet.getColumnsAxis();
	if (correctedColumn > -1 && correctedColumn < columnAxis.getTuplesCount() && this.isOnHeaderRow(row))
	{
		var dimIndex = this.getCell(this.getDataRegionCornerCol(), row).getIntegerByKey(oFF.SacTableConstants.RCS_N_INDEX);
		var columnTuple = columnAxis.getTupleAt(correctedColumn);
...
oFF.RsSacTableContextResolver.prototype.getColumnTupleElements = function(columnMin, columnMax, row)
{
	...
	var correctedColumnMax = columnMax - this.getDataRegionCornerCol() - 1;
	var columnAxis = this.m_resultSet.getColumnsAxis();
	if (correctedColumnMin > -1 && correctedColumnMax < columnAxis.getTuplesCount() && this.isOnHeaderRow(row))
	{
		var dimIndex = this.getCell(this.getDataRegionCornerCol(), row).getIntegerByKey(oFF.SacTableConstants.RCS_N_INDEX);
		for (var i = correctedColumnMin; i <= correctedColumnMax; i++)
		{
...
oFF.RsSacTableContextResolver.prototype.getRowTupleElement = function(column, row)
{
	var correctedRow = row - this.getDataRegionCornerRow() - 1;
	var rowAxis = this.m_resultSet.getRowsAxis();
	if (correctedRow > -1 && correctedRow < rowAxis.getTuplesCount() && this.isOnHeaderColumn(column))
	{
		var dimIndex = this.getCell(column, this.getDataRegionCornerRow()).getIntegerByKey(oFF.SacTableConstants.RCS_N_INDEX);
		var rowTuple = rowAxis.getTupleAt(correctedRow);
...
oFF.RsSacTableContextResolver.prototype.getRowTupleElements = function(column, rowMin, rowMax)
{
	...
	var correctedRowMax = rowMax - this.getDataRegionCornerRow() - 1;
	var rowAxis = this.m_resultSet.getRowsAxis();
	if (correctedRowMin > -1 && correctedRowMax < rowAxis.getTuplesCount() && this.isOnHeaderColumn(column))
	{
		var dimIndex = this.getCell(column, this.getDataRegionCornerRow()).getIntegerByKey(oFF.SacTableConstants.RCS_N_INDEX);
		for (var i = correctedRowMin; i <= correctedRowMax; i++)
		{
			var rowTuple = rowAxis.getTupleAt(i);
...
oFF.RsSacTableContextResolver.prototype.getColumnTupleFieldName = function(row)
{
	...
	{
		return this.getCell(this.getDataRegionCornerCol(), row).getStringByKey(oFF.SacTableConstants.C_SN_PLAIN);
	}
	return null;
};
oFF.RsSacTableContextResolver.prototype.getRowTupleFieldName = function(column)
{
	...
	{
		return this.getCell(column, this.getDataRegionCornerRow()).getStringByKey(oFF.SacTableConstants.C_SN_PLAIN);
	}
	return null;
};
...
oFF.RsSacTableContextResolver.prototype.getColumnDimension = function(row)
{
	...
	{
		var dimIndex = this.getCell(this.getDataRegionCornerCol(), row).getIntegerByKey(oFF.SacTableConstants.RCS_N_INDEX);
		return columnAxis.getRsDimensions().get(dimIndex);
	}
	return null;
};
oFF.RsSacTableContextResolver.prototype.getColumnDimensions = function(rowMin, rowMax)
{
	...
	if (this.isOnHeaderRow(rowMax))
	{
		...
		{
			var dimIndex = this.getCell(cornerCol, i).getIntegerByKey(oFF.SacTableConstants.RCS_N_INDEX);
			result.add(columnAxis.getRsDimensions().get(dimIndex));
		}
	}
	return result;
};
...
oFF.RsSacTableContextResolver.prototype.getRowDimension = function(column)
{
	...
	{
		var dimIndex = this.getCell(column, this.getDataRegionCornerRow()).getIntegerByKey(oFF.SacTableConstants.RCS_N_INDEX);
		return rowAxis.getRsDimensions().get(dimIndex);
	}
	return null;
};
oFF.RsSacTableContextResolver.prototype.getRowDimensions = function(columnMin, columnMax)
{
	...
	if (this.isOnHeaderColumn(columnMax))
	{
		...
		{
			var dimIndex = this.getCell(i, cornerRow).getIntegerByKey(oFF.SacTableConstants.RCS_N_INDEX);
			result.add(rowAxis.getRsDimensions().get(dimIndex));
		}
	}
	return result;
};
oFF.RsSacTableContextResolver.prototype.getDataCell = function(column, row)
{
	var correctedColumn = column - this.getDataRegionCornerCol() - 1;
	var correctedRow = row - this.getDataRegionCornerRow() - 1;
	if (correctedRow > -1 && correctedRow < this.m_resultSet.getDataRows() && correctedColumn > -1 && correctedColumn < this.m_resultSet.getDataColumns())
	{
		return this.m_resultSet.getDataCell(correctedColumn, correctedRow);
	}
	return null;
};
oFF.RsSacTableContextResolver.prototype.hasRowDimensions = function()
{
	return oFF.XCollectionUtils.hasElements(this.m_resultSet.getRowsAxis().getRsDimensions());
};
...
oFF.RsSacTableRenderer.COLUMN_LIMIT = 50;
...
{
	var testJson = oFF.PrFactory.createStructure();
	testJson.putInteger(oFF.SacTableConstants.TD_N_WIDGET_HEIGHT, 451);
	testJson.putInteger(oFF.SacTableConstants.TD_N_WIDGET_WIDTH, 1257);
	testJson.putNewStructure(oFF.SacTableConstants.TD_M_FEATURE_TOGGLES);
	...
	this.renderTitle(testJson, rs);
	this.renderRows(testJson, rs);
	return testJson;
};
oFF.RsSacTableRenderer.prototype.renderTitle = function(testJson, rs)
{
	var title = testJson.putNewStructure(oFF.SacTableConstants.TD_M_TITLE);
	...
	title.putBoolean(oFF.SacTableConstants.TD_B_TITLE_VISIBLE, true);
	title.putBoolean(oFF.SacTableConstants.TD_B_SUBTITLE_VISIBLE, false);
	title.putBoolean(oFF.SacTableConstants.TD_B_EDITABLE, false);
	titleStyle.putInteger(oFF.SacTableConstants.TS_N_HEIGHT, 52);
	var titleText = rs.getQueryModel().getText();
...
oFF.RsSacTableRenderer.prototype.renderRows = function(testJson, rs)
{
	var rowList = testJson.putNewList(oFF.SacTableConstants.TD_L_ROWS);
	var queryModel = rs.getQueryModel();
	...
	var columnTuplesCount = oFF.XMath.min(cursorColumnAxis.getTuplesCount(), oFF.RsSacTableRenderer.COLUMN_LIMIT);
	var rowTuplesCount = cursorRowAxis.getTuplesCount();
	var columnHeaderAggregation = oFF.SacTableAggregation.getDimHeader(cursorColumnAxis, queryModel);
	var rowHeaderAggregation = oFF.SacTableAggregation.getDimHeader(cursorRowAxis, queryModel);
	var columnKeys = columnHeaderAggregation.getKeysAsReadOnlyListOfString();
	var rowKeys = rowHeaderAggregation.getKeysAsReadOnlyListOfString();
	var preColumnsAmount = rowKeys.size();
	var emptyRowDimensions = preColumnsAmount === 0;
	if (emptyRowDimensions)
	{
		preColumnsAmount = 1;
	}
	var preRowAmount = columnKeys.size();
	var headerRowAmount = preRowAmount + (emptyRowDimensions ? 0 : 1);
	var i;
	var j;
	var cellsList;
	var rowStructure;
	...
	{
		rowStructure = rowList.addNewStructure();
		...
		rowStructure.putBoolean(oFF.SacTableConstants.R_B_FIXED, true);
		cellsList = rowStructure.putNewList(oFF.SacTableConstants.R_L_CELLS);
		for (j = 0; j < preColumnsAmount - 1; j++)
		{
			this.addNewEmptyHeaderCell(cellsList, i, j);
		}
	}
	if (preColumnsAmount > 0)
	{
		for (i = 0; i < preRowAmount; i++)
		{
			this.addHeaderCell(rowList, i, preColumnsAmount - 1, columnHeaderAggregation.getByKey(columnKeys.get(i)), oFF.SacTableConstants.CT_COL_DIM_HEADER, oFF.SacTableConstants.CT_ATTRIBUTE_COL_DIM_HEADER, oFF.SacTableConstants.C_N_HIERARCHY_PADDING_TOP, oFF.SacTableConstants.DF_C_N_HIERARCHY_PADDING_TOP);
		}
	}
	var valueAggregation = null;
	cursorColumnAxis.setTupleCursorBeforeStart();
	j = preColumnsAmount;
	while (cursorColumnAxis.hasNextTuple() && j - preColumnsAmount < oFF.RsSacTableRenderer.COLUMN_LIMIT)
	{
		cursorColumnAxis.nextTuple();
		valueAggregation = oFF.SacTableAggregation.getMemberHeader(cursorColumnAxis, columnHeaderAggregation, valueAggregation);
		for (i = 0; i < preRowAmount; i++)
		{
			valueAggregationElement = valueAggregation.getByKey(columnKeys.get(i));
			rowStructure = rowList.getStructureAt(i);
			var newHeight = oFF.SacTableConstants.DF_R_N_HEIGHT / 2 * (3 + valueAggregationElement.getDisplayLevel());
			...
			this.addHeaderCell(rowList, i, j, valueAggregationElement, oFF.SacTableConstants.CT_COL_DIM_MEMBER, oFF.SacTableConstants.CT_ATTRIBUTE_COL_DIM_MEMBER, oFF.SacTableConstants.C_N_HIERARCHY_PADDING_TOP, oFF.SacTableConstants.DF_C_N_HIERARCHY_PADDING_TOP);
		}
		j++;
	}
	if (!emptyRowDimensions)
	{
		...
		{
			this.addHeaderCell(rowList, preRowAmount, i, rowHeaderAggregation.getByKey(rowKeys.get(i)), oFF.SacTableConstants.CT_ROW_DIM_HEADER, oFF.SacTableConstants.CT_ATTRIBUTE_ROW_DIM_HEADER, oFF.SacTableConstants.C_N_HIERARCHY_PADDING_LEFT, oFF.SacTableConstants.DF_C_N_HIERARCHY_PADDING_LEFT);
		}
		for (i = 0; i < columnTuplesCount; i++)
		{
			this.addNewEmptyHeaderCell(cellsList, preRowAmount, preColumnsAmount + i);
		}
	}
	valueAggregation = null;
	cursorRowAxis.setTupleCursorBeforeStart();
	j = headerRowAmount;
	if (cursorRowAxis.hasNextTuple())
	{
		while (cursorRowAxis.hasNextTuple())
		{
			cursorRowAxis.nextTuple();
			valueAggregation = oFF.SacTableAggregation.getMemberHeader(cursorRowAxis, rowHeaderAggregation, valueAggregation);
			rowStructure = rowList.addNewStructure();
			rowStructure.putInteger(oFF.SacTableConstants.R_N_ROW, j);
			rowStructure.putInteger(oFF.SacTableConstants.R_N_HEIGHT, oFF.SacTableConstants.DF_R_N_HEIGHT);
			rowStructure.putNewList(oFF.SacTableConstants.R_L_CELLS);
			if (emptyRowDimensions)
			{
				for (i = 0; i < preColumnsAmount; i++)
				{
					this.addNewEmptyHeaderCell(rowList.getStructureAt(j).getListByKey(oFF.SacTableConstants.R_L_CELLS), j, i);
				}
			}
			else
			{
				for (i = 0; i < preColumnsAmount; i++)
				{
					valueAggregationElement = valueAggregation.getByKey(rowKeys.get(i));
					dimensionMemberType = valueAggregationElement.getDimensionMemberType();
					if (dimensionMemberType === oFF.MemberType.RESULT || oFF.OlapComponentType.TOTALS === dimensionMemberType)
					{
						this.addToIntMap(this.m_totalRowIndexes, i, j);
					}
					this.addHeaderCell(rowList, j, i, valueAggregationElement, oFF.SacTableConstants.CT_ROW_DIM_MEMBER, oFF.SacTableConstants.CT_ATTRIBUTE_ROW_DIM_MEMBER, oFF.SacTableConstants.C_N_HIERARCHY_PADDING_LEFT, oFF.SacTableConstants.DF_C_N_HIERARCHY_PADDING_LEFT);
				}
			}
			j++;
		}
	...
	{
		rowStructure = rowList.addNewStructure();
		rowStructure.putInteger(oFF.SacTableConstants.R_N_ROW, 0);
		rowStructure.putInteger(oFF.SacTableConstants.R_N_HEIGHT, oFF.SacTableConstants.DF_R_N_HEIGHT);
		rowStructure.putNewList(oFF.SacTableConstants.R_L_CELLS);
	}
	var actualRow;
	var dataColumnsAmount = oFF.XMath.min(rs.getDataColumns(), oFF.RsSacTableRenderer.COLUMN_LIMIT);
	for (i = 0; i < rowTuplesCount; i++)
	{
		actualRow = headerRowAmount + i;
		cellsList = rowList.getStructureAt(actualRow).getListByKey(oFF.SacTableConstants.R_L_CELLS);
		for (j = 0; j < dataColumnsAmount; j++)
		{
			this.addDataCell(cellsList, rs.getDataCell(j, i), j + preColumnsAmount, actualRow);
		}
	}
	testJson.putInteger(oFF.SacTableConstants.TD_N_LAST_ROW_INDEX, headerRowAmount + rowTuplesCount - 1);
	testJson.putInteger(oFF.SacTableConstants.TD_N_DATA_REGION_START_COL, 0);
	testJson.putInteger(oFF.SacTableConstants.TD_N_DATA_REGION_START_ROW, 0);
	testJson.putInteger(oFF.SacTableConstants.TD_N_DATA_REGION_END_COL, preColumnsAmount + columnTuplesCount - 1);
	testJson.putInteger(oFF.SacTableConstants.TD_N_DATA_REGION_END_ROW, headerRowAmount + rowTuplesCount - 1);
	testJson.putInteger(oFF.SacTableConstants.TD_N_DATA_REGION_CORNER_COL, preColumnsAmount - 1);
	testJson.putInteger(oFF.SacTableConstants.TD_N_FREEZE_END_COL, preColumnsAmount - 1);
	testJson.putInteger(oFF.SacTableConstants.TD_N_DATA_REGION_CORNER_ROW, headerRowAmount - 1);
	testJson.putInteger(oFF.SacTableConstants.TD_N_FREEZE_END_ROW, headerRowAmount - 1);
	testJson.putBoolean(oFF.SacTableConstants.TD_B_SHOW_GRID, false);
	testJson.putBoolean(oFF.SacTableConstants.TD_B_SHOW_FREEZE_LINES, true);
	testJson.putBoolean(oFF.SacTableConstants.TD_B_HAS_FIXED_ROWS_COLS, true);
	var columnSettings = testJson.putNewList(oFF.SacTableConstants.TD_L_COLUMN_SETTINGS);
	var columnStructure;
	var preciseWidth;
	for (i = 0; i < preColumnsAmount; i++)
	{
		...
		{
			preciseWidth = this.m_columnWidths.get(i).getInteger() * 9;
		}
		...
		columnStructure.putBoolean(oFF.SacTableConstants.CS_B_FIXED, true);
	}
	for (; i < preColumnsAmount + columnTuplesCount; i++)
	{
		preciseWidth = 125;
		if (i < this.m_columnWidths.size())
		{
			preciseWidth = this.m_columnWidths.get(i).getInteger() * 12;
		}
		columnStructure = columnSettings.addNewStructure();
		columnStructure.putInteger(oFF.SacTableConstants.CS_N_COLUMN, i);
...
oFF.RsSacTableRenderer.prototype.addDataCell = function(cellsList, dataCell, column, row)
{
	...
	this.formatCellByTotals(cell, 5, row, column, "rgba(220,220,150,0,3)");
	this.formatCellByTotals(cell, 4, row, column, "rgba(230,230,150,0,4)");
	this.formatCellByTotals(cell, 3, row, column, "rgba(220,220,135,0.4)");
	this.formatCellByTotals(cell, 2, row, column, "rgba(220,220,135,0.5)");
	this.formatCellByTotals(cell, 1, row, column, "rgba(220,220,220,1)");
	this.formatCellByTotals(cell, 0, row, column, "rgba(204,204,204,1)");
	...
	this.ensureWidth(column, formattedValue);
	var valueException = dataCell.getValueException();
	if (valueException === oFF.ValueException.UNDEFINED || valueException === oFF.ValueException.NULL_VALUE || valueException === oFF.ValueException.NO_PRESENTATION)
	{
		cell.putInteger(oFF.SacTableConstants.C_N_TYPE, oFF.SacTableConstants.CT_UNBOOKED);
	}
	else if (dataCell.getValueType().isNumber())
	{
...
oFF.RsSacTableRenderer.prototype.formatCellByTotals = function(cell, level, row, column, formatString)
{
	var indexValue = oFF.XIntegerValue.create(level);
	if (this.m_totalRowIndexes.containsKey(indexValue) && this.m_totalRowIndexes.getByKey(indexValue).contains(oFF.XIntegerValue.create(row)) || this.m_totalColumnIndexes.containsKey(indexValue) && this.m_totalColumnIndexes.getByKey(indexValue).contains(oFF.XIntegerValue.create(column)))
	{
		cell.putBoolean(oFF.SacTableConstants.C_B_STYLE_UPDATED_BY_USER, true);
		cell.putNewStructure(oFF.SacTableConstants.C_M_STYLE).putString(oFF.SacTableConstants.ST_S_FILL_COLOR, formatString);
	}
};
oFF.RsSacTableRenderer.prototype.addHeaderCell = function(rowList, row, col, headerAggregation, mainType, sideType, hierarchyPaddingElement, hierarchyPaddingValue)
{
	var rowStructure = rowList.getStructureAt(row);
	var cellsList = rowStructure.getListByKey(oFF.SacTableConstants.R_L_CELLS);
	var cell = cellsList.addNewStructure();
	cell.putInteger(oFF.SacTableConstants.C_N_ROW, row);
	cell.putInteger(oFF.SacTableConstants.C_N_COLUMN, col);
	cell.putInteger(oFF.SacTableConstants.C_N_TYPE, headerAggregation.isMainAttribute() ? mainType : sideType);
	var formattedValue = headerAggregation.getText();
	this.ensureWidth(col, formattedValue);
	cell.putString(oFF.SacTableConstants.C_S_FORMATTED, formattedValue);
	...
	cell.putNewStructure(oFF.SacTableConstants.C_M_STYLE).putString(oFF.SacTableConstants.ST_S_FILL_COLOR, "rgba(173, 212, 216, 1)");
	cell.putBoolean(oFF.SacTableConstants.C_B_STYLE_UPDATED_BY_USER, true);
...
oFF.RsSacTableRenderer.prototype.ensureWidth = function(col, formattedValue)
{
	while (this.m_columnWidths.size() < col)
	{
		this.m_columnWidths.add(oFF.XIntegerValue.create(0));
	}
	var newLength = oFF.XString.size(formattedValue);
...
oFF.RsSacTableRenderer.prototype.addNewEmptyHeaderCell = function(cellsList, row, column)
{
	var cell = cellsList.addNewStructure();
	cell.putInteger(oFF.SacTableConstants.C_N_ROW, row);
	cell.putInteger(oFF.SacTableConstants.C_N_COLUMN, column);
...
oFF.SacTableAggregation.create = function(text, plain, dimension, dimensionMemberType, drillState, displayLevel, field, index, isMainAttribute, dimensionIndex)
{
...
oFF.SacTableAggregation.getMemberHeader = function(cursorAxis, dimHeader, previousHeader)
{
	...
	while (cursorAxis.hasNextTupleElement())
	{
		...
		while (cursorAxis.hasNextFieldValue())
		{
			...
			result.put(fieldName, oFF.SacTableAggregation.create(formattedValue, plain, header.getDimension(), tupleElement.getDimensionMemberType(), tupleElement.getDrillState(), displayLevel, header.getField(), header.getIndex(), header.isMainAttribute(), header.getDimensionIndex()));
		}
	}
	return result;
};
oFF.SacTableAggregation.getDimHeader = function(cursorAxis, queryModel)
{
	...
	for (i = 0; i < size; i++)
	{
		...
		if (dimension.isUniversalDisplayHierarchyDimension())
		{
			...
			for (j = 0; j < rsfSize; j++)
			{
				field = resultSetFields.get(j);
				if (j > 0)
				{
					text = "";
				}
				result.put(field.getName(), oFF.SacTableAggregation.create(text, field.getName(), dimension, null, null, -1, field, index, true, i));
				index++;
			}
		}
		else if (dimension.getFieldLayoutType() === oFF.FieldLayoutType.FIELD_BASED)
		{
			...
			for (j = 0; j < rsfSize; j++)
			{
				...
				result.put(field.getName(), oFF.SacTableAggregation.create(text, field.getName(), dimension, null, null, -1, field, index, inMainFields, i));
				index++;
			}
		}
		else
		{
			...
			for (j = 0; j < rsaSize; j++)
			{
				...
				{
					var attributeField = subFields.get(k);
					if (attributeField.getObtainability() !== oFF.ObtainabilityType.USER_INTERFACE)
					{
						result.put(attributeField.getName(), oFF.SacTableAggregation.create(text, attributeField.getName(), dimension, null, null, -1, attributeField, index, isMainAttribute, i));
						text = "";
						index++;
					}
				}
		...
		for (var x = 0; x < dc; x++)
		{
			...
			if (oFF.notNull(rsDC))
			{
				...
				if (oFF.notNull(refStr))
				{
					...
					var sM = cell.getMember();
					if (cell.getMember() !== null && this.m_semStyles.getByKey(sM) !== null)
					{
						cell.setSemantic(this.m_semStyles.getByKey(sM));
					}
				}
...
oFF.ReferenceGrid.SEP_VERT_TOP = "/";
oFF.ReferenceGrid.SEP_VERT_BOTTOM = "~";
oFF.ReferenceGrid.SEP_VERT = ":";
oFF.ReferenceGrid.COLUMN = "|";
oFF.ReferenceGrid.ROW = "~";
oFF.ReferenceGrid.MAGIC_CELL_DIV = "/";
oFF.ReferenceGrid.LINEFEED = "\r\n";
oFF.ReferenceGrid.S_MAX_GRID_SIZE = 100000;
...
oFF.ReferenceGrid.prototype.m_cells = null;
oFF.ReferenceGrid.prototype.m_rowDefs = null;
oFF.ReferenceGrid.prototype.m_colDefs = null;
oFF.ReferenceGrid.prototype.m_fixedWidth = 0;
oFF.ReferenceGrid.prototype.m_fixedHeight = 0;
oFF.ReferenceGrid.prototype.m_resultSet = null;
oFF.ReferenceGrid.prototype.m_createFingerprint = false;
oFF.ReferenceGrid.prototype.m_fingerprint = null;
...
oFF.ReferenceGrid.prototype.checkMaxGridSize = function()
{
	if (oFF.notNull(this.m_cells))
	{
		...
		if (colCount > 0 && rowCount > 0)
		{
			...
			if (size > oFF.ReferenceGrid.S_MAX_GRID_SIZE)
			{
				message.append("Grid size ").appendInt(size);
				message.append(" is above maximum grid size ").appendInt(oFF.ReferenceGrid.S_MAX_GRID_SIZE);
				throw oFF.XException.createRuntimeException(message.toString());
			}
		}
		else
		{
			if (colCount > oFF.ReferenceGrid.S_MAX_GRID_SIZE || rowCount > oFF.ReferenceGrid.S_MAX_GRID_SIZE)
			{
				if (colCount > oFF.ReferenceGrid.S_MAX_GRID_SIZE)
				{
...
oFF.ReferenceGrid.prototype.setMaxColumnSize = function(maxCellSize, columns, rowStart, maxRowCount)
{
	var colCount = this.m_cells.size0();
...
oFF.ReferenceGrid.prototype.exportToAsciiExt = function(maxCellSize, useRowsHeaderPane, useColumnsHeaderPane, rowStart, maxRowCount, columnStart, maxColumnCount)
{
	...
	while (idxRow < totalRows && (maxRowCount === -1 || rowCounter < maxRowCount))
	{
		if (rowCounter > 0)
		{
			buffer.append(oFF.ReferenceGrid.LINEFEED);
		}
		if (idxRow === this.m_fixedHeight && useColumnsHeaderPane)
		{
			buffer.append(this.renderLine(linesBySeparator, columns, oFF.ReferenceGrid.SEP_VERT_TOP, columnStart, maxColumnCount));
			buffer.append(oFF.ReferenceGrid.LINEFEED);
		}
		var colCounter = 0;
		...
		while (idxCol < totalCols && (maxColumnCount === -1 || colCounter < maxColumnCount))
		{
			if (colCounter > 0)
			{
				...
				{
					buffer.append(oFF.ReferenceGrid.SEP_VERT);
				}
				else
				{
					buffer.append(oFF.ReferenceGrid.COLUMN);
				}
			}
	...
	{
		buffer.append(oFF.ReferenceGrid.LINEFEED);
		buffer.append(this.renderLine(linesBySeparator, columns, oFF.ReferenceGrid.SEP_VERT_BOTTOM, columnStart, maxColumnCount));
	}
	return buffer.toString();
};
...
oFF.ReferenceGrid.prototype.prepareStructure = function(useRowsHeaderPane, useColumnsHeaderPane)
{
	...
	{
		this.m_fixedHeight = oFF.XMath.max(this.getEffectiveFieldSize(columnsAxis), this.m_renderForFireflyGrid && this.getEffectiveFieldSize(rowsAxis) === 0 ? 0 : 1);
	}
	else
	{
	...
	{
		this.m_fixedWidth = oFF.XMath.max(this.getEffectiveFieldSize(rowsAxis), 1);
	}
	else
	{
		this.m_fixedWidth = 0;
	...
	for (var x = 0; x < totalColumns; x++)
	{
		this.m_colDefs.add(oFF.RgLineDef.create());
	}
	for (var y = 0; y < totalRows; y++)
	{
		this.m_rowDefs.add(oFF.RgLineDef.create());
	}
...
oFF.ReferenceGrid.prototype.setTitleCells = function(axis, offset, complementaryAxisEmpty)
{
	var type = axis.getType();
	var rsDimensions = axis.getRsDimensions();
	var position = 0;
	...
	for (var idxDim = 0; idxDim < dimSize; idxDim++)
	{
		...
		for (var idxField = 0; idxField < fieldSize; idxField++)
		{
			...
			if (type === oFF.AxisType.ROWS)
			{
				...
				{
					var titleCell = oFF.ReferenceGridCell.create(buffer.toString(), oFF.RgCellType.TITLE);
					titleCell.setDisplayValue(bufferDV.toString());
					titleCell.setDimension(rsDimension.getName());
					titleCell.setPart("Rows");
					titleCell.setRow(offset);
					titleCell.setColumn(position);
					this.setCell(position, offset, titleCell, false);
				}
			...
			else
			{
				if (!(this.m_renderForFireflyGrid && rsDimension.isStructure() && dimSize === idxDim + 1))
				{
					...
					var titCell = oFF.ReferenceGridCell.create(buffer.toString(), oFF.RgCellType.TITLE);
					if (this.m_renderForFireflyGrid)
					{
						titCell.setRow(offset);
						titCell.setColumn(position);
						titCell.setDisplayValue(bufferDV.toString());
						if (oFF.notNull(existingCell))
						{
					...
					this.setCell(offset, position, titCell, true);
				}
				else if (complementaryAxisEmpty)
				{
					this.m_fixedWidthOff = 1;
...
oFF.ReferenceGrid.prototype.setDataCells = function()
{
	...
	for (var y = 0; y < dr; y++)
	{
		for (var x = 0; x < dc; x++)
		{
			...
			if (this.m_withDetails)
			{
				cell = oFF.ReferenceGridCell.create(this.getCellValueWithDetails(dataCell, readinessStates), oFF.RgCellType.DATA);
			}
			else
			{
				cell = oFF.ReferenceGridCell.create(this.getCellValue(this.formatDataCellValue(dataCell), dataCell), oFF.RgCellType.DATA);
			}
			if (this.m_renderForFireflyGrid)
			{
				sb.clear();
			...
			this.setCell(x + this.m_fixedWidth - this.m_fixedWidthOff, y + this.m_fixedHeight - this.m_fixedHeightOff, cell, false);
		}
	}
};
...
oFF.ReferenceGrid.prototype.setHeaderCells = function(axis, tupleOffset)
{
	...
	var formattedValue = oFF.XStringBuffer.create();
	var displayValue = oFF.XStringBuffer.create();
	var emptyHeaderCell = oFF.ReferenceGridCell.create(null, oFF.RgCellType.HEADER);
	var emptyHeaderTotalsCell = oFF.ReferenceGridCell.create(null, oFF.RgCellType.HEADER);
	for (var tupleIndex = 0; tupleIndex < tuplesCount; tupleIndex++)
	{
		var tuple = axis.getTupleAt(tupleIndex);
		var position = 0;
			...
			if (element.getFirstTuple() === tuple || this.m_renderForFireflyGrid === true)
			{
				...
				if (memberType.isResult())
				{
					...
					if (visibleFieldCount > 0)
					{
						...
						if (!hasTotals)
						{
							...
							this.appendAlertLevelToValue(element, formattedValue);
							var totalsCell = oFF.ReferenceGridCell.create(formattedValue.toString(), oFF.RgCellType.HEADER);
							if (this.m_renderForFireflyGrid)
							{
				...
				else
				{
					for (var fieldIndex = 0; fieldIndex < visibleFieldCount; fieldIndex++)
					{
						...
						var gridCell = oFF.ReferenceGridCell.create(formattedValue.toString(), oFF.RgCellType.HEADER);
						if (this.m_renderForFireflyGrid)
						{
							if (axisType === oFF.AxisType.ROWS)
							{
...
oFF.ReferenceGrid.prototype.enableTotals = function(axisType, position)
{
	...
	if (axisType === oFF.AxisType.ROWS)
	{
		rgLineDef = this.m_rowDefs.get(position);
	}
	else
	{
		rgLineDef = this.m_colDefs.get(position);
	}
...
oFF.ReferenceGrid.prototype.addCell = function(axisType, position, tupleIndex, tupleOffset, cell)
{
	if (axisType === oFF.AxisType.ROWS)
	{
		...
		this.setCell(position, tupleIndex + tupleOffset, cell, false);
	}
	else
	{
		cell.setRow(position);
		cell.setColumn(tupleIndex + tupleOffset);
		this.setCell(tupleIndex + tupleOffset, position, cell, false);
	}
};
...
oFF.ReferenceGrid.prototype.renderLine = function(linesBySeparator, columns, verticalSeparator, columnStart, maxColumnCount)
{
	...
	if (oFF.isNull(line))
	{
		...
		for (var idxCol = columnStart; idxCol < totalColumns && (maxColumnCount === -1 || colCount < maxColumnCount); idxCol++)
		{
			if (colCount > 0)
			{
				...
				{
					buffer.append(oFF.ReferenceGrid.ROW);
				}
			}
			var rest = columns.get(idxCol);
...
oFF.ReferenceGridCell = function() {};
oFF.ReferenceGridCell.prototype = new oFF.XObject();
oFF.ReferenceGridCell.prototype._ff_c = "ReferenceGridCell";

oFF.ReferenceGridCell.create = function(content, cellType)
{
	var object = new oFF.ReferenceGridCell();
	object.setupExt(content, cellType);
	return object;
};
oFF.ReferenceGridCell.prototype.m_content = null;
oFF.ReferenceGridCell.prototype.m_isLeftAligned = false;
oFF.ReferenceGridCell.prototype.m_isTotal = false;
oFF.ReferenceGridCell.prototype.m_drillState = null;
oFF.ReferenceGridCell.prototype.m_displayLevel = 0;
oFF.ReferenceGridCell.prototype.m_displayValue = null;
oFF.ReferenceGridCell.prototype.m_cellType = null;
oFF.ReferenceGridCell.prototype.m_dataValueId = null;
oFF.ReferenceGridCell.prototype.m_dimName = null;
oFF.ReferenceGridCell.prototype.m_memName = null;
oFF.ReferenceGridCell.prototype.m_memName2 = null;
oFF.ReferenceGridCell.prototype.m_rowIndex = 0;
oFF.ReferenceGridCell.prototype.m_colIndex = 0;
oFF.ReferenceGridCell.prototype.m_icon = null;
oFF.ReferenceGridCell.prototype.m_part = null;
oFF.ReferenceGridCell.prototype.m_alertLevel = null;
oFF.ReferenceGridCell.prototype.m_input = false;
oFF.ReferenceGridCell.prototype.m_row = 0;
oFF.ReferenceGridCell.prototype.m_col = 0;
oFF.ReferenceGridCell.prototype.setupExt = function(content, cellType)
{
	this.m_content = content;
	this.m_isLeftAligned = cellType !== oFF.RgCellType.DATA;
	this.m_cellType = cellType;
};
oFF.ReferenceGridCell.prototype.releaseObject = function()
{
	this.m_content = null;
	this.m_cellType = null;
	oFF.XObject.prototype.releaseObject.call( this );
};
oFF.ReferenceGridCell.prototype.getCharacterCount = function()
{
	if (oFF.isNull(this.m_content))
	{
		return 0;
...
oFF.ReferenceGridCell.prototype.isLeftAligned = function()
{
	return this.m_isLeftAligned;
};
oFF.ReferenceGridCell.prototype.getText = function(max)
{
	if (oFF.isNull(this.m_content))
	{
		return "";
...
oFF.ReferenceGridCell.prototype.getCellType = function()
{
	return this.m_cellType;
};
oFF.ReferenceGridCell.prototype.isTotals = function()
{
	return this.m_isTotal;
};
oFF.ReferenceGridCell.prototype.setIsTotal = function(isTotal)
{
	this.m_isTotal = isTotal;
};
oFF.ReferenceGridCell.prototype.setDisplayLevel = function(displayLevel)
{
	this.m_displayLevel = displayLevel;
};
oFF.ReferenceGridCell.prototype.setDrillState = function(drillState)
{
	this.m_drillState = drillState;
};
oFF.ReferenceGridCell.prototype.toString = function()
{
	if (oFF.isNull(this.m_content))
	{
		return "";
...
oFF.ReferenceGridCell.prototype.getDisplayLevel = function()
{
	return this.m_displayLevel;
};
oFF.ReferenceGridCell.prototype.getDrillState = function()
{
	return this.m_drillState;
};
oFF.ReferenceGridCell.prototype.setDisplayValue = function(displayValue)
{
	this.m_displayValue = displayValue;
};
oFF.ReferenceGridCell.prototype.getDisplayValue = function()
{
	return this.m_displayValue;
};
oFF.ReferenceGridCell.prototype.setDataValueId = function(dataValueId)
{
	this.m_dataValueId = dataValueId;
};
oFF.ReferenceGridCell.prototype.getDataValueId = function()
{
	return this.m_dataValueId;
};
oFF.ReferenceGridCell.prototype.getDimension = function()
{
	return this.m_dimName;
};
oFF.ReferenceGridCell.prototype.setDimension = function(value)
{
	this.m_dimName = value;
};
oFF.ReferenceGridCell.prototype.getMember = function()
{
	return this.m_memName;
};
oFF.ReferenceGridCell.prototype.setMember = function(value)
{
	this.m_memName = value;
};
oFF.ReferenceGridCell.prototype.getMember2 = function()
{
	return this.m_memName2;
};
oFF.ReferenceGridCell.prototype.setMember2 = function(value)
{
	this.m_memName2 = value;
};
oFF.ReferenceGridCell.prototype.getRowTupleIndex = function()
{
	return this.m_rowIndex;
};
oFF.ReferenceGridCell.prototype.setRowTupleIndex = function(index)
{
	this.m_rowIndex = index;
};
oFF.ReferenceGridCell.prototype.getColumnTupleIndex = function()
{
	return this.m_colIndex;
};
oFF.ReferenceGridCell.prototype.setColumnTupleIndex = function(index)
{
	this.m_colIndex = index;
};
oFF.ReferenceGridCell.prototype.getIcon = function()
{
	return this.m_icon;
};
oFF.ReferenceGridCell.prototype.setIcon = function(icon)
{
	this.m_icon = icon;
};
oFF.ReferenceGridCell.prototype.getPart = function()
{
	return this.m_part;
};
oFF.ReferenceGridCell.prototype.setPart = function(part)
{
	this.m_part = part;
};
oFF.ReferenceGridCell.prototype.getAlertLevel = function()
{
	return this.m_alertLevel;
};
oFF.ReferenceGridCell.prototype.setAlertLeve = function(alertLevel)
{
	this.m_alertLevel = alertLevel;
};
oFF.ReferenceGridCell.prototype.getInput = function()
{
	return this.m_input;
};
oFF.ReferenceGridCell.prototype.setInput = function(bInput)
{
	this.m_input = bInput;
};
oFF.ReferenceGridCell.prototype.getColumn = function()
{
	return this.m_col;
};
oFF.ReferenceGridCell.prototype.setColumn = function(column)
{
	this.m_col = column;
};
oFF.ReferenceGridCell.prototype.getRow = function()
{
	return this.m_row;
};
oFF.ReferenceGridCell.prototype.setRow = function(row)
{
	this.m_row = row;
};
...
oFF.RgLineDef = function() {};
oFF.RgLineDef.prototype = new oFF.XObject();
oFF.RgLineDef.prototype._ff_c = "RgLineDef";

oFF.RgLineDef.create = function()
{
	return new oFF.RgLineDef();
};
oFF.RgLineDef.prototype.m_hasTotals = false;
oFF.RgLineDef.prototype.hasTotals = function()
{
	return this.m_hasTotals;
};
oFF.RgLineDef.prototype.setHasTotals = function(hasTotals)
{
	this.m_hasTotals = hasTotals;
};
oFF.RgLineDef.prototype.toString = function()
{
	return oFF.XStringBuffer.create().append("Totals:").appendBoolean(this.m_hasTotals).toString();
};
