...
{
	var appTabBar = this.m_tridentClient.createUiElement(oFF.UiType.TAB_STRIP);
...
oFF.Atlas.prototype.createHomeTab = function(tabBar)
{
	...
	urlInput.setWidthExt(350, oFF.UiSizeUnit.PIXEL);
...
oFF.Atlas.prototype.createSettingsTab = function(tabBar)
{
	...
	spacerLayoutDrp.setHeightValue(20);
	spacerLayoutDrp.setHeightUnit(oFF.UiSizeUnit.PIXEL);
	...
	logLevelDropdown.setWidthExt(350, oFF.UiSizeUnit.PIXEL);
	logLevelDropdown.setHeightExt(125, oFF.UiSizeUnit.PIXEL);
...
oFF.Atlas.prototype.loadStory = function(story)
{
	...
	if (pageCount > 0)
	{
		...
		var tabBar = selectedContainer.setNewContent(oFF.UiType.TAB_STRIP).useMaxSpace();
...
{
	if (event.getControl().getUiType() === oFF.UiType.TAB_STRIP)
	{
...
oFF.Asteria.prototype.onPress = function(event)
{
	...
	if (oFF.notNull(appFile))
	{
		...
		var subGenesis = oFF.UiGenesis.create(newView, oFF.UiPosition.CONTENT, oFF.UiOperation.SET, 0, 0);
...
oFF.AtlasSimpleProgram.prototype.showProgressMessage = function(text, isError)
{
	if (oFF.notNull(this.m_activityIndicator))
	{
		...
		else
		{
			...
			errorIcon.setFontColor(oFF.UiColor.RED);
			errorIcon.setSizeValue(30, 30);
...
oFF.AtlasSimpleProgram.prototype.onOrcaServiceCreated = function(extResult, orcaService, customIdentifier)
{
	if (extResult.isValid())
	{
		...
		this.m_orcaService = orcaService;
		orcaService.processSystemLandscapeLoad(null, this, null);
	}
	else
	{
		this.log("#1 failure logon");
...
oFF.PoseidonClient.prototype.getDefaultProgramName = function()
{
	return "PoseidonClient";
};
oFF.PoseidonClient.prototype.isShouldWindowOpenMaximized = function()
{
	return false;
};
...
{
	this.m_genesis = oFF.UiGenesis.create(uiParent, oFF.UiPosition.CONTENT, oFF.UiOperation.SET, 0, 0);
...
oFF.AppGallery.prototype.onPress = function(event)
{
	...
	else
	{
		...
		var subGenesis = oFF.UiGenesis.create(newTab, oFF.UiPosition.CONTENT, oFF.UiOperation.SET, 0, 0);
		poseidonClient.buildUi(subGenesis);
	}
};
