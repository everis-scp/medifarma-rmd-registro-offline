...
oFF.OrcaSystemConverter.prototype.convert = function(connection, connectionsJson)
{
	...
	if (oFF.notNull(systems))
	{
		...
		for (var index = 0; index < names.size(); index++)
		{
			...
			if (!oFF.XString.isEqual(oFF.OcOrcaServiceConfig.ORCA_MASTER_SYSTEM_NAME, sysName) && !oFF.XString.isEqual(masterSystem.getSystemName(), sysName))
			{
				var currentSystemDesc = systems.getStructureByKey(names.get(index));
				var systemDescProperties = this.convertToProperties(currentSystemDesc, masterSystem);
...
oFF.OrcaSystemConverter.prototype.convertToProperties = function(input, masterSystem)
{
	...
	if (oFF.notNull(authMethod))
	{
		if (oFF.XString.isEqual(authMethod, "NOAUTH"))
		{
...
oFF.OcSystemsLoadAction.createAndRun = function(syncType, listener, customIdentifier, context)
{
...
oFF.OcSystemsLoadAction.prototype.onFunctionExecuted = function(extResult, response, customIdentifier)
{
	...
	if (extResult.isValid())
	{
		...
		{
			var systemDescriptions = oFF.OrcaSystemConverter.create().convert(this.getActionContext().getConnection(), connData);
...
oFF.OcOrcaService.prototype.processSystemLandscapeLoad = function(syncType, listener, customIdentifier)
{
	return oFF.OcSystemsLoadAction.createAndRun(syncType, listener, customIdentifier, this);
};
