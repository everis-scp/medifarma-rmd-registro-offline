...
oFF.XStream = {
	...
	of:function(values)
	{
			return oFF.XStreamReference.create(values);
	}
};
...
oFF.XStreamCollectorImpl.prototype.apply = function(valueIterator)
{
	...
	{
		this.m_applyValueFunction(result, valueIterator.next());
	}
	return result;
};
...
oFF.XStreamReference.create = function(values)
{
	var streamReference = new oFF.XStreamReference();
	streamReference.m_values = oFF.notNull(values) ? values.getValuesAsReadOnlyList() : null;
...
oFF.XStreamReference.prototype.hasNext = function()
{
	...
	{
		this.applyNextValue(this.m_values.get(this.m_index));
		this.m_index++;
		return true;
	}
	return false;
