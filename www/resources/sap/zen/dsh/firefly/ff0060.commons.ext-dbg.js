...
{
	if (oFF.XStringUtils.isNullOrEmpty(colorString))
	{
		return null;
	}
	var rgba = new oFF.UiColor();
...
{
	return oFF.UiColor.create(this.getRed(), this.getGreen(), this.getBlue(), alpha);
};
oFF.UiColor.prototype.newBrighterColor = function(factor)
{
	...
	return oFF.UiColor.create(newRed, newGreen, newBlue, this.getAlpha());
};
...
oFF.UiColor.prototype.setColorByString = function(colorString)
{
	if (oFF.notNull(colorString))
	{
		...
		else if (oFF.XString.startsWith(colorString, "rgb") || oFF.XString.startsWith(colorString, "rgba"))
		{
			this.setColorByRgb(colorString);
		}
		else
		{
			var tmpColor = oFF.UiColor.lookup(colorString);
...
oFF.UiColor.prototype.getRgbaColor = function()
{
	...
	var alphaStr4 = oFF.XStringUtils.concatenate2(oFF.XDouble.convertToString(this.m_alpha), ")");
...
oFF.UiColor.prototype.setColorByHex = function(hexString)
{
	...
	{
		var rCol1 = oFF.XInteger.convertFromStringWithRadix(oFF.XString.substring(hexString, 0 + offset, 2 + offset), 16);
		var gCol1 = oFF.XInteger.convertFromStringWithRadix(oFF.XString.substring(hexString, 2 + offset, 4 + offset), 16);
		var bCol1 = oFF.XInteger.convertFromStringWithRadix(oFF.XString.substring(hexString, 4 + offset, 6 + offset), 16);
		this.setColor(rCol1, gCol1, bCol1, 1.0);
	}
	else if (oFF.XString.size(hexString) === 8 + offset)
	{
		var alpha2 = oFF.XInteger.convertFromStringWithRadix(oFF.XString.substring(hexString, 0 + offset, 2 + offset), 16);
		var newAlpha = alpha2 / 255.0;
		var rCol2 = oFF.XInteger.convertFromStringWithRadix(oFF.XString.substring(hexString, 2 + offset, 4 + offset), 16);
		var gCol2 = oFF.XInteger.convertFromStringWithRadix(oFF.XString.substring(hexString, 4 + offset, 6 + offset), 16);
		var bCol2 = oFF.XInteger.convertFromStringWithRadix(oFF.XString.substring(hexString, 6 + offset, 8 + offset), 16);
...
oFF.UiUnitValue = function() {};
oFF.UiUnitValue.prototype = new oFF.XObject();
oFF.UiUnitValue.prototype._ff_c = "UiUnitValue";

oFF.UiUnitValue.MAXIMUM = -99990;
oFF.UiUnitValue.MINIMUM = -99991;
oFF.UiUnitValue.AUTOMATIC = -99999;
oFF.UiUnitValue.create = function(value, unit)
{
	var newObject = new oFF.UiUnitValue();
	...
	{
		newObject.setUnit(oFF.UiSizeUnit.PIXEL);
	}
	return newObject;
};
oFF.UiUnitValue.createByCss = function(cssValue)
{
...
oFF.UiUnitValue.prototype.m_value = 0;
oFF.UiUnitValue.prototype.m_unit = null;
oFF.UiUnitValue.prototype.getValue = function()
{
	return this.m_value;
};
oFF.UiUnitValue.prototype.setValue = function(value)
{
	this.m_value = value;
};
oFF.UiUnitValue.prototype.getUnit = function()
{
	return this.m_unit;
};
oFF.UiUnitValue.prototype.setUnit = function(unit)
{
	this.m_unit = unit;
};
oFF.UiUnitValue.prototype.setCssValue = function(cssValue)
{
	if (oFF.XStringUtils.isNullOrEmpty(cssValue))
	{
		this.m_value = oFF.UiUnitValue.AUTOMATIC;
		this.m_unit = oFF.UiSizeUnit.AUTO;
		return;
	}
	var size = oFF.XString.size(cssValue);
	var startNotNumber = size;
	for (var i = 0; i < size; i++)
	{
		var c = oFF.XString.getCharAt(cssValue, i);
		if (c <= 47 || c >= 58)
		{
			startNotNumber = i;
			break;
	...
	var number = oFF.XString.substring(cssValue, 0, startNotNumber);
	var numberValue = 0;
	if (oFF.XStringUtils.isNotNullAndNotEmpty(number))
	{
		numberValue = oFF.XInteger.convertFromString(number);
	}
	var unit = oFF.XString.substring(cssValue, startNotNumber, -1);
	var unitValue = null;
	if (oFF.XStringUtils.isNotNullAndNotEmpty(unit))
	{
		unitValue = oFF.UiSizeUnit.lookup(unit);
	}
	if (oFF.isNull(unitValue))
	{
		unitValue = oFF.UiSizeUnit.PIXEL;
	}
	if (unitValue === oFF.UiSizeUnit.AUTO)
	{
		numberValue = oFF.UiUnitValue.AUTOMATIC;
	}
	this.m_value = numberValue;
	this.m_unit = unitValue;
};
oFF.UiUnitValue.prototype.getCssValue = function()
{
	if (this.isAutomatic())
	{
		return oFF.UiSizeUnit.AUTO.getDocumentName();
	}
	if (this.isMaximum())
	{
		return "100%";
	...
	var buffer = oFF.XStringBuffer.create().appendInt(this.m_value);
	...
	{
		buffer.append(oFF.UiSizeUnit.PIXEL.getDocumentName());
	}
	var cssValue = buffer.toString();
	return cssValue;
};
oFF.UiUnitValue.prototype.isMinimum = function()
{
	return this.m_value === oFF.UiUnitValue.MINIMUM || this.m_value === 0;
};
oFF.UiUnitValue.prototype.isMaximum = function()
{
	return this.m_value === oFF.UiUnitValue.MAXIMUM || this.m_value === 100 && this.m_unit === oFF.UiSizeUnit.PERCENT;
};
oFF.UiUnitValue.prototype.isAutomatic = function()
{
	return this.m_value === oFF.UiUnitValue.AUTOMATIC || this.m_unit === oFF.UiSizeUnit.AUTO;
};
oFF.UiUnitValue.prototype.toString = function()
{
...
oFF.UiPositionValue = function() {};
oFF.UiPositionValue.prototype = new oFF.XObject();
oFF.UiPositionValue.prototype._ff_c = "UiPositionValue";

oFF.UiPositionValue.create = function(x, y)
{
	var newObj = new oFF.UiPositionValue();
	newObj.setXValue(x);
	newObj.setYValue(y);
	return newObj;
};
oFF.UiPositionValue.createExt = function(x, y)
{
	var newObj = new oFF.UiPositionValue();
	newObj.setX(x);
	newObj.setY(y);
...
oFF.UiPositionValue.createByCss = function(xCss, yCss)
{
	var newObj = new oFF.UiPositionValue();
	newObj.setX(oFF.UiUnitValue.createByCss(xCss));
	newObj.setY(oFF.UiUnitValue.createByCss(yCss));
	return newObj;
};
oFF.UiPositionValue.createByPositionString = function(positionStr)
{
	var newObj = new oFF.UiPositionValue();
	newObj.setFromString(positionStr);
	return newObj;
};
oFF.UiPositionValue.createEmpty = function()
{
	return oFF.UiPositionValue.createByPositionString(null);
};
oFF.UiPositionValue.prototype.m_x = null;
oFF.UiPositionValue.prototype.m_y = null;
oFF.UiPositionValue.prototype.setByValue = function(xValue, yValue)
{
	this.setXValue(xValue);
	this.setYValue(yValue);
};
oFF.UiPositionValue.prototype.setByCss = function(xCss, yCss)
{
	this.setX(oFF.UiUnitValue.createByCss(xCss));
	this.setY(oFF.UiUnitValue.createByCss(yCss));
};
oFF.UiPositionValue.prototype.setXValue = function(value)
{
	this.setX(oFF.UiUnitValue.create(value, oFF.UiSizeUnit.PIXEL));
};
oFF.UiPositionValue.prototype.getXValue = function()
{
	var xObj = this.getX();
	if (oFF.notNull(xObj))
	{
		return xObj.getValue();
	}
	return 0;
};
oFF.UiPositionValue.prototype.getXUnit = function()
{
	var xObj = this.getX();
	if (oFF.notNull(xObj))
	{
...
oFF.UiPositionValue.prototype.getXCss = function()
{
	var xObj = this.getX();
	if (oFF.notNull(xObj))
	{
		return xObj.getCssValue();
	}
	return null;
};
oFF.UiPositionValue.prototype.setXUnit = function(unit)
{
	this.setX(oFF.UiUnitValue.create(this.getXValue(), unit));
};
oFF.UiPositionValue.prototype.setXCss = function(xCss)
{
	this.setX(oFF.UiUnitValue.createByCss(xCss));
};
oFF.UiPositionValue.prototype.setXExt = function(x, unit)
{
	this.setX(oFF.UiUnitValue.create(x, unit));
};
oFF.UiPositionValue.prototype.setX = function(x)
{
	this.m_x = x;
};
oFF.UiPositionValue.prototype.getX = function()
{
	return this.m_x;
};
oFF.UiPositionValue.prototype.setYValue = function(value)
{
	this.setY(oFF.UiUnitValue.create(value, oFF.UiSizeUnit.PIXEL));
};
oFF.UiPositionValue.prototype.getYValue = function()
{
	var yObj = this.getY();
	if (oFF.notNull(yObj))
	{
		return yObj.getValue();
	}
	return 0;
};
oFF.UiPositionValue.prototype.getYUnit = function()
{
	var yObj = this.getY();
	if (oFF.notNull(yObj))
	{
...
oFF.UiPositionValue.prototype.getYCss = function()
{
	var yObj = this.getY();
	if (oFF.notNull(yObj))
	{
		return yObj.getCssValue();
	}
	return null;
};
oFF.UiPositionValue.prototype.setYUnit = function(unit)
{
	this.setY(oFF.UiUnitValue.create(this.getYValue(), unit));
};
oFF.UiPositionValue.prototype.setYCss = function(yCss)
{
	this.setY(oFF.UiUnitValue.createByCss(yCss));
};
oFF.UiPositionValue.prototype.setYExt = function(y, unit)
{
	this.setY(oFF.UiUnitValue.create(y, unit));
};
oFF.UiPositionValue.prototype.setY = function(y)
{
	this.m_y = y;
};
oFF.UiPositionValue.prototype.getY = function()
{
	return this.m_y;
};
oFF.UiPositionValue.prototype.getAsString = function()
{
	var xStr = null;
	var yStr = null;
	if (this.getX() !== null)
	...
	return oFF.XStringUtils.concatenate3(xStr, oFF.UiSizeValue.POSITION_SIZE_SEPARATOR, yStr);
};
oFF.UiPositionValue.prototype.setFromString = function(postionStr)
{
	var positions = oFF.XStringTokenizer.splitString(postionStr, oFF.UiSizeValue.POSITION_SIZE_SEPARATOR);
	if (oFF.notNull(positions) && positions.size() === 2)
	{
		var newPosX = positions.get(0);
		var newPosY = positions.get(1);
		this.setX(oFF.UiUnitValue.createByCss(newPosX));
		this.setY(oFF.UiUnitValue.createByCss(newPosY));
	}
};

oFF.UiSizeValue = function() {};
oFF.UiSizeValue.prototype = new oFF.XObject();
oFF.UiSizeValue.prototype._ff_c = "UiSizeValue";

oFF.UiSizeValue.POSITION_SIZE_SEPARATOR = ",";
oFF.UiSizeValue.create = function(width, height)
{
	var newObj = new oFF.UiSizeValue();
	newObj.setWidthValue(width);
	newObj.setHeightValue(height);
	return newObj;
};
oFF.UiSizeValue.createExt = function(width, height)
{
	var newObj = new oFF.UiSizeValue();
...
oFF.UiSizeValue.createByCss = function(widthCss, heightCss)
{
	var newObj = new oFF.UiSizeValue();
	newObj.setWidth(oFF.UiUnitValue.createByCss(widthCss));
	newObj.setHeight(oFF.UiUnitValue.createByCss(heightCss));
	return newObj;
};
oFF.UiSizeValue.createBySizeString = function(sizeStr)
{
	var newObj = new oFF.UiSizeValue();
	newObj.setFromString(sizeStr);
	return newObj;
};
oFF.UiSizeValue.createEmpty = function()
{
	return oFF.UiSizeValue.createBySizeString(null);
};
oFF.UiSizeValue.prototype.m_width = null;
oFF.UiSizeValue.prototype.m_height = null;
oFF.UiSizeValue.prototype.setByValue = function(widthValue, heightValue)
{
	this.setWidthValue(widthValue);
	this.setHeightValue(heightValue);
};
oFF.UiSizeValue.prototype.setByCss = function(widthCss, heightCss)
{
	this.setWidthCss(widthCss);
	this.setHeightCss(heightCss);
};
oFF.UiSizeValue.prototype.getWidthValue = function()
{
	var widthObj = this.getWidth();
	if (oFF.notNull(widthObj))
	{
		return widthObj.getValue();
	}
	return 0;
};
oFF.UiSizeValue.prototype.getWidthUnit = function()
{
	var widthObj = this.getWidth();
	if (oFF.notNull(widthObj))
	{
		return widthObj.getUnit();
	}
	return null;
};
oFF.UiSizeValue.prototype.getWidthCss = function()
{
	var widthObj = this.getWidth();
	if (oFF.notNull(widthObj))
	{
		return widthObj.getCssValue();
	}
	return null;
};
oFF.UiSizeValue.prototype.getWidth = function()
{
	return this.m_width;
};
oFF.UiSizeValue.prototype.setWidthValue = function(value)
{
	this.setWidth(oFF.UiUnitValue.create(value, this.getWidthUnit()));
};
oFF.UiSizeValue.prototype.setWidthUnit = function(unit)
{
	this.setWidth(oFF.UiUnitValue.create(this.getWidthValue(), unit));
};
oFF.UiSizeValue.prototype.setWidthExt = function(width, unit)
{
	this.setWidth(oFF.UiUnitValue.create(width, unit));
};
oFF.UiSizeValue.prototype.setWidthCss = function(width)
{
	this.setWidth(oFF.UiUnitValue.createByCss(width));
};
oFF.UiSizeValue.prototype.setWidth = function(width)
{
	this.m_width = width;
};
oFF.UiSizeValue.prototype.getHeightValue = function()
{
	var heightObj = this.getHeight();
	if (oFF.notNull(heightObj))
	{
		return heightObj.getValue();
	}
	return 0;
};
oFF.UiSizeValue.prototype.getHeightUnit = function()
{
	var heightObj = this.getHeight();
	if (oFF.notNull(heightObj))
	{
		return heightObj.getUnit();
	}
	return null;
};
oFF.UiSizeValue.prototype.getHeightCss = function()
{
	var heightObj = this.getHeight();
	if (oFF.notNull(heightObj))
	{
		return heightObj.getCssValue();
	}
	return null;
};
oFF.UiSizeValue.prototype.getHeight = function()
{
	return this.m_height;
};
oFF.UiSizeValue.prototype.setHeightValue = function(value)
{
	this.setHeight(oFF.UiUnitValue.create(value, this.getHeightUnit()));
};
oFF.UiSizeValue.prototype.setHeightUnit = function(unit)
{
	this.setHeight(oFF.UiUnitValue.create(this.getHeightValue(), unit));
};
oFF.UiSizeValue.prototype.setHeightCss = function(height)
{
	this.setHeight(oFF.UiUnitValue.createByCss(height));
};
oFF.UiSizeValue.prototype.setHeightExt = function(height, unit)
{
	this.setHeight(oFF.UiUnitValue.create(height, unit));
};
oFF.UiSizeValue.prototype.setHeight = function(height)
{
	this.m_height = height;
};
oFF.UiSizeValue.prototype.getAsString = function()
{
	var widthStr = null;
	var heightStr = null;
	...
	return oFF.XStringUtils.concatenate3(widthStr, oFF.UiSizeValue.POSITION_SIZE_SEPARATOR, heightStr);
};
oFF.UiSizeValue.prototype.setFromString = function(sizeStr)
{
	var sizes = oFF.XStringTokenizer.splitString(sizeStr, oFF.UiSizeValue.POSITION_SIZE_SEPARATOR);
	if (oFF.notNull(sizes) && sizes.size() === 2)
	{
		var newWidth = sizes.get(0);
		var newHeight = sizes.get(1);
		this.setWidth(oFF.UiUnitValue.createByCss(newWidth));
		this.setHeight(oFF.UiUnitValue.createByCss(newHeight));
	}
};
...
oFF.XDateTime.createCurrentLocalDateTime = function()
{
	...
	calendar.setTimeInMillis(oFF.XSystemUtils.getCurrentTimeInMilliseconds());
	var year = calendar.get(oFF.DateConstants.YEAR);
	var month = calendar.get(oFF.DateConstants.MONTH) + 1;
	var day = calendar.get(oFF.DateConstants.DAY_OF_MONTH);
	var hour = calendar.get(oFF.DateConstants.HOUR_OF_DAY);
	var minute = calendar.get(oFF.DateConstants.MINUTE);
	var second = calendar.get(oFF.DateConstants.SECOND);
...
oFF.UiSizeUnit = function() {};
oFF.UiSizeUnit.prototype = new oFF.UiAbstractConstant();
oFF.UiSizeUnit.prototype._ff_c = "UiSizeUnit";

oFF.UiSizeUnit.POINT = null;
oFF.UiSizeUnit.PIXEL = null;
oFF.UiSizeUnit.PICA = null;
oFF.UiSizeUnit.EM = null;
oFF.UiSizeUnit.REM = null;
oFF.UiSizeUnit.VW = null;
oFF.UiSizeUnit.VH = null;
oFF.UiSizeUnit.VMIN = null;
oFF.UiSizeUnit.VMAX = null;
oFF.UiSizeUnit.PERCENT = null;
oFF.UiSizeUnit.AUTO = null;
oFF.UiSizeUnit.s_lookup = null;
oFF.UiSizeUnit.staticSetup = function()
{
	oFF.UiSizeUnit.s_lookup = oFF.XHashMapByString.create();
	oFF.UiSizeUnit.POINT = oFF.UiSizeUnit.create("Point", "pt");
	oFF.UiSizeUnit.PIXEL = oFF.UiSizeUnit.create("Pixel", "px");
	oFF.UiSizeUnit.PICA = oFF.UiSizeUnit.create("Pica", "pc");
	oFF.UiSizeUnit.PERCENT = oFF.UiSizeUnit.create("Percent", "%");
	oFF.UiSizeUnit.EM = oFF.UiSizeUnit.create("Em", "em");
	oFF.UiSizeUnit.REM = oFF.UiSizeUnit.create("Rem", "rem");
	oFF.UiSizeUnit.VW = oFF.UiSizeUnit.create("Vw", "vw");
	oFF.UiSizeUnit.VH = oFF.UiSizeUnit.create("Vh", "vh");
	oFF.UiSizeUnit.VMIN = oFF.UiSizeUnit.create("Vmin", "vmin");
	oFF.UiSizeUnit.VMAX = oFF.UiSizeUnit.create("Vmax", "vmax");
	oFF.UiSizeUnit.AUTO = oFF.UiSizeUnit.create("Auto", "auto");
};
oFF.UiSizeUnit.create = function(name, documentName)
{
	return oFF.UiAbstractConstant.createUiConstantExt(new oFF.UiSizeUnit(), name, documentName, null, oFF.UiSizeUnit.s_lookup, null);
};
oFF.UiSizeUnit.lookup = function(name)
{
	return oFF.UiSizeUnit.s_lookup.getByKey(name);
};
