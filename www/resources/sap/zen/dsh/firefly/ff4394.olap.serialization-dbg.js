...
oFF.QInAExportUtil = {
	...
	getMeasureMemberNameByAlias:function(queryModel, aliasName, exporter)
	{
		...
		if ((exporter.getMode().isTypeOf(oFF.QModelFormat.INA_DATA) || exporter.mode.isTypeOf(oFF.QModelFormat.INA_VALUE_HELP)) && oFF.notNull(queryModel))
		{
			...
			if (exporter.getSession().hasFeature(oFF.FeatureToggleOlap.MULTIPLE_ACCOUNT_HIERARCHIES) && queryModel.isUniversalAccountModel() && oFF.notNull(memberName))
			{
...
oFF.QInARepoOptions = {
	...
	exportOptions:function(provider)
	{
		...
		if (provider.isAutoVariableSubmitFullQM())
		{
			inaOptions.putBoolean("IgnoreAutoSubmit", queryModel.isIgnoreAutoVariableSubmit());
		}
			...
			if (oFF.notNull(inaResultStructureFeature))
		{
			if (provider.supportsReturnedDataSelection())
			{
				...
				if (oFF.notNull(inaReturnedDataSelection) && inaReturnedDataSelection.hasElements())
				{
					...
					for (var i = 0; i < size; i++)
					{
						var dataSelection = oFF.ReturnedDataSelection.lookupOrCreate(inaElementNames.get(i));
...
oFF.QInARepoUtils = {
	...
	addKeyRefStructure:function(exporter, modelComponent, inaParentStructure)
	{
...
oFF.QInARepository.prototype.exportKeyReference = function(exporter, modelComponent, inaStructure)
{
	...
	if (oFF.isNull(queryManager) || queryManager.isKeyRefSerializationEnabled())
	{
		...
		{
			oFF.QInARepoUtils.addKeyRefStructure(exporter, modelComponent, inaStructure);
			done = true;
		}
...
{
	var dimCount = axis.getDimensionCount();
	if (!oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConversionRequired(exporter, axis))
	{
		for (var i = 0; i < dimCount; i++)
		{
			var dimension = axis.get(i);
	...
	{
		var dimensionWithLeveledHierarchy = oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchy(axis).getName();
		var udhDimensions = oFF.QInARepoUniversalDisplayHierarchies.getDimensionNames(exporter, axis);
		for (var k = 0; k < dimCount; k++)
		{
			...
			{
				oFF.QInARepoDimensionsAllLegacy.exportUdhDimensions(exporter, axis, parameterDimensionList, udhDimensions, exportedDimensions);
			}
			else if (!udhDimensions.contains(dimName))
			{
				oFF.QInARepoDimensionsAllLegacy.exportDimension(exporter, dim, parameterDimensionList, exportedDimensions);
			}
		}
	}
};
oFF.QInARepoDimensionsAllLegacy.exportUdhDimensions = function(exporter, axis, parameterDimensionList, udhDimensions, exportedDimensions)
{
	var size = udhDimensions.size();
	for (var k = 0; k < size; k++)
	{
		var dim = axis.getQueryModel().getDimensionByName(udhDimensions.get(k));
		dim.stopEventing();
...
oFF.QInARepoDimensionsAllLegacy.prototype.importComponentWithList = function(importer, inaList, modelComponent, parentComponent, context)
{
	if (oFF.notNull(inaList))
	{
		...
		for (var k = 0; k < size; k++)
		{
			...
			{
				var importFormulaCalculatedDimension = importer.importFormulaCalculatedDimension(inaDimension, queryModel);
...
oFF.QInARepoDimensionsLegacy.exportAxisDimensions = function(exporter, axis, parameterDimensionList, exportedDimensions)
{
	...
	else
	{
		...
		{
			var dim = axis.get(k);
			if (oFF.QInARepoDimensionsLegacy._isForRendering(exporter.getMode(), dim))
			{
				continue;
			}
			var dimName = dim.getName();
			if (oFF.XString.isEqual(dimName, dimensionWithLeveledHierarchy))
			{
				oFF.QInARepoDimensionsLegacy.exportUdhDimensions(exporter, axis, parameterDimensionList, udhDimensions, exportedDimensions);
			}
			else if (!udhDimensions.contains(dimName))
			{
				oFF.QInARepoDimensionsLegacy.exportDimension(exporter, dim, parameterDimensionList, exportedDimensions);
			}
		}
	}
};
...
oFF.QInARepoDimensionsLegacy.exportUdhDimensions = function(exporter, axis, parameterDimensionList, udhDimensions, exportedDimensions)
{
	var size = udhDimensions.size();
	for (var k = 0; k < size; k++)
	{
...
oFF.QInARepoQueryLegacy.prototype.getInactiveCapabilities = function(queryModel)
{
	...
	var experimentalFeatureSet = queryModel.getQueryManager().getQueryServiceConfig().getFeatureToggles();
...
oFF.QInARepoSortLegacy.getDimensionWithLeveledHierarchyWhichContainsGivenDim = function(exporter, dimension)
{
	if (oFF.QInARepoSortLegacy.isHierarchyConversionActive(exporter, dimension.getQueryManager()))
	{
		var dimensionName = dimension.getName();
		var rowsAxis = dimension.getQueryModel().getRowsAxis();
		if (oFF.QInARepoSortLegacy.getDimensionNames(exporter, rowsAxis).contains(dimensionName))
		{
			return oFF.QInARepoSortLegacy.getDimensionWithLeveledHierarchy(rowsAxis);
		}
		var colAxis = dimension.getQueryModel().getColumnsAxis();
		if (oFF.QInARepoSortLegacy.getDimensionNames(exporter, colAxis).contains(dimensionName))
		{
			return oFF.QInARepoSortLegacy.getDimensionWithLeveledHierarchy(colAxis);
		}
	}
	return null;
};
oFF.QInARepoSortLegacy.getDimensionNames = function(exporter, axis)
{
	if (oFF.QInARepoSortLegacy.isHierarchyConversionActive(exporter, axis.getQueryManager()))
	{
		var dimension = oFF.QInARepoSortLegacy.getDimensionWithLeveledHierarchy(axis);
		if (oFF.notNull(dimension))
		{
...
oFF.QInARepoSortLegacy.getDimensionWithLeveledHierarchy = function(axis)
{
...
oFF.QInARepoSortLegacy.isHierarchyConversionActive = function(exporter, queryManager)
{
...
oFF.QInARepoBlendableQueryManager.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	inaStructure.putBoolean("UseAllMemberFilter", blendableQueryManager.getUseEmptyAllMemberMeasureFilter());
	return inaStructure;
};
...
oFF.QInARepoDataCell.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	{
		var inANameDataCell = inaStructure.getStringByKey("Name");
		dataCell = queryDataCellList.newQueryDataCell(inANameDataCell);
		createdNew = true;
	}
...
oFF.QInARepoDataCell.prototype.getDataCellFromExistingList = function(queryModel, inaStructure)
{
	...
	var dataCell = null;
	var queryDataCellList = queryModel.getQueryDataCellsBase();
	if (oFF.XStringUtils.isNotNullAndNotEmpty(inANameDataCell))
	{
		dataCell = queryDataCellList.getByKey(inANameDataCell);
	}
	else
	{
		var inaMemberReferences = inaStructure.getListByKey("DimensionMemberReferences");
		if (oFF.XCollectionUtils.hasElements(inaMemberReferences))
		{
			...
			var numberOfDataCells = queryDataCellList.size();
			for (var i = 0; i < numberOfDataCells; i++)
			{
				var iqDataCell = queryDataCellList.get(i);
				if (oFF.XStringUtils.isNullOrEmpty(member2) && iqDataCell.hasMeasureReference(member1) || iqDataCell.hasMeasureReference(member1) && iqDataCell.hasMeasureReference(member2))
				{
					dataCell = iqDataCell;
					break;
				}
...
oFF.QInARepoDimension.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (oFF.notNull(dimension))
	{
		...
		{
			dimension.getMetadataBase().setAccountTypeAttributeName(accountTypeAttributeName);
		}
		var fiscalEnabled = inaStructure.getBooleanByKeyExt("FiscalEnabled", false);
		if (fiscalEnabled)
		{
			dimension.getMetadataBase().setFiscalEnabled(fiscalEnabled);
			dimension.getMetadataBase().setFiscalDisplayPeriod(inaStructure.getStringByKey("FiscalDisplayPeriod"));
			dimension.getMetadataBase().setFiscalShift(inaStructure.getIntegerByKeyExt("FiscalShift", 0));
		}
		if (inaStructure.containsKey("additionalPeriods"))
		{
			dimension.getMetadataBase().setAdditionalPeriods(inaStructure.getIntegerByKey("additionalPeriods"));
		}
		if (inaStructure.containsKey("periodPrefix"))
		{
			dimension.getMetadataBase().setPeriodPrefix(inaStructure.getStringByKey("periodPrefix"));
		}
		if (inaStructure.containsKey("useMonthLabel"))
		{
			dimension.getMetadataBase().setUseMonthLabel(inaStructure.getBooleanByKey("useMonthLabel"));
		}
		var hasDayTimeYYYYMMDDFormat = inaStructure.getBooleanByKeyExt("HasDayTimeYYYYMMDDFormat", false);
		if (hasDayTimeYYYYMMDDFormat)
		{
			dimension.getMetadataBase().setHasDayTimeYYYYMMDDFormat(hasDayTimeYYYYMMDDFormat);
		}
...
oFF.QInARepoDimension.prototype.migratePlaceholderId = function(importer, queryModel, placeholderId)
{
	...
	if (importer.getSession().hasFeature(oFF.FeatureToggleOlap.MULTIPLE_ACCOUNT_HIERARCHIES) && oFF.notNull(queryModel) && queryModel.isUniversalAccountModel() && oFF.XString.startsWith(placeholderId, "[") && oFF.XString.indexOf(placeholderId, placeholderPrefix) !== -1)
	{
...
oFF.QInARepoDimensions.exportAxisDimensions = function(exporter, axis, parameterDimensionList, exportedDimensions)
{
	...
	else
	{
		...
		{
			var dim = axis.get(k);
			if (oFF.QInARepoDimensions._isForRendering(exporter.getMode(), dim))
			{
				continue;
			}
			var dimName = dim.getName();
			if (oFF.XString.isEqual(dimName, dimensionWithLeveledHierarchy))
			{
				oFF.QInARepoDimensions.exportUdhDimensions(exporter, axis, parameterDimensionList, udhDimensions, exportedDimensions);
			}
			else if (!udhDimensions.contains(dimName))
			{
				oFF.QInARepoDimensions.exportDimension(exporter, dim, parameterDimensionList, exportedDimensions);
			}
		}
	}
};
...
oFF.QInARepoDimensions.exportUdhDimensions = function(exporter, axis, parameterDimensionList, udhDimensions, exportedDimensions)
{
	var size = udhDimensions.size();
	for (var k = 0; k < size; k++)
	{
...
oFF.QInARepoFilterElement.prototype.shouldContinueExport = function(filterElement, exporter)
{
	...
	if (modelCapabilities.supportsComplexTupleFilter() && this.supportsVariableMasking(queryModel))
	{
		if (!this.m_exportingAtLevel1)
		{
...
oFF.QInARepoFilterElement.prototype.supportsVariableMasking = function(queryModel)
{
	if (oFF.isNull(queryModel))
	{
		return false;
	}
	return queryModel.getSession().hasFeature(oFF.FeatureToggleOlap.AV_CAPABILITY_VARIABLE_MASKING);
};
...
oFF.QInARepoFormulaCalculatedDimension.exportFormulaCalcProperties = function(exporter, inaStructure, dimension)
{
	...
	if (isKeyRefSerializationEnabled && dimension.getOlapEnv() !== null && dimension.getOlapEnv().getClientQueryObjectManager().getClientQueryObjectByKeyRef(properties) !== null)
	{
		...
		oFF.QInARepoUtils.addKeyRefStructure(exporter, dimension.getProperties(), inaProperties);
		return;
	}
...
oFF.QInARepoFormulaException.prototype.exportKeyReference = function(exporter, modelComponent, inaStructure)
{
	...
	if (oFF.notNull(queryModel))
	{
		...
		{
			oFF.QInARepoUtils.addKeyRefStructure(exporter, modelComponent3, inaStructure);
			return true;
		}
	}
...
oFF.QInARepoHierarchy.isNamePlaceHolderForAutoSubmit = function(dimension, exportMode)
{
	var isHierarchyNameVariableForAutoSubmit = false;
	if (oFF.notNull(dimension))
	{
		var queryManager = dimension.getQueryManager();
		isHierarchyNameVariableForAutoSubmit = oFF.notNull(queryManager) && queryManager.isAutoVariableSubmitFullQM() && exportMode.isTypeOf(oFF.QModelFormat.INA_REPOSITORY) && dimension.useHierarchyNameVariable() && oFF.XStringUtils.isNotNullAndNotEmpty(dimension.getHierarchyNameVariableName());
	}
	return isHierarchyNameVariableForAutoSubmit;
};
oFF.QInARepoHierarchy.isDueDatePlaceHolderForAutoSubmit = function(dimension, exportMode)
{
	var isDueDateVariableForAutoSubmit = false;
	if (oFF.notNull(dimension))
	{
		var queryManager = dimension.getQueryManager();
		isDueDateVariableForAutoSubmit = oFF.notNull(queryManager) && queryManager.isAutoVariableSubmitFullQM() && exportMode.isTypeOf(oFF.QModelFormat.INA_REPOSITORY) && dimension.useHierarchyDueDateVariable() && oFF.XStringUtils.isNotNullAndNotEmpty(dimension.getHierarchyDueDateVariableName());
	}
	return isDueDateVariableForAutoSubmit;
};
oFF.QInARepoHierarchy.isVersionPlaceHolderForAutoSubmit = function(dimension, exportMode)
{
	var isVersionVariableForAutoSubmit = false;
	if (oFF.notNull(dimension))
	{
		var queryManager = dimension.getQueryManager();
		isVersionVariableForAutoSubmit = oFF.notNull(queryManager) && queryManager.isAutoVariableSubmitFullQM() && exportMode.isTypeOf(oFF.QModelFormat.INA_REPOSITORY) && dimension.useHierarchyVersionVariable() && oFF.XStringUtils.isNotNullAndNotEmpty(dimension.getHierarchyVersionVariableName());
	}
...
oFF.QInARepoHierarchy.isAutoSubmitForFullQM = function(queryManager)
{
	return oFF.notNull(queryManager) && queryManager.isAutoVariableSubmitFullQM();
};
oFF.QInARepoHierarchy.exportHierarchyForFilter = function(exporter, isAbap, inaStructure, dimension, filterExpression, queryManager, hierarchyName, dueDate, version)
{
	if (oFF.notNull(hierarchyName))
	{
		...
		{
			if (oFF.isNull(filterExpression) || !oFF.XString.isEqual(filterExpression.getName(), oFF.QFilter.FIXED_FILTER_CONTAINER) || oFF.QInARepoHierarchy.isAutoSubmitForFullQM(queryManager))
			{
...
oFF.QInARepoHierarchy.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (dimension.supportsHierarchy())
	{
		...
		else
		{
			...
			var hierarchyName = this.importHierarchyName(importer, inaStructure, dimension, isNameVariable);
			var hierarchy = null;
			if (oFF.XStringUtils.isNotNullAndNotEmpty(hierarchyName))
			{
				if (dimension.getModelCapabilities().supportsDimensionHierarchyLevels())
				...
				this.importHierarchyDueDate(inaStructure, dimension, hierarchy, importer.capabilities.supportsSapDate());
...
oFF.QInARepoHierarchy.prototype.importHierarchyName = function(importer, inaStructure, dimension, isNameVariable)
{
	var hierarchyName;
	if (inaStructure.hasStringByKey("Name"))
	{
	...
	var queryManager = dimension.getQueryManager();
	if ((importer.getOriginalMode() === oFF.QModelFormat.INA_CLONE || importer.getMode().isTypeOf(oFF.QModelFormat.INA_REPOSITORY)) && oFF.notNull(queryManager) && queryManager.isAutoVariableSubmitFullQM())
	{
		dimension.setUseHierarchyNameVariable(isNameVariable && queryManager.getVariableProcessorState() === oFF.VariableProcessorState.CHANGEABLE_STARTUP);
		if (isNameVariable)
		{
...
oFF.QInARepoHierarchy.prototype.importHierarchyDueDate = function(inaStructure, dimension, hierarchy, supportsSapDate)
{
	var dueDateIs = inaStructure.getStringByKey("DueDateIs");
	var isDueDateVariable = oFF.notNull(dueDateIs) && oFF.XString.isEqual("Variable", dueDateIs);
	var dueDate = inaStructure.getStringByKey("DueDate");
...
oFF.QInARepoHierarchy.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	if (dimension.supportsHierarchy())
	{
		...
		var isHierarchyNameVariableForAutoSubmit = oFF.QInARepoHierarchy.isNamePlaceHolderForAutoSubmit(dimension, exporter.mode);
		var isDueDateVariableForAutoSubmit = oFF.QInARepoHierarchy.isDueDatePlaceHolderForAutoSubmit(dimension, exporter.mode);
		var isVersionVariableForAutoSubmit = oFF.QInARepoHierarchy.isVersionPlaceHolderForAutoSubmit(dimension, exporter.mode);
...
oFF.QInARepoMembersAll.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (oFF.notNull(structureLayout))
	{
		...
		if (isDelta)
		{
			var structureMemberIter = dimension.getAllStructureMembers().getIterator();
		...
		for (var i = 0; i < len; i++)
		{
			...
			else if (isKeyRefSerializationEnabled && structureMember.isModelDefinedMember())
			{
				if (exporter.getOriginalMode().isTypeOf(oFF.QModelFormat.INA_REPOSITORY))
				{
					...
					oFF.QInARepoUtils.addKeyRefStructure(exporter, structureMember, inaModellerMember);
					continue;
				}
...
oFF.QInARepoQuery.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (oFF.notNull(inaDefinition))
	{
		...
		if (oFF.notNull(queryManager))
		{
			...
			queryManager.setResultSetPersistanceTargetSchema(inaStructure.getStringByKey("ResultSetPersistanceSchema"));
			queryManager.setResultSetPersistenceIdentifier(inaStructure.getStringByKey("ResultSetPersistanceIdentifier"));
			if (queryManager.isAutoVariableSubmitFullQM())
			{
				queryModel.setIgnoreAutoVariableSubmit(inaStructure.getBooleanByKey("IgnoreAutoSubmit"));
			}
		}
		var inaPreQueries = inaDefinition.getListByKey("PreQueries");
		if (oFF.notNull(inaPreQueries))
...
oFF.QInARepoQuery.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	var featureToggles = oFF.InactiveCapabilityUtil.exportFeatureToggles(serviceConfig.getFeatureToggles());
	...
	if (queryManager.isAutoVariableSubmitFullQM())
	{
		inaStructure.putBoolean("IgnoreAutoSubmit", queryModel.isIgnoreAutoVariableSubmit());
	}
	return oFF.QInARepository.prototype.exportComponentWithStructure.call( this , exporter, modelComponent, inaStructure, flags);
};
...
oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConversionRequired = function(exporter, axis)
{
	return oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConversionActive(exporter, axis.getQueryManager()) && oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchy(axis) !== null;
};
oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConverted = function(exporter, dimension)
{
	return oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConversionActive(exporter, dimension.getQueryManager()) && dimension === oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchy(dimension.getAxis());
};
oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConversionActive = function(exporter, queryManager)
{
	return exporter.mode === oFF.QModelFormat.INA_DATA && queryManager.isHierarchyToUDHConversionEnabledForCurrentRs() && oFF.XCollectionUtils.hasElements(queryManager.getQueryModel().getUniversalDisplayHierarchies().getHierarchies());
};
oFF.QInARepoUniversalDisplayHierarchies.getDimensionNames = function(exporter, axis)
{
	if (oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConversionActive(exporter, axis.getQueryManager()))
	{
		var dimension = oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchy(axis);
		if (oFF.notNull(dimension))
		{
			var leveledHierarchy = dimension.getLeveledHierarchy(dimension.getHierarchyName());
			var size = leveledHierarchy.getAllLevel().size();
...
oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchyWhichContainsGivenDim = function(exporter, dimension)
{
	if (oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConversionActive(exporter, dimension.getQueryManager()))
	{
		var dimensionName = dimension.getName();
		var rowsAxis = dimension.getQueryModel().getRowsAxis();
		if (oFF.QInARepoUniversalDisplayHierarchies.getDimensionNames(exporter, rowsAxis).contains(dimensionName))
		{
			return oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchy(rowsAxis);
		}
		var colAxis = dimension.getQueryModel().getColumnsAxis();
		if (oFF.QInARepoUniversalDisplayHierarchies.getDimensionNames(exporter, colAxis).contains(dimensionName))
		{
			return oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchy(colAxis);
		}
	}
	return null;
};
oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchy = function(axis)
{
	if (oFF.isNull(axis) || axis.getType() !== oFF.AxisType.ROWS && axis.getType() !== oFF.AxisType.COLUMNS)
	{
		return null;
...
oFF.QInARepoUniversalDisplayHierarchies.prototype.exportHierarchy = function(exporter, udhStructure, hierarchy)
{
	...
	var hierarchyConversionDimension = oFF.QInARepoUniversalDisplayHierarchies.isHierarchyConversionActive(exporter, axis.getQueryManager()) ? oFF.QInARepoUniversalDisplayHierarchies.getDimensionWithLeveledHierarchy(axis) : null;
	if (oFF.notNull(hierarchyConversionDimension))
	{
		...
		udhConvertedFromLBH.putString("LowerLevelNodeAlignment", hierarchy.getAlignment().getName());
		udhConvertedFromLBH.putBoolean("Active", true);
		udhConvertedFromLBH.putNewList("DimensionNames").addAllStrings(oFF.QInARepoUniversalDisplayHierarchies.getDimensionNames(exporter, axis));
...
oFF.QInARepoMemberBasicMeasure.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (oFF.notNull(member))
	{
		...
		if (overrideDescription)
		{
			...
			{
				if (oFF.XStringUtils.isNullOrEmpty(member.getText()))
				{
			...
			if (!oFF.XString.isEqual(newMemberText, member.getText()))
			{
				var textField = dimension.getTextField();
				if (oFF.notNull(textField))
				{
		...
		if (oFF.notNull(newMemberText) && (member.getText() === null || overrideDescription))
		{
