...
oFF.BlendingMappingUtils = {
	...
	processTransientCalculations:function(queryModel, transientMembers)
	{
			if (oFF.notNull(transientMembers))
		{
			...
			while (transientMembersIter.hasNext())
			{
				...
				if (calculationDimension.getFilter() !== null && calculationDimension.getFilter().hasElements())
				{
	...
	processCalculationDimensionMappings:function(blendingDefinition, primarySource, secondarySource, blendedDrillOperations)
	{
		...
		if (oFF.notNull(primaryAccountDimension))
		{
			...
			if (oFF.isNull(secondaryAccountDimension))
			{
				...
				if (secondaryQueryManager.getSystemType().isTypeOf(oFF.SystemType.BW))
				{
		...
		else
		{
			...
			if (oFF.notNull(secondaryAccountDimension))
			{
				...
				if (primaryQueryManager.getSystemType().isTypeOf(oFF.SystemType.BW))
				{
	...
	processLinkedDimensions:function(blendingProcessConfig, blendingDefinition, primarySource, secondarySource, blendingProcess, blendedDrillOperations)
	{
		...
		while (dimensionLinksIter.hasNext())
		{
			...
			if (primaryLinkDimension.supportsHierarchy())
			{
				...
				primaryLinkDimension.setInitialDrillLevel(primaryLinkDimensionOOC ? 0 : -1);
			}
	...
	{
			if (oFF.notNull(dimension) && dimension.isHierarchyActive() && dimension.getDrillManager().getDrillOperationsByDimension(dimension).size() === 0)
		{
	...
	processUnlinkedDimensions:function(blendingSource, blendingDefinition, qmLink, usedUnlinkDimensionNames, blendedDrillOperations)
	{
		...
		while (dimensionsIter.hasNext())
		{
			...
			if (oFF.isNull(mappingDimensionNames) || !mappingDimensionNames.contains(dimensionName))
			{
				...
				else if (qmLink.getUnlinkedDimensionJoinType() === oFF.UnlinkedDimensionJoinType.CREATE_NEW_MEMBERS)
				{
					...
					{
						mapping.addNewConstantMapping("(No Value)", queryAliasName);
					}
				}
...
oFF.BlendingUtils = {
	...
	convertToBlendedFormula:function(formulaItem, parentMeasure)
	{
		...
		else if (formulaItem.getOlapComponentType() === oFF.OlapComponentType.FORMULA_ITEM_MEMBER)
		{
			...
			var structureMember = dimensionContext.getStructureMember(formulaItemMember.getMemberName());
			...
			aliasPrefixName = oFF.BlendingUtils.getAliasPrefix(parentMeasure.getQueryModel(), structureMember);
	...
	getAliasPrefix:function(queryModel, member)
	{
			var aliasPrefix = "";
		var queryManager = queryModel.getQueryManager();
		if (queryManager.getMode() === oFF.QueryManagerMode.BLENDING)
		{
			...
			for (var i = 0; i < blendingSources.size(); i++)
			{
				...
				{
					aliasPrefix = blendingSource.getQueryAliasName();
					break;
				}
			}
		}
	...
	isQueryModelInUseForBlending:function(queryModel, useEmptyAllMemberMeasureFilter)
	{
		...
		var leadingMeasureStructure = oFF.notNull(accountDimension) ? accountDimension : measureDimension;
		var leadingMeasureStructureCartesianList = queryModel.getFilter().getDynamicFilter().getCartesianList(leadingMeasureStructure);
		var hasMeasureMemberInFilter = oFF.notNull(leadingMeasureStructureCartesianList) && leadingMeasureStructureCartesianList.size() > 0;
	...
	activateDimensionHierarchiesInDrill:function(queryModel)
	{
		...
		for (var i = 0; i < dimensionsInDrill.size(); i++)
		{
			...
			{
				dimensionInDrill.setHierarchyName(dimensionInDrill.getHierarchyName() !== null ? dimensionInDrill.getHierarchyName() : dimensionInDrill.getDefaultHierarchyName());
...
oFF.OlapEnvQueryManagerHandler.prototype.m_featureToggles = null;
...
oFF.OlapEnvQueryManagerHandler.prototype.releaseObject = function()
{
	...
	this.m_featureToggles = oFF.XObjectExt.release(this.m_featureToggles);
...
oFF.OlapEnvQueryManagerHandler.prototype.activateFeatureToggle = function(featureToggle)
{
	this.m_featureToggles.add(featureToggle);
};
oFF.OlapEnvQueryManagerHandler.prototype.deactivateFeatureToggle = function(featureToggle)
{
	this.m_featureToggles.removeElement(featureToggle);
};
oFF.OlapEnvQueryManagerHandler.prototype.clearAllFeatureToggles = function()
{
...
oFF.OlapEnvQueryManagerHandler.prototype.processQueryManagerCreationWithDataSourceName = function(syncType, listener, customIdentifier, systemName, dataSourceName)
{
	...
	queryServiceConfig.activateFeatureToggleSet(this.m_featureToggles);
...
oFF.OlapEnvQueryManagerHandler.prototype.activateFeatureToggleSet = function(featureToggles)
{
	if (oFF.notNull(featureToggles))
	{
		var iterator = featureToggles.getIterator();
		while (iterator.hasNext())
		{
			this.activateFeatureToggle(iterator.next());
		}
	}
};
oFF.OlapEnvQueryManagerHandler.prototype.getFeatureToggles = function()
{
	return this.m_featureToggles;
};
oFF.OlapEnvQueryManagerHandler.prototype.hasFeature = function(featureToggle)
{
	return this.getOlapEnv().getSession().hasFeature(featureToggle) || this.m_featureToggles.contains(featureToggle);
};
...
oFF.QFactoryImpl.prototype.newRsDefStructureMemberProperties = function(placeholderAliasMappings, minimumDrillStateMap, unsatisfiedRequiredDimensionNames)
{
	return oFF.RsDefStructureMemberProperties.create(placeholderAliasMappings, minimumDrillStateMap, unsatisfiedRequiredDimensionNames);
};
...
oFF.InACapabilitiesProvider.staticSetup = function()
{
	...
	oFF.InACapabilitiesProvider.SAC_DEV_CAPABILITIES.addCapability(oFF.InACapabilities.V137_SORT_NEW_VALUES);
	oFF.InACapabilitiesProvider.SAC_DEV_CAPABILITIES.addCapability(oFF.InACapabilities.V234_MEASURE_MEMBER_DEFINITION);
	oFF.InACapabilitiesProvider.SAC_DEV_CAPABILITIES.addCapability(oFF.InACapabilities.V238_MEASURE_MEMBER_DETAILS);
	oFF.InACapabilitiesProvider.SAC_DEV_CAPABILITIES.addCapability(oFF.InACapabilities.V212_DATASOURCE_TYPE_QUERY_METADATA);
	oFF.InACapabilitiesProvider.SAC_DEV_CAPABILITIES.addCapability(oFF.InACapabilities.V214_DATASOURCE_TYPE_QUERY);
};
...
oFF.InACapabilitiesProvider.importCapabilities = function(sysCapabilities, queryCapabilities)
{
	...
	queryCapabilities.setSupportsHierarchyCatalog(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V500_HIERARCHY_CATALOG, false));
	...
	queryCapabilities.setSupportsSortNewValues(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V137_SORT_NEW_VALUES, false));
	...
	queryCapabilities.setSupportsCubeBlendingWithNSubqueries(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V208_CUBE_BLENDING_N_QUERIES, false));
	...
	queryCapabilities.setSupportsUnifiedDataCells(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V121_UNIFIED_DATA_CELLS, false));
	...
	queryCapabilities.setSupportsCustomDimension2(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V122_CUSTOM_DIMENSION_2, false));
	...
	queryCapabilities.setSupportsSetOperandCurrentMemberSingleNavigation(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V125_SET_OPERAND_CURRENT_MEMBER_SINGLE_NAVIGATION, false));
	queryCapabilities.setSupportsDimensionKeyAttributes(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V294_DIMENSION_KEY_ATTRIBUTES, false));
	...
	queryCapabilities.setSupportsVirtualDescription(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V210_IS_VIRTUAL_DESCRIPTION, false));
	...
	queryCapabilities.setSupportsCustomMeasuresInMetadata(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V238_MEASURE_MEMBER_DETAILS, false) && oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V234_MEASURE_MEMBER_DEFINITION, false) && oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V240_MEASURE_MEMBER_TYPE, false));
	...
	queryCapabilities.setSupportsUniversalModel(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V504_UNIVERSAL_MODEL, false));
	queryCapabilities.setSupportsCurrencyTranslation(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V260_CURRENCY_TRANSLATION, false));
	queryCapabilities.setSupportsMeasureMemberCurrencyTranslations(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V262_MEASURE_MEMBER_CURRENCY_TRANSLATIONS, false));
	...
	queryCapabilities.setSupportsRootOrphansAfterVisibilityFilter(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V196_ROOT_ORPHANS_AFTER_VISIBILITY_FILTER, false));
	...
	queryCapabilities.setSupportsUndefinedTupleCountTotals(oFF.InACapabilitiesProvider.supports(sysCapabilities, oFF.InACapabilities.V280_UNDEFINED_TUPLE_COUNT_TOTALS, false));
};
...
oFF.InACapabilitiesProvider.prototype.createFeatureToggleAnalyticsCapabilities = function(featureToggleCapabilities)
{
	...
	this.checkSystemType(featureToggleCapabilities, oFF.FeatureToggleOlap.BW_UNIFIED_DATACELLS, oFF.InACapabilities.V121_UNIFIED_DATA_CELLS, systemType, oFF.SystemType.ABAP);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.CUSTOM_DIMENSION_2, oFF.InACapabilities.V122_CUSTOM_DIMENSION_2);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.SORT_NEW_VALUES, oFF.InACapabilities.V137_SORT_NEW_VALUES);
	...
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.CUSTOM_DIMENSION_2_FOR_AGILE_BI, oFF.InACapabilities.V122_CUSTOM_DIMENSION_2);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.CUSTOM_DIMENSION_2_FOR_INA_MODEL, oFF.InACapabilities.V122_CUSTOM_DIMENSION_2);
	...
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.UNIFIED_DATACELLS, oFF.InACapabilities.V121_UNIFIED_DATA_CELLS);
	...
	this.checkExt(featureToggleCapabilities, oFF.FeatureToggleOlap.MDS_CONDITIONS, oFF.InACapabilities.C101_CONDITIONS, systemType, oFF.SystemType.HANA);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.CUBE_BLENDING_N_QUERIES, oFF.InACapabilities.V208_CUBE_BLENDING_N_QUERIES);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.DATASOURCE_TYPE_QUERY_METADATA, oFF.InACapabilities.V212_DATASOURCE_TYPE_QUERY_METADATA);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.DATASOURCE_TYPE_QUERY, oFF.InACapabilities.V214_DATASOURCE_TYPE_QUERY);
	...
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.HIERARCHY_CATALOG, oFF.InACapabilities.V500_HIERARCHY_CATALOG);
	...
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.SET_OPERAND_CURRENT_MEMBER_SINGLE_NAVIGATION, oFF.InACapabilities.V125_SET_OPERAND_CURRENT_MEMBER_SINGLE_NAVIGATION);
	...
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.DIMENSION_KEY_ATTRIBUTES, oFF.InACapabilities.V294_DIMENSION_KEY_ATTRIBUTES);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.IS_VIRTUAL_DESCRIPTION, oFF.InACapabilities.V210_IS_VIRTUAL_DESCRIPTION);
	...
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.MEASURE_MEMBER_DEFINITION, oFF.InACapabilities.V234_MEASURE_MEMBER_DEFINITION);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.MEASURE_MEMBER_DETAILS, oFF.InACapabilities.V238_MEASURE_MEMBER_DETAILS);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.MEASURE_MEMBER_TYPE, oFF.InACapabilities.V240_MEASURE_MEMBER_TYPE);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.UNIVERSAL_MODEL, oFF.InACapabilities.V504_UNIVERSAL_MODEL);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.ROOT_ORPHANS_AFTER_VISIBILITY_FILTER, oFF.InACapabilities.V196_ROOT_ORPHANS_AFTER_VISIBILITY_FILTER);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.CURRENCY_TRANSLATION, oFF.InACapabilities.V260_CURRENCY_TRANSLATION);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.MEASURE_MEMBER_CURRENCY_TRANSLATIONS, oFF.InACapabilities.V262_MEASURE_MEMBER_CURRENCY_TRANSLATIONS);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.AUTO_VARIABLE_SUBMIT, oFF.InACapabilities.V286_AUTO_VARIABLE_SUBMIT);
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.AUTO_VARIABLE_SUBMIT_FOR_FULL_QM, oFF.InACapabilities.V286_AUTO_VARIABLE_SUBMIT);
	...
	this.check(featureToggleCapabilities, oFF.FeatureToggleOlap.UNDEFINED_TUPLE_COUNT_TOTALS, oFF.InACapabilities.V280_UNDEFINED_TUPLE_COUNT_TOTALS);
...
oFF.InACapabilitiesProvider.prototype.createFeatureTogglePlanningCapabilities = function(featureToggleCapabilities)
{
	if (this.getSession().hasFeature(oFF.FeatureToggleOlap.DEVELOPMENT_MODE_PLANNING))
	{
		...
		featureToggleCapabilities.addCapability(oFF.InACapabilities.V500_HIERARCHY_CATALOG);
	}
	return featureToggleCapabilities;
};
...
oFF.InACapabilitiesProvider.prototype.checkSystemType = function(supportedCapabilities, featureToggle, capabilityName, actualSystemType, conditionalSystemType)
{
	if (actualSystemType.isTypeOf(conditionalSystemType) && this.getSession().hasFeature(featureToggle))
	{
		supportedCapabilities.addCapability(capabilityName);
	}
};
...
oFF.InACapabilitiesProvider.prototype.getActiveCapabilities = function()
{
	if (oFF.isNull(this.m_activeCapabilities))
	{
		...
		if (this.m_serverMetadata.getSystemDescription().getSystemType().isTypeOf(oFF.SystemType.HANA) && oFF.notNull(this.m_activeCapabilities) && this.m_activeCapabilities.containsKey(oFF.InACapabilities.V122_CUSTOM_DIMENSION_2) && (oFF.ServerVersionComparator.compare(this.m_serverMetadata.getVersion(), "1.0.122.14.00000000") === 1 || !this.doesServerSatisfyMinCustomDimension2CapabilitySet()))
		{
			this.m_activeCapabilities.remove(oFF.InACapabilities.V122_CUSTOM_DIMENSION_2);
		}
	}
...
{
	if (oFF.notNull(storageName) && oFF.notNull(dimensionName))
	{
		var overrideTextMap = this.m_dimensionOverrideTexts.getByKey(storageName);
...
{
	if (oFF.notNull(storageName) && oFF.notNull(dimensionName) && oFF.XStringUtils.isNotNullAndNotEmpty(overrideText))
	{
		var overrideTextMap = this.m_dimensionOverrideTexts.getByKey(storageName);
		if (oFF.isNull(overrideTextMap))
		{
			overrideTextMap = oFF.XHashMapOfStringByString.create();
			this.m_dimensionOverrideTexts.put(storageName, overrideTextMap);
		}
...
{
	if (oFF.notNull(storageName) && oFF.notNull(dimensionName) && oFF.notNull(memberName))
	{
		var dimensionNameMap = this.m_memberOverrideTexts.getByKey(storageName);
...
{
	if (oFF.notNull(storageName) && oFF.notNull(dimensionName) && oFF.notNull(memberName) && oFF.XStringUtils.isNotNullAndNotEmpty(overrideText))
	{
		var dimensionNameMap = this.m_memberOverrideTexts.getByKey(storageName);
		if (oFF.isNull(dimensionNameMap))
		{
			dimensionNameMap = oFF.XHashMapByString.create();
			this.m_memberOverrideTexts.put(storageName, dimensionNameMap);
		}
		var overrideTextMap = dimensionNameMap.getByKey(dimensionName);
		if (oFF.isNull(overrideTextMap))
		{
...
oFF.PreQueryExecutor.prototype.processExecutionRuntimePrequery = function(syncType, runtimeQuery, preQueryExecutedListener)
{
	...
	remoteFunction.getRequest().setRequestStructure(generateRemoteQuery.getData());
...
oFF.OlapEnvCapabilityAction.prototype._sacFallback = function(serverMetadata, serverService, newCapabilities)
{
	...
	if (betaMetadataForService.containsKey(oFF.InACapabilities.V137_SORT_NEW_VALUES))
	{
		newCapabilities.setSupportsSortNewValues(true);
	}
	if (betaMetadataForService.containsKey(oFF.InACapabilities.V234_MEASURE_MEMBER_DEFINITION) && betaMetadataForService.containsKey(oFF.InACapabilities.V238_MEASURE_MEMBER_DETAILS))
	{
		newCapabilities.setSupportsCustomMeasuresInMetadata(true);
	}
};
...
oFF.OlapEnvSupportedFormulaAction.prototype.processSyncAction = function(syncType, listener, customIdentifier)
{
	...
	if (systemType.isTypeOf(oFF.SystemType.ABAP) && connection.supportsAnalyticCapability(oFF.InACapabilities.C167_FORMULA_OPERATORS_CATALOG) || systemType.isTypeOf(oFF.SystemType.HANA))
	{
		...
		_function.getRequest().setRequestStructure(this.createRequest(connection));
...
oFF.OlapEnvironment.prototype.activateFeatureToggle = function(featureToggle)
{
	this.m_queryManagerHandler.activateFeatureToggle(featureToggle);
};
oFF.OlapEnvironment.prototype.clearAllFeatureToggles = function()
{
	this.m_queryManagerHandler.clearAllFeatureToggles();
};
oFF.OlapEnvironment.prototype.deactivateFeatureToggle = function(featureToggle)
{
	this.m_queryManagerHandler.deactivateFeatureToggle(featureToggle);
};
...
oFF.OlapEnvironment.prototype.activateFeatureToggleSet = function(featureToggles)
{
	this.m_queryManagerHandler.activateFeatureToggleSet(featureToggles);
};
oFF.OlapEnvironment.prototype.getFeatureToggles = function()
{
	return this.m_queryManagerHandler.getFeatureToggles();
};
oFF.OlapEnvironment.prototype.hasFeature = function(featureToggle)
{
	return this.m_queryManagerHandler.hasFeature(featureToggle);
};
...
oFF.BlendingProcess.create = function(blendableQueryManager, configs, runtimeBlendedQueryManager)
{
	...
	blendingProcess.populateBlendedAndTransientMeasures(blendableQueryManager);
...
oFF.BlendingProcess.prototype.populateBlendedAndTransientMeasures = function(blendableQueryManager)
{
		...
		if (blendedMeasure.getMemberType() === oFF.MemberType.FORMULA)
		{
			...
			while (formulaItemMembersIter.hasNext())
			{
				...
				if (oFF.notNull(formulaDimensionContext))
				{
					...
					if (oFF.notNull(blendDimensionContext))
					{
						...
						if (structureMember.isMeasureInUse(blendQueryModelContext))
						{
			...
			if (validFormula)
			{
				...
				while (transientMemberDatasetIds.hasNext())
				{
					...
					var transientMembersList = transientMembers.getByKey(transientMemberDatasetId);
					...
					existingTransientList.addAll(transientMembersList);
				}
			}
		}
	}
	if (this.m_transientMembers.size() > 0)
	{
		this.m_cachedRuntimeBlendedQueryManagerResult = null;
	}
};
oFF.BlendingProcess.prototype.processSynchronization = function(syncType)
{
	...
	else if (oFF.BlendingUtils.isQueryModelInUseForBlending(primaryQueryModel, this.m_blendableQueryManager.getUseEmptyAllMemberMeasureFilter()))
	{
...
oFF.BlendingProcess.prototype.processBlendingQueryManagerCreation = function()
{
	...
	oFF.BlendingMappingUtils.processUnlinkedDimensions(primarySource, blendingDefinition, qmLink, unlinkedDimensionsNames, this.m_blendedDrillOperations);
	oFF.BlendingMappingUtils.processUnlinkedDimensions(secondarySource, blendingDefinition, qmLink, unlinkedDimensionsNames, this.m_blendedDrillOperations);
	oFF.BlendingMappingUtils.processTransientCalculations(primaryQueryModel, this.m_transientMembers.getByKey(primaryQueryModel.getDatasetId()));
	oFF.BlendingMappingUtils.processTransientCalculations(secondaryQueryModel, this.m_transientMembers.getByKey(secondaryQueryModel.getDatasetId()));
...
oFF.BlendingProcess.prototype.cleanupTransientState = function(queryManager)
{
	...
	else
	{
		...
		if (oFF.notNull(transientMembers))
		{
			...
			this.m_transientMembers.remove(queryModel.getDatasetId());
		}
...
oFF.BlendingProcess.prototype.processBlendedDrills = function(queryManager)
{
	var blendedDrillInfoIter = this.m_blendedDrillOperations.getIterator();
	while (blendedDrillInfoIter.hasNext())
	{
		var blendedDrillInfo = blendedDrillInfoIter.next();
		var dimension = queryManager.getQueryModel().getDimensionByName(blendedDrillInfo.getDimensionName());
		if (oFF.notNull(dimension))
		{
			if (!dimension.isHierarchyActive())
			{
			...
			while (blendedDrillPathElementInfosIter.hasNext())
			{
				...
				var contextDimension = queryManager.getQueryModel().getDimensionByName(blendedDrillPathElementInfo.getDimensionName());
				if (oFF.notNull(contextDimension))
				{
					...
					drillPathElements.add(oFF.QFactory.createDrillPathElement(queryManager.getQueryModel(), blendedDrillPathElementInfo.getDrillMember(), contextDimension));
				}
			}
...
oFF.BlendingProcess.prototype.prepareQueryExecution = function()
{
	...
	{
		this.applyBlendedMeasures(this.m_primaryBlendedMeasures);
		...
		this.processBlendedDrills(this.m_blendedQueryManager);
	}
};
oFF.BlendingProcess.prototype.postQueryExecution = function()
{
	...
	{
		this.m_childProcess.postQueryExecution();
	}
	else if (oFF.notNull(this.m_blendedQueryManager))
	{
		this.cleanupTransientState(this.m_blendedQueryManager);
	}
};
oFF.BlendingProcess.prototype.applyBlendedMeasures = function(blendedMeasures)
{
	if (oFF.notNull(this.m_blendedQueryManager) && oFF.notNull(blendedMeasures))
	{
		...
		while (transientMemberListIter.hasNext())
		{
			...
			while (transientMemberIter.hasNext())
			{
				...
				{
					aliasPrefix = oFF.BlendingUtils.getAliasPrefix(queryModel, transientMember);
				}
...
oFF.OlapEnvVarProcessor.prototype.supportsVariableMasking = function()
{
	...
	for (var i = 0; i < queryManagerSize; i++)
	{
		var queryManager = queryManagerList.get(i);
		var variableProcessor = queryManager.getVariableProcessor();
		if (variableProcessor.supportsVariableMasking())
		{
			return true;
		}
	}
...
oFF.OlapEnvVarProcessor.prototype.sync = function()
{
...
{
	this.createBlendedQueryManager(syncType, this, customIdentifier);
	return this;
};
...
oFF.BlendableResultSetContainer.prototype.getRuntimeQueryManager = function(syncType, listener, customIdentifier)
{
	return this.createBlendedQueryManager(syncType, listener, customIdentifier);
};
...
{
	if (oFF.notNull(queryManager) && queryManager.getActiveResultSetContainer().getSyncState().isNotInSync() && queryManager.isAutoVariableSubmitFullQM() && this.getSession().hasFeature(oFF.FeatureToggleOlap.EXTERNALIZED_DYNAMIC_FILTER))
	{
		var queryModel = queryManager.getQueryModel();
		...
		if (isValidState && variableManager.hasDimensionMemberVariableAffectingDynamicFilter())
		{
			...
			var variableDimensionNames = variableManager.getDimensionNamesOfVariablesAffectingDynamicFilter();
			var dimensionsUsedInFilter = filter.getDynamicFilter().getDimensionsUsedInFilter();
			if (taggingStoryVariables)
			{
				filterDimensions.addAll(variableDimensionNames);
			}
			...
			{
				queryManager.getConvenienceCommands().clearFiltersByDimensionName(dimensionNames.get(i));
			}
			var variableDimensionIterator = variableDimensionNames.getIterator();
			while (variableDimensionIterator.hasNext())
			{
				queryManager.getConvenienceCommands().clearFiltersByDimensionName(variableDimensionIterator.next());
			}
			...
			if (oFF.notNull(dynamicProduct))
			{
				...
				var prunedDimension = this.m_prunedDimensions.getByKey(queryManager.getInstanceId());
				for (var j = 0; j < variablesAffectingDynamicFilter.size(); j++)
				{
					...
					if (!variable.isHierarchyInfoVariable())
					{
						...
						if (!dimensionNames.contains(dimension.getName()) && (!isInitialSubmit || oFF.isNull(prunedDimension) || !prunedDimension.contains(dimension.getName())) || oFF.XString.isEqual(queryManager.getTagging().getByKey(oFF.OlapFilterManager.TAGGING_VARIABLE_DIALOG_HAS_BEEN_SET), "true"))
						{
							...
							lowValueBag.setFilterValueType(oFF.XValueType.VARIABLE);
							if (variable.getVariableType().isTypeOf(oFF.VariableType.HIERARCHY_NODE_VARIABLE))
							{
								dynamicList.setHierarchyName(variable.getDimension().getHierarchyName());
							}
						}
					}
				}
...
oFF.OlapFilterManager.prototype._externalizeDynamicFilter = function(queryManager, isInitialSubmit)
{
	...
	if (!dynamicFilter.isComplexFilter() && isValidState)
	{
		...
		if (dynamicFilter.isCartesianProduct() && dynamicFilter.getCartesianProduct().size() === 1)
		{
...
oFF.OlapFilterManager.prototype.pruneDimensionNamesForDynamicFilter = function(queryManager, dimensionNames)
{
	...
	{
		if (oFF.XCollectionUtils.hasElements(dimensionNames) && !oFF.XString.isEqual(queryManager.getTagging().getByKey(oFF.OlapFilterManager.TAGGING_VARIABLE_DIALOG_HAS_BEEN_SET), "true"))
		{
...
oFF.OlapFilterManager.prototype.pruneDimensionNamesInternal = function(queryManager, dimensionNames)
{
	...
	else if (queryManager.isAutoVariableSubmitFullQM())
	{
...
oFF.QueryManager.prototype.setupClone = function(origin, cloneMode, dataSource)
{
	...
	this.m_metadataModel = origin.getMetadataModel().cloneOlapComponent(null, null);
...
oFF.QueryManager.prototype.cloneQueryManagerExt = function(cloneMode)
{
	...
	if (this.getMode() === oFF.QueryManagerMode.BLENDING)
	{
		...
		blendingServiceConfig.activateFeatureToggleSet(serviceConfig.getFeatureToggles());
...
oFF.QueryManager.prototype.maintainStateForRsWithAutoSubmit = function(resultSetContainer)
{
	...
	if (this.isAutoVariableSubmitFullQM() && variableProcessorState !== oFF.VariableProcessorState.PROCESSING_AUTO_SUBMIT && variableProcessorState !== oFF.VariableProcessorState.PROCESSING_AUTO_SUBMIT_AFTER_REINIT)
	{
		...
		if (variableProcessorState !== oFF.VariableProcessorState.SUBMITTED && (this.hasVariables() || this.supportsAndAllowsEmptyExtendedVarDefinition()))
		{
			...
			{
				this.m_queryModel.setIgnoreAutoVariableSubmit(false);
			...
			if (!this.m_queryModel.isIgnoreAutoVariableSubmit() && variableProcessorState !== oFF.VariableProcessorState.VALUE_HELP)
			{
...
oFF.QueryManager.prototype.variablesRequireImplicitSubmit = function()
{
	...
	if (this.isAutoVariableSubmitFullQM())
	{
		return false;
	}
	return (this.supportsAndAllowsEmptyExtendedVarDefinition() || this.hasVariables()) && !this.isSubmitted() && !this.isVariableSubmitFailed();
};
oFF.QueryManager.prototype.isAutoVariableSubmitFullQM = function()
{
	return oFF.notNull(this.m_queryModel) && this.getSession().hasFeature(oFF.FeatureToggleOlap.AUTO_VARIABLE_SUBMIT_FOR_FULL_QM) && this.getModelCapabilities().supportsAutoVariableSubmit();
};
...
oFF.AbstractBlendableQueryManager.prototype.getResultsetContainer = function(enforceOutOfSync)
{
	if (oFF.isNull(this.m_activeContainer) || this.m_activeContainer.isReleased() || enforceOutOfSync && this.m_activeContainer.getSyncState() !== oFF.SyncState.OUT_OF_SYNC)
	{
		...
		{
			var activeContainer = this.m_activeContainer;
			if (activeContainer.getSyncState() === oFF.SyncState.IN_SYNC)
			{
				var runtimeQueryManagerAction = activeContainer.getRuntimeQueryManager(oFF.SyncType.BLOCKING, null, null);
				var runtimeQueryManager = runtimeQueryManagerAction.getData();
...
oFF.AbstractBlendableQueryManager.prototype.isAutoVariableSubmitFullQM = function()
{
	return this.getQueryManagerBase().isAutoVariableSubmitFullQM();
};
...
oFF.BlendableQueryManager.prototype.getBlendingProcessConfigs = function()
{
	...
	while (queryModelLinkSettingsIterator.hasNext())
	{
		...
		if (modelLinks.size() > 0)
		{
			...
			if (oFF.isNull(secondaryQueryManager) || !oFF.BlendingUtils.isQueryModelInUseForBlending(secondaryQueryManager.getQueryModel(), this.m_useEmptyAllMemberMeasureFilter))
			{
				continue;
			}
...
oFF.BlendableQueryManager.prototype.getRuntimeQueryManager = function(syncType, listener, customIdentifier)
{
	...
	return resultsetContainer.getRuntimeQueryManager(syncType, listener, customIdentifier);
};
oFF.BlendableQueryManager.prototype.serializeToContentExt = function(modelFormat, capabilities)
{
	...
	if (modelFormat.isTypeOf(oFF.QModelFormat.INA_DATA))
	{
		...
		if (!processRuntimeQueryManagerAction.hasErrors())
		{
			...
			content = runtimeQueryManager.serializeToContentExt(modelFormat, capabilities);
		}
		else
		{
			content = null;
...
oFF.BlendableQueryManager.prototype.getUseEmptyAllMemberMeasureFilter = function()
{
