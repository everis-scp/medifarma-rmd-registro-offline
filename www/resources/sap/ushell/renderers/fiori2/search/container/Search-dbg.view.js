...
sap.ui.define([
    ...
    SearchConfiguration, SearchLayout, SearchResultListContainer, SearchResultList, SearchResultTable, SearchResultGrid, SearchSpreadsheet, SearchNoResultScreen,
    ...
    return sap.ui.jsview("sap.ushell.renderers.fiori2.search.container.Search", {
        ...
        assembleSearchToolbar: function (bWithoutShareButton) {
            ...
            displaySwitchTapStrips.addEventDelegate({
                ...
                onAfterRendering: function (oEvent) {
                    ...
                    var itemLength = that.getModel().config.gridView ? 3 : 2;
                    if (oDisplaySwitchButtons.getItems().length === itemLength && that.determineIfMaps(that)) {
                        oDisplaySwitchButtons.addItem(new sap.m.SegmentedButtonItem({
                            icon: "sap-icon://map",
                            tooltip: sap.ushell.resources.i18n.getText("displayMap"),
                            key: "map"
                        }));
                    } else if (oDisplaySwitchButtons.getItems().length === (itemLength + 1) && !that.determineIfMaps(that)) {
        ...
        assembleDisplaySwitchTapStrips: function () {
            ...
            if (config.gridView) {
                items.push(new sap.m.SegmentedButtonItem({
                    icon: "sap-icon://grid",
                    tooltip: sap.ushell.resources.i18n.getText("displayGrid"),
                    key: "grid"
                }));
            }
            ...
            var oSegmentedButton = new sap.m.SegmentedButton('ResultViewType', {
                ...
                select: function (eObj) {
                    ...
                    case "table":
                        ...
                        break;
                    case "grid":
                        model.setProperty('/resultToDisplay', "searchResultGrid");
                        that.showMoreFooter.setVisible(that.isShowMoreFooterVisible());
                        that.searchResultMap.setVisible(false);
                        break;
                    case "map":
        ...
        assembleCenterArea: function () {
            ...
            return [that.tableSortDialog, searchResultList, that.searchResultTable, that.searchResultGrid, that.searchResultMap,
                that.appSearchResult, that.showMoreFooter, that.totalCountHiddenElement
            ];
        },
        ...
        assembleSearchResultGrid: function () {
            var l = new sap.f.GridContainerSettings({
                rowSize: "11rem",
                columnSize: "11rem",
                gap: "0.5rem"
            });
            var resultGrid = new SearchResultGrid("ushell-search-result-grid", {
                layout: l,
                snapToRow: true,
                visible: {
                    parts: [{
