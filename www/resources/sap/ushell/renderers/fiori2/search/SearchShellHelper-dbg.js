...
sap.ui.define([
    ...
    jQuery.extend(module, {
        ...
        registerFocusHandler: function () {
            ...
            that.oSearchSelect.addEventDelegate({
                onAfterRendering: function () {
                    ...
                    domRef = domRef.querySelector('#searchFieldInShell-select-hiddenSelect'); // starting from 1.86
        ...
        onAfterNavigate: function (oEvent) {
            ...
            if (oSearchView) {
                var oSearchResultList = oSearchView.resultList;
                if (oSearchResultList && oSearchResultList.getItems()) {
                    var aMyListItems = oSearchResultList.getItems();
                    for (var i = 0; i < aMyListItems.length; i++) {
                        var oMyItem = aMyListItems[i];
                        if (oMyItem.getContent() && oMyItem.getContent().length > 0) {
                            oMyItem.getContent()[0]._showOrHideExpandButton();
                        }
                    }
                }
            }
