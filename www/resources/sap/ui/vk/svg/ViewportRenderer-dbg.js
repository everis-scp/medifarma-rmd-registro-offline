...
sap.ui.define([
	...
	ViewportRenderer.render = function(rm, viewport) {
		...
		if (viewport._styles.size > 0) {
			rm.write("<defs><style>");
			viewport._styles.forEach(function(styleInfo) {
				rm.write("." + styleInfo.id + "{\n");
				var attibutes = styleInfo.attibutes;
				for (var i = 0; i < attibutes.length; i += 2) {
					rm.write(attibutes[ i ] + ":" + attibutes[ i + 1 ] + ";\n");
				}
				rm.write("}\n");
			});
			rm.write("</style></defs>");
