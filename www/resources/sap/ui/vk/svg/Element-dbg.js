...
sap.ui.define([
	...
	Element.prototype._updateColor = function(mask) {
		...
		if (domRef !== null) {
			if (this._styleId) {
				this.removeClass(this._styleId);
				this._styleId = null;
			}

			this._setStyleAttributes(domRef.setAttribute.bind(domRef));
	...
	Element.prototype._setBaseAttributes = function(setAttributeFunc, mask) {
		...
		if (!this._styleId || this.highlightColor || this.tintColor) {
			this._setStyleAttributes(setAttributeFunc);
		}
	...
	Element.prototype._setStyleAttributes = function(setAttributeFunc) {
	...
	Element.prototype._getStyleKey = function() {
		var key = "";
		if (this.fillURL !== undefined) {
			key += this.fillURL;
		} else if (this.fill !== undefined) {
			key += this._cssColor(this.fill);
		}
		key += ";";
		if (this.stroke !== undefined && this.stroke[ 3 ] > 0 && this.strokeWidth) {
			key += this._cssColor(this.stroke) + "," + this.strokeWidth;
			if (this.strokeDashArray.length > 0) {
				key += "," + this.strokeDashArray.join("-");
			}
		}
	...
	Element.prototype.render = function(rm, mask, viewport) {
		...
		if (!skipIt) {
			...
			if (viewport && this._isGeometryNode()) {
				var styleId = viewport._getStyleId(this);
