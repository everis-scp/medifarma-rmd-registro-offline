	...
	*/
	...
	SceneBuilder.prototype.createImageNote = function(annotation, sceneId) {
		...
		if (annotation.type === "image") {
			...
			// Will create a temportary material if it doesn't exist yet, can't return null
			var material = this._getMaterial(materialId);
	...
	SceneBuilder.prototype.createMaterial = function(materialInfo) {
		...
		if (materialInfo.textureEmissive && materialInfo.textureEmissive.length
			...
			if (this._detailsByImageId.has(textureImageId)) {
				var imageDetail = this._images.get(textureImageId);
				material.textureHeight = imageDetail.height;
				material.textureWidth = imageDetail.width;
			} else {
				// There are textures dimension that need to fetched
				if (!material.userData) {
					material.userData = {};
				}

				// Prevent TotaraLoader from resolving its Promise to CDS
				material.userData.imageDetailsToLoad = new Set();
				material.userData.imageDetailsToLoad.add(textureImageId);
			}
	...
	SceneBuilder.prototype.setImageDetails = function(imageDetails) {
		this._detailsByImageId.set(imageDetails.id, imageDetails);

		var materialIds = this._materialsByTextureImageId.get(imageDetails.id);
		if (materialIds) {
			for (var i = 0; i < materialIds.length; i++) {
				var material = this._materialMap.get(materialIds[i].materialId);
				if (material.userData && material.userData.imageDetailsToLoad && material.userData.imageDetailsToLoad.size) {
					material.userData.imageDetailsToLoad.delete(imageDetails.id);
				}
				material.textureHeight = imageDetails.height;
				material.textureWidth = imageDetails.width;

				var nodeIdsToUpdate = this._materialNodesMap.get(materialIds[i].materialId);
				if (nodeIdsToUpdate) {
					for (var j = 0; j < nodeIdsToUpdate.length; j++) {
						nodeIdsToUpdate[j].setMaterial(material, true);
					}
