...
sap.ui.define([
	...
	Controller.prototype.getUISettings = function() {
		...
		var oPromises = this._callControllers(oSettingForOpen, "getAdaptationUI");

		var oUISettings = {
			showReset: oSettingForOpen.showReset,
	...
	Controller.prototype.reset = function(o, aKeys) {
		return new Promise(function(resolve){
			...
			for (var sKey in this._oSettingCurrent) {
				...
				if (oSubController.oPanel && oSubController.oPanel.setP13nModel) {
					oSubController.oPanel.setP13nModel(oSubController.getAdaptationModel());
				}
			}
			resolve();
		}.bind(this));
		...
		return Validator.checkGroupAndColumns(sTableType, this._oSettingCurrent, oColumnKey2ColumnMap, oControlDataReduceTotal, []).then(function(aResultTotal) {
			...
			if (aResultTotal.length > 0) {
				aResultTotal.forEach(function(oValidation){
					for (var sKey in this._oSettingCurrent) {
						if (oValidation.panelTypes.indexOf(sKey) > -1){
							this._oSettingCurrent[sKey].controller.oPanel.setMessageStrip(new sap.m.MessageStrip({
				...
				for (var sKey in this._oSettingCurrent) {
					if (this._oSettingCurrent[sKey].controller.oPanel && this._oSettingCurrent[sKey].controller.oPanel.isA("sap.ui.mdc.p13n.panels.BasePanel")) {
						this._oSettingCurrent[sKey].controller.oPanel.setMessageStrip(undefined);
					}
				}
			}
		}.bind(this));
	...
	Controller.prototype._callControllers = function(oSettings, sMethodName) {
		...
		for (var sType in oSettings) {
			...
			if (sMethodName === "getPanel" || sMethodName === "getAdaptationUI" ) {
