...
sap.ui.define([
	...
	SmartTable.prototype._getColumnLabel = function(oColumn) {
		...
		if (!oColumn) {
			return this._oTableProvider.getFieldLabel(this.getEntitySet(), sFieldName);
		}
	 ...
	 * @param {int} [iMin=0] The minimum text length value to measure the given text
	 * @param {boolean} [bHeader=false] Whether column header font style should be taken into account
	 * @returns {float} The text width
	 * @private
	 */
	function measureText(sText, iMin, bHeader) {
		iMin = iMin || 0;
		sText = sText || "";
		...
		if (!SmartTable.oCanvasContext) {
			var fnUpdateFonts = function() {
				SmartTable.oCanvasContextFonts = {
					"default" : "normal "
									+ (parseFloat(ThemeParameters.get("sapMFontMediumSize") || 0.875) * fBaseFontSize) + "px "
									+ (ThemeParameters.get("sapUiFontFamily") || "Arial,Helvetica,sans-serif"),
					"header"  : (ThemeParameters.get("sapUiColumnHeaderFontWeight") || "bold") + " "
									+ (parseFloat(ThemeParameters.get("sapUiColumnHeaderFontSize") || 0.875) * fBaseFontSize) + "px "
									+ (ThemeParameters.get("sapUiFontFamily") || "Arial,Helvetica,sans-serif")
				};
			...
			Core.attachThemeChanged(fnUpdateFonts);
			fnUpdateFonts();
		}

		SmartTable.oCanvasContext.font = SmartTable.oCanvasContextFonts[bHeader ? "header" : "default"];

		return SmartTable.oCanvasContext.measureText(sText).width / fBaseFontSize;
	}
	...
	SmartTable.prototype._calcColumnWidth = function(oField, bAdditionalProperty, mConfig) {
		...
		var vSample,
			...
			sType = oField.type || "",
			mNumericLimits = {
				"Edm.Int16" : 5,    // longest possible value 32768
				"Edm.Int32" : 9,	// longest possible value 2147483648
				"Edm.Int64" : 12,	// we do not expect 64-bit integer to be used therefore limited to 999,999,999,999
				"Edm.Float" : 6,	// represents a floating point number with 7 digits precision
				"Edm.Single" : 6,	// represents a floating point number with 7 digits precision
				"Edm.Double" : 13,	// represents a floating point number with 14 digits precision
				"Edm.Decimal" : 15  // we do not expect 2^255 to be used therefore limited to 15 digits precision
			};
		...
		if (bAdditionalProperty || oField.unit || !sAdditionalProperty || sDisplayBehaviour != "descriptionOnly") {
			if (oField.isImageURL || sType == "Edm.Byte" || sType == "Edm.SByte") {
				fWidth += iMin;
			} else if (oField.isCalendarDate || ODataType.isDateOrTime(oField.type)) {
				...
				} else if (oModelType.oFormat && oModelType.oFormat.oFormatOptions) {
					// in case of week day or month names are used as a format option then we just add a few more pixels not to truncate dates
					(oModelType.oFormat.oFormatOptions.pattern || "").replace(/[MELc]{3,4}/, function(sWideFormat) {
						fWidth += (sWideFormat.length == 4 ? 0.9 : 0.3);
					});
				}
			} else if (sType in mNumericLimits) {
				var iPrecision = Math.min(oField.precision || 20, mNumericLimits[sType]);
				vSample = 2 * Math.pow(10, iPrecision - (oField.scale || 0) - 1); // create a numeric sample from the precision and scale
			}  else if (sType == "Edm.Boolean") {
				// the text of boolean values are static therefore it is enough to calculate the longest one only once
				if (!SmartTable.hasOwnProperty("fBooleanWidth")) {
					var fTrueWidth = measureText(oModelType.formatValue(true, "string"), iMin);
					var fFalseWidth = measureText(oModelType.formatValue(false, "string"), iMin);
				...
				if (iMaxLength == 0) {
					fWidth += Math.max(10, iMax * 0.75); // if the max length is unknown we do not have enough data to estimate the content width
				} else if (iMaxLength <= iMin) {
					fWidth += iMin; // assumption: 1 character < 1rem
				} else if (iMaxLength * 0.25 > iMax) {
					fWidth = iMax; // assumption: 1 character > 0.25rem
				} else if (iMaxLength < iMax * 0.7 || iMax < 5) {
					vSample = sChar.repeat(iMaxLength); // up to 14 characters we take the longest possible text as a reference to calculate the width
				} else {
					var fSampleWidth = measureText(sChar.repeat(iMaxLength), iMin);
					fWidth += Math.log(fSampleWidth - iMax * 0.2) / Math.log(iMax / 3) * (iMax / 2); // for larger maxLength we use logarithmic growth formula to chop the long texts
				}
			} else {
			...
			if (vSample) {
				...
				var sSample = oModelType.formatValue(vSample, "string");
				fWidth += measureText(sSample, iMin);
				fWidth += mConfig.epsilon;
			}
		}
		...
		if (mConfig.label) {
			...
			if (fWidth < iMax && fWidth < iLabelLength && iLabelLength > iMin) {
				...
				// logartihmic ratio between content width and max label width
				// content width 2, max label width 5      or     content width 5 max label width 7.4,
				// content width 9, max label width 10     or     content width 13 max label width 13.1,
				// snippix#59084
				var fContentLabelRatio = Math.max(1, 1 - (Math.log(fWidth - iMin * 0.85) / Math.log(iMax * 0.5)) + 1);
				var fMaxLabelWidth = fContentLabelRatio * fWidth;

				// if the column label is too long we should respect that and increase the max label widthx
				var fOrigLabelWidth = Math.min(measureText(sLabel, 0, true) + mConfig.epsilon, iMax * 0.7);
				var fLabelWidthDiff = Math.max(0, fOrigLabelWidth - fMaxLabelWidth);

				// if the label does not fit to the max width then we increase the max label width per Euler number difference
				fLabelWidth = (fLabelWidthDiff < 0.15) ? fOrigLabelWidth : fMaxLabelWidth + fLabelWidthDiff * (1 - 1 / fWidth) / Math.E;

				// store the calculated label width
				oField.labelWidth = fLabelWidth;
			}
		}
	...
	SmartTable.prototype._updateColumnWidthForEdit = function(oColumn, oField) {
		...
		if (!this.getEditable()) {
			...
			if (!this._aEditableColumnAdditionalWidth) {
				...
				var fIconWidth = parseFloat(ThemeParameters.get("_sap_m_InputBase" + sCompact + "_IconWidth"));
				fEditableColumnAdditionalWidth += parseFloat(ThemeParameters.get("_sap_m_InputBaseWrapper_Sum_Border"));
				fEditableColumnAdditionalWidth += parseFloat(ThemeParameters.get("_sap_m_InputBase" + sCompact + "_InnerPadding").split(" ").pop()) * 2;
