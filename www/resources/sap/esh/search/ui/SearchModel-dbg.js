 ...
 * @returns
 */
function (Log, ErrorHandler, MessageBox, SearchHelper, SearchResultListFormatter, SearchFacetsFormatter, SearchTabStripsFormatter, SuggestionHandler, SearchConfiguration, PersonalizationStorage, EventLogger, SearchUrlParser, cFLPUtil, UserCategoryManager, System) {
    ("use strict");
    ...
    var SearchModel = sap.ui.model.json.JSONModel.extend("sap.esh.search.ui.SearchModel", {
        ...
        constructor: function (properties) {
            ...
            that.userCategoryManagerDeferred = null;
        },
        // ################################################################################
        // Initialization:
        // ################################################################################
         ...
         */
        createSina: function () {
            var sinaPromise = jQuery.Deferred();
            ...
            sap.ui.require(["sap/esh/search/ui/sinaNexTS/sina/sinaFactory"], function (sinaFactory) {
                // sap.ui.require(['sap/ushell/renderers/fiori2/search/sinaNext/sina/sinaFactory'], function (sinaFactory) {
                ...
                // cFlp
                cFLPUtil.readCFlpConfiguration(trials).then(function (trials) {
                    ...
                    sinaFactory.createByTrialAsync(trials).then(function (sina) {
                        sinaPromise.resolve(sina);
                    }, function (error) {
                        sinaPromise.reject(error);
                    });
        ...
        initBusinessObjSearch: function () {
            ...
            var dummyDataSourceForLoadingPhase = {
                label: sap.esh.search.ui.resources.i18n.getText("genericLoading"),
                labelPlural: sap.esh.search.ui.resources.i18n.getText("genericLoading"),
                enabled: false,
                id: "$$Loading$$",
            };
            ...
            // create sina async
            that._initBusinessObjSearchProm = PersonalizationStorage.create(this.config.personalizationStorage)
                ...
                if (that.userCategoryManager.isFavActive()) {
                    that.setProperty("/defaultDataSource", that.userCategoryManager.getCategory("MyFavorites"));
                }
                else {
                ...
                if (that.config.defaultDataSource) {
                    that.setProperty("/defaultDataSource", that.sinaNext.getDataSource(that.config.defaultDataSource));
                }
        ...
        _calculateCountText: function () {
            ...
            if (count > 0) {
                ...
                if (that.getProperty("/searchInLabel")) {
                    return ((that.getProperty("/searchInLabel") ||
                        sap.esh.search.ui.resources.i18n.getText("results")) +
                        " (" +
                        countAsStr +
                        ")");
                }
                return sap.esh.search.ui.resources.i18n.getText("results") + " (" + countAsStr + ")";
            }
            return "";
        },
        /**
        ...
        getDocumentTitle: function () {
            ...
            if (this.getDataSource() === this.allDataSource) {
                title = sap.esh.search.ui.resources.i18n.getText("searchTileTitleProposalAll", [
                    searchTerm,
                ]);
            }
            else {
                title = sap.esh.search.ui.resources.i18n.getText("searchTileTitleProposal", [
                    searchTerm,
                    dataSourceLabel,
                ]);
            }
        ...
        createAllAndAppDataSource: function () {
            ...
            this.allDataSource.label = sap.esh.search.ui.resources.i18n.getText("label_all");
            this.allDataSource.labelPlural = sap.esh.search.ui.resources.i18n.getText("label_all");
            // app datasource
            this.appDataSource = this.sinaNext._createDataSource({
                id: "$$APPS$$",
                label: sap.esh.search.ui.resources.i18n.getText("label_apps"),
                labelPlural: sap.esh.search.ui.resources.i18n.getText("label_apps"),
        ...
        getUserCategoryManager: function () {
            //caching
            if (this.userCategoryManagerDeferred) {
                return this.userCategoryManagerDeferred;
            }
            //create
            this.userCategoryManagerDeferred = this.initBusinessObjSearch().then(function () {
        ...
        normalSearch: function (preserveFormerResults) {
            ...
            var jQueryDefferd = jQuery.Deferred();
            /**
            ...
            var successHandler = function (searchResultSet) {
                ...
                that._afterSearchPrepareResultList(that.perspective).then(function () {
            ...
            that.query.getResultSetAsync().then(function (resultSet) {
                var searchResultSet = 
        ...
        _prepareTableResults: function (results) {
            ...
            for (i = 0; i < results.length; i++) {
                ...
                if (results[i].navigationObjects !== undefined &&
                    ...
                    rows[i].cells.push({
                        value: sap.esh.search.ui.resources.i18n.getText("intents"),
            ...
            if (results[0].hasTitleDescription) {
                var titleDescriptionLabel = results[0].titleDescriptionLabel +
                    " (" +
                    sap.esh.search.ui.resources.i18n.getText("titleDescription") +
                    ")";
                columns.unshift({
            ...
            if (that.config.titleColumnName && that.config.titleColumnName !== "") {
                titleColumn.name = sap.esh.search.ui.resources.i18n.getText(that.config.titleColumnName);
            }
            else {
            ...
            if (rows[0].cells[lastCellIndex].isRelatedApps) {
                columns.push({
                    name: sap.esh.search.ui.resources.i18n.getText("intents"),
            ...
            sortColumns.unshift({
                name: sap.esh.search.ui.resources.i18n.getText("defaultRank"),
        ...
        _afterSearchPrepareResultList: function (searchResultSet) {
            ...
            //         that.pushUIMessage({
            //             type: "warning",
            //             title: sap.esh.search.ui.resources.i18n.getText("preserveFormerResultErrorTitle"),
            //             description: sap.esh.search.ui.resources.i18n.getText("preserveFormerResultErrorDetails", listOfFaultyDatasourcesString)
            //         });
            //     }
            // }
            var temporaryAuxDeferred = $.Deferred();
            Promise.all([Promise.resolve(searchResultSet), loadCustomModulesProm]).then(function (params) {
                //TODO: error handling
        ...
        calculatePlaceholder: function () {
            var that = this;
            if (that.isAllCategory() || that.config.bPlaceHolderFixedValue === true) {
                return sap.esh.search.ui.resources.i18n.getText("search");
            }
            return sap.esh.search.ui.resources.i18n.getText("searchInPlaceholder", [
                that.getDataSource().labelPlural,
            ]); //TODO plural?
        },
        /**
         * @this sap.esh.search.ui.SearchModel
        ...
        calculateVisibility: function () {
            ...
            if (that.isAppCategory()) {
                // search in app
                that.setResultToDisplay("appSearchResult"); // ToDo: create ENUM/constants: "searchResultList", "searchResultTable", "searchResultGrid", "searchResultMap", "appSearchResult"
                that.setProperty("/displaySwitchVisibility", false);
            }
            else if (that.isAllCategory() || that.isOtherCategory()) {
                // search in all or category
                that.setResultToDisplay("searchResultList"); // ToDo: create ENUM/constants: "searchResultList", "searchResultTable", "searchResultGrid", "searchResultMap", "appSearchResult"
                that.setProperty("/displaySwitchVisibility", false);
            }
            else {
        ...
        pushUIMessage: function (error) {
            error.title =
                error.title === "[object Object]"
                    ? sap.esh.search.ui.resources.i18n.getText("searchError")
                    : error.title;
