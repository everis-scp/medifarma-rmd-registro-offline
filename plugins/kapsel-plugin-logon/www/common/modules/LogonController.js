        ...
        if (inputContext != null && inputContext.auth != null && inputContext.auth.length > 0) {
            for (var i = 0; i < inputContext.auth.length; i++) {
                if (inputContext.auth[i]["type"] == "oauth2") {
                    ...
                    if (inputContext.auth[i]["config"]["oauth2.redirectURL"] == null) {
                        // check if "oauth2.clients" array has item
                        if (inputContext.auth[i]["config"]["oauth2.clients"] && inputContext.auth[i]["config"]["oauth2.clients"].length > 0 && inputContext.auth[i]["config"]["oauth2.clients"][0].redirectURL != null) {
                            inputContext.auth[i]["config"]["oauth2.redirectURL"] = inputContext.auth[i]["config"]["oauth2.clients"][0].redirectURL;
                        }
                    }
                    return inputContext.auth[i];
                }
    ...
    var pingServer = function (context, successCallback, errorCallback, retry, handleBasicChallenge) {
        handleBasicChallenge = handleBasicChallenge || false;
        ...
        if (cordova.require("cordova/platform").id.indexOf("windows") === -1) {
            ...
            // so remove clientcert from authConfig.
            authConfig = {};
        }

        if (handleBasicChallenge) {
            // Handle basic auth challenges in pingServer
    ...
    var RegistrationFlow = function RegistrationFlow(logonCore, logonView, onCoreResult, onFlowSuccess, onFlowError, onFlowCancel) {
        ...
        var onUnlockSubmit = function(context) {
            ...
            if (context.multiUser && context.multiUserInfo.currentUser != context.currentSelectedUser.deviceUserId){
                ...
                // logout current user and logon another user
                var logonAnotherUser = function() {
                    if (context.multiUserInfo.currentUser) {
                        // Note: call deactivateCurrentUser to logout current user.
                        // The reason is that Logon plugin will cache the secure store context if we do not logout.
                        // As a result, second user will still get the context belonging to the first user, including
        ...
        */
        ...
        var onForgotAppPasscode = function(context) {
            ...
            if (context.multiUser){
                if (sap.FioriClient) {
                    var bClearAgreementForAnybody = false;
                    if (context.multiUserInfo.userList.length <= 1) {
                        bClearAgreementForAnybody = true;
                    }
                    sap.FioriClient.removeAgreementForUser(null, context.currentSelectedUser.deviceUserId, bClearAgreementForAnybody)
                            .then(function() {
                        if(bClearAgreementForAnybody) {
                            return sap.FioriClient.removeAgreement(null);
                        } else {
                            return null;
                        }
                    }).then(function() {
     ...
     *  @private
     */
    var refreshSession = function (successCallback, errorCallback, handleBasicChallenge) {
        handleBasicChallenge = handleBasicChallenge || false; // Default value is false
        var retry = false;
        if (_providedContext) {
            pingServer(_providedContext, successCallback, errorCallback, retry, handleBasicChallenge);
        } else {
            sap.logon.Core.getContext(function(context) {
                pingServer(context, successCallback, errorCallback, retry, handleBasicChallenge);
            }, function(error) {
                if (errorCallback) {
    ...
    module.exports = {
        ...
        **/
         ...
         *  // Used by windows only. When reset application, all the cookies for saml endpoint url will be deleted.
         *  "samlEndpointUrl": "https://saml.endpoint.url",
         *
         *  "custom": {
        ...
        */
         ...
         * @param handleBasicChallenge - A boolean value indicates if refreshSession should
         * handle basic challenge. The default value is false.
         *
         *
         * @example
         * var successCallback = function(){
        ...
        *     alert("Error: " + JSON.stringify(errorInfo));
        * }
        * var handleBasicChallenge = true;
        * sap.Logon.refreshSession(successCallback,errorCallback,handleBasicChallenge);
        */
