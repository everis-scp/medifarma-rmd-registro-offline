...
sap.hybrid = {
	...
	openStoreNewInitial :function(){
		...
		//Cargamos las demas tiendas vacias----------------------------------
			...
			var SAP_ImpresionOrd_properties = {
				...
				"definingRequests": {
					"OrdenSet": "/OrdenSet?$filter=Aufnr eq '300000444'"//Por defecto
				}
			};
	...
	openStoreRegister:function(RMDData){
		...
		var uri = sap.hybrid.kapsel.appContext.applicationEndpointURL + "_RMD_SRV_TEST/v2/browse/RMD?$filter=activo eq true and "+filterRmd;  //JSON format is less verbose than atom/xml
		var oHeaders = {};
		var request = {
				headers: oHeaders,
				requestUri: uri,
		...
		OData.read(request, function (odata) {
			...
			var HANAproperties = {
				...
				"definingRequests": {
					...
					"RMD": "/RMD?$expand=mdId/estadoIdRmd,estadoIdRmd,aReceta/recetaId&$filter=activo eq true and estadoIdRmd_iMaestraId ne 478 and ("+filterOrdenHanaRMDID+")",
					"MAESTRA": "/MAESTRA?$expand=oMaestraTipo&$filter=activo eq true",
					"MD_RECETA": "/MD_RECETA?$filter=activo eq true and ("+filterOrdenHanaMDID+")",
					"RMD_RECETA": "/RMD_RECETA?$filter=activo eq true and ("+filterOrdenHana+")",
					"RMD_OBSERVACION": "/RMD_OBSERVACION?$filter=activo eq true and ("+filterOrdenHana+")",
					...
					"UsuarioRol":"UsuarioRol?$expand=oRol&$filter=activo eq true",
					"RolAppAcciones":"/RolAppAcciones?$expand=oMaestraAccion&$filter=activo eq true",
					"MD_ES_EQUIPO":"/MD_ES_EQUIPO?$expand=mdId,estructuraId,equipoId($expand=tipoId)&$filter=activo eq true and ("+filterOrdenHanaMDID+")",//471
					"MD_ES_UTENSILIO":"/MD_ES_UTENSILIO?$expand=mdId,estructuraId,utensilioId($expand=estadoId,tipoId)&$filter=activo eq true and ("+filterOrdenHanaMDID+")",
					"MD_ES_ESPECIFICACION":"/MD_ES_ESPECIFICACION?$filter=activo eq true and ("+filterOrdenHanaMDID+")",
					"MD_ES_PASO":"/MD_ES_PASO?$filter=activo eq true and ("+filterOrdenHanaMDID+")",//1000
					"RMD_ESTRUCTURA":"/RMD_ESTRUCTURA?$expand=rmdId($expand=estadoIdRmd),estructuraId($expand=tipoEstructuraId)&$filter=activo eq true and "+filterOrdenHana,//muchos
					...
					"MD_ES_PASO_INSUMO_PASO":"/MD_ES_PASO_INSUMO_PASO?$filter=activo eq true and fechaRegistro and ("+filterOrdenHanaMDID+")",//1000
					...
					"RMD_ES_UTENSILIO":"/RMD_ES_UTENSILIO?$expand=rmdId,rmdEstructuraId,utensilioId($expand=estadoId),utensilioId($expand=estadoId),agrupadorId&$filter=activo eq true and ("+filterOrdenHana+")",//muchos
					...
					"RMD_ES_PASO_INSUMO_PASO":"/RMD_ES_PASO_INSUMO_PASO?$expand=rmdId,rmdEstructuraId,etiquetaId,pasoId($expand=tipoDatoId,pasoId($expand=estadoId)),pasoHijoId($expand=tipoDatoId,estadoId),rmdEstructuraRecetaInsumoId&$filter=activo eq true and ("+filterOrdenHana+")",//muchos
					"RMD_ES_PASO":"/RMD_ES_PASO?$expand=rmdId,rmdEstructuraId,tipoDatoId,pasoId($expand=tipoDatoId,estadoId,tipoLapsoId,tipoCondicionId,etiquetaId)&$filter=activo eq true and ("+filterOrdenHana+")",//muchos
					"RMD_ES_ETIQUETA":"/RMD_ES_ETIQUETA?$expand=rmdId,rmdEstructuraId,etiquetaId($expand=estructuraId($expand=tipoEstructuraId))&$filter=activo eq true and ("+filterOrdenHana+")",//MUCHOS
			...
			//Eliminando la tienda fuera de linea
			var pathHana;
			for(var count=0; count < sap.OData.stores.length; count++){
				if(sap.OData.stores[count].name === "store_HANA"){
					pathHana = count;
				}
			}
			sap.OData.stores.splice(pathHana,1);
