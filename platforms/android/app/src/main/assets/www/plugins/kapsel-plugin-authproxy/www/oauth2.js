...
function authorize(successCallback, errorCallback) {
    ...
    if (device.platform.toLowerCase() == "ios" && currentConfig.useExternalBrowser) {
        ...
        var loadListener = function (event) {
            ...
            // In multi-response redirect URL cases, to compare and match the specified 'oauth2.redirectURL' with response redirect URL for right grant code. The 'oauth2.redirectURL' was required in appConfig.js in this case.
            // But, we still use the original logic to get the code from first response redirect URL if there is no specified 'oauth2.redirectURL' in appConfig.js
            var canPass = true;
            if (currentConfig.redirectURL) {
                var splitedURLs = event.url.split("?");
                var hostURL = splitedURLs[0];
                if (hostURL.endsWith("/")) {
                    hostURL = hostURL.substring(0, hostURL.length -1);
                }
                
                var splitedRedURLs = currentConfig.redirectURL.split("?");
                var redURL = splitedRedURLs[0];
                if (redURL && redURL.endsWith("/")) {
                    redURL = redURL.substring(0, redURL.length -1);
                }
                
                if (redURL.toLowerCase() !== hostURL.toLowerCase()) {
                    canPass = false;
                }
//                else if(splitedRedURLs.length > 1) {
//                    // To match the parameters of redirect URL
//                    var vars = splitedRedURLs[1].split("&");
//                    for (var i = 0; i < vars.length; i++) {
//                        var pair = vars[i].split("=");
//                        var value = decodeURIComponent(getUrlParameter(pair[0], event.url));
//                        if (pair[0] != value) {
//                            canPass = false;
//                            break;
//                        }
//                    }
//                } // For now, don't compare the query in redirect URL until NEO and CF server support it well.
            }
