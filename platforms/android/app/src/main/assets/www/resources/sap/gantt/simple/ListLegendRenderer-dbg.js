...
function (Utility, RenderUtils, LegendShapeGroupOrientation, IconPool) {
	...
	ListLegendRenderer.renderShape = function (oRm, oItem, bHasInteractiveItem) {
		...
		if (oShape.isA("sap.gantt.simple.BaseConditionalShape")) {
			...
			this.normalizeShape(baseConditionalShape, iWidth, iHeight);
		} else {
	...
	ListLegendRenderer.normalizeShapeGroup = function (allShapes, subShape, orientation, iWidth, legendItemHeight, index) {
		...
		if (subShape.isA("sap.gantt.simple.BaseConditionalShape")) {
			...
			if (baseConditionalShape.isA('sap.gantt.simple.LegendShapeGroup')) {
				this.normalizeLegendShapeGroup(baseConditionalShape, iWidth, legendItemHeight);
			} else {
				this.normalizeShape(baseConditionalShape, iWidth, legendItemHeight);
			}
			return;
        }
		if (subShape.isA('sap.gantt.simple.LegendShapeGroup')){
			this.normalizeLegendShapeGroup(subShape, iWidth, legendItemHeight);
			return;
		}
	...
	ListLegendRenderer.normalizeLegendShapeGroup = function (aLegendShapeGroup, iWidth, legendItemHeight) {
		var aShapes = aLegendShapeGroup.getAggregation('shapes');
		var sShapeGroupOrientation = aLegendShapeGroup.getOrientation();
		aShapes.forEach(function(oSubShape, index) {
			this.normalizeShapeGroup(aShapes, oSubShape, sShapeGroupOrientation, iWidth, legendItemHeight, index);
		}.bind(this));
	};
