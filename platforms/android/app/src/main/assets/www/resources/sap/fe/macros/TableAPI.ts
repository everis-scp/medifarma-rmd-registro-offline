...
import { openMassEditDialog } from "sap/fe/macros/massedit/MassEditHandler";
import { getLocalizedText } from "sap/fe/macros/DelegateUtil";
...
class TableAPI extends MacroAPI {
	 ...
	 */
	static isDataStateVisible(oEvent: UI5Event, oTable: any): boolean {
		...
		if (oMDCTable.getBindingContext()) {
			if (!aFilteredMessages) {
				...
				const bMessageExistsOnCurrentTable = sap.ui
					...
					.some((oMessage: any) => {
						return (
							oTableContexts.some((oRowContext: any) => {
