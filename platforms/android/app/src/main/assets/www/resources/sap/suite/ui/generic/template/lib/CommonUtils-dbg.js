	...
	function getMethods(oController, oServices, oComponentUtils) {
		...
		function fnHandleToolbarButtonsEnablement (oToolbarControlData, oModel, aContexts, oControl) {
			...
			if (oToolbarControlData.RecordType === "CRUDActionDelete") {
				...
				oComponentUtils.getTemplatePrivateModel().setProperty("/listReport/multiEditEnabled", aUpdatableContexts.length > 0);
			}
		...
		*/
		 ...
		 * @public
		 */
		function getInnerTableBindingInfo(oTable) {
		...
		/*
		 * Returns the binding of the given list
		 *
		 * @param {sap.ui.comp.smartlist.SmartList} oList The list which binding is to returned
		 * @returns {object} The found binding or null
		 */
		function getSmartListBindingInfo(oList) {
			// items array contain toolbar and smart list controls.
			return oList.getAggregation("items")[1].getBindingInfo("items");
		}

		function getTableOrListBindingInfo(oControl) {
			if (controlHelper.isSmartList(oControl)) {
				return getSmartListBindingInfo(oControl);
			} else if (controlHelper.isSmartTable(oControl)) {
				oControl = oControl.getTable(); // get SmartTable's inner table first
				return getInnerTableBindingInfo(oControl); // responsive, tree, analytical and grid tables
			} else {	// assume inner table
				return getInnerTableBindingInfo(oControl);
			}
		}
		/*
		...
		function fnRefreshSmartTable(oSmartTable, sBatchGroupId, bNoMessageRefresh) {
			var oBindingInfo = getTableOrListBindingInfo(oSmartTable);
		...
		/*
		 * Refresh given SmartList
		 *
		 * @param {ssap.ui.comp.smartlist.SmartList} oSmartList - The list to refresh.
		 */
		function fnRefreshSmartList (oSmartList) {
			var oBindingInfo = getSmartListBindingInfo(oSmartList);
			if (oBindingInfo && oBindingInfo.binding) {
				oBindingInfo.binding.refresh();
			}
		}
		...
		*/
		function fnRefreshModel(oSmartTable, fnRefreshSiblingTables) {
			...
			if (bMustRefresh) {
				oTableBindingInfo = getTableOrListBindingInfo(oSmartTable);
		...
		function fnNavigateExternal(oOutbound, oState) {
			...
			}, Function.prototype, oState, (oComponentUtils.isDraftEnabled() && "NavigateExternal") || "LeavePage");
		}
		...
		function fnProcessDataLossOrDraftDiscardConfirmation(fnProcessFunction, fnCancelFunction, oState, sMode, bNoBusyCheck){
			if (oComponentUtils.isDraftEnabled()){
		...
		function fnExecuteIfControlReady(fnHandler, sControlId, oPredefinedValues){
			...
			var oRet = (sControlId && !oControl) ?  Promise.reject() : new Promise(function(fnResolve, fnReject){
				oServices.oApplication.performAfterSideEffectExecution(function(){
					...
					var oRet = oControl ? fnHandler(oControl, oPredefinedValues) : fnHandler();
					if (oRet instanceof Promise){
						oRet.then(fnResolve, fnReject);
		...
		*/
		...
		return {
			...
			getTableOrListBindingInfo: getTableOrListBindingInfo,
			refreshSmartTable: fnRefreshSmartTable,
			refreshModel: fnRefreshModel,
			refreshSmartList:fnRefreshSmartList,
			getElementCustomData: getElementCustomData,
			getSmartListBindingInfo: getSmartListBindingInfo,
