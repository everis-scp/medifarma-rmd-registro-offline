...
oFF.NativeXCacheProviderIdbOpenAction.prototype.processSynchronization = function(syncType)
{
	...
	var openReq = indexedDB.open( "ff_cache23", 23 );
	...
	openReq.onupgradeneeded = function( event ) 
	{
		...
		if( dbChange !== null )
		{
			...
			{
				var os = dbChange.createObjectStore( "main", {keyPath: "id", autoIncrement:true} );
...
oFF.NativeXCacheProviderIdb.prototype.setupSessionContext = function( session )
{
	...
	this.m_indexedDB = win.indexedDB || win.mozIndexedDB || win.webkitIndexedDB || win.msIndexedDB;
			...
			// todo: implement native open action 
};
...
oFF.NativeXCacheProviderIdb.prototype.putString = function( name, stringValue )
{
...
oFF.RpcFunctionWasabi.prototype.processSynchronization = function()
{
  ...
  var fnDebug = function(){
    return sap.zen.commons.Log.info.apply(this, arguments);
  };
  ...
  if (path === "/gsaInfo") {
    ...
    prm.resolve(
      ...
      function(){
        return sap.zen.commons.getResponse(
          requestJsonString
        );
      }
    ).then(
