(function() {
	...
	function LoaderProxy() {
		...
		this._sendGetImageDetails = function(command) {
			var request;
			request = request || ("images/" + command.imageId + "/details");
			this._connectionHTTP.send(request, command, function(result) {
				var jsonContent = JSON.parse(utf8Decoder.decode(new Uint8Array(result)));
				jsonContent.sceneId = command.sceneId;
				jsonContent.id = command.id;
			...
			if (this._connectionHTTP) {
				switch (command.method) {
					...
					case "getImageDetails":
						this._sendGetImageDetails(command);
						return;
