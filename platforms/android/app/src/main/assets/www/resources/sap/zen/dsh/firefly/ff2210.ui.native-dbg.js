...
sap.firefly.loadUi5LibIfNeeded = function(ui5Lib) {
  if (sap == undefined || sap.ui == undefined) {
    sap.firefly.logError("Could not find SAPUi5 library! Is the the SAPUi5 library loaded?");
    return;
  }
  // load a sapui5 library on demand
  if (ui5Lib && ui5Lib.length > 0 && sap.ui.getCore().getLoadedLibraries()[ui5Lib] == undefined) {
    sap.ui.getCore().loadLibrary(ui5Lib);
  }
};
...
sap.firefly.loadExternalLibrary = function(libUrl) {
  // synchronus
  sap.firefly.logDebug("[EXTERNAL LIB] Dynamically loading external lib: " + libUrl, sap.firefly.getBlackColor(), sap.firefly.getYellowColor())

  var request = new XMLHttpRequest();
  request.open('GET', libUrl, false); // `false` makes the request synchronous
  request.send(null);

  if (request.status === 200) {
    jQuery.globalEval(request.responseText);
    sap.firefly.logDebug("[EXTERNAL LIB] Success! External lib was loaded and evaluated!", sap.firefly.getBlackColor(), sap.firefly.getYellowColor());
  } else {
    sap.firefly.logDebug("[EXTERNAL LIB] Something went wrong! Could not load external lib...", sap.firefly.getRedColor());
  }

  // below code does not work synchronus, async is deparecated
  // async will not work anyway right now since our ui does not wait for the resource
  // it expects the external lib to already be available
  /*
      $.ajax({
        async: false,
        url: libUrl,
...
sap.firefly.loadExternalCssStyles = function(stylesUrl) {
  $("<link rel='stylesheet' media='screen' type='text/css' href='" + stylesUrl + "' >").appendTo("head");
};
sap.firefly.loadSacTableIfNeeded = function(location) {
  if (!globalThis.sactable) {
    var sactablepath = "../../../extlibs/js/sactable/dist/";

    // check if the path is set on the window object
    if (location && location.length > 0) {
      sactablepath = location;
      // make sure it ends with a slash
      var lastChar = sactablepath.substr(-1);
      if (lastChar != '/') {
        sactablepath = sactablepath + '/';
      }
    }

    sap.firefly.loadExternalLibrary(sactablepath + "sactable.js");
  }

  if (!globalThis.sactable) {
    sap.firefly.logError("Failed to load sactable library.");
  }
};
sap.firefly.loadHighchartsIfNeeded = function() {
  if (!globalThis.Highcharts) {
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/stock/highstock.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/stock/highcharts-more.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/stock/modules/solid-gauge.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/modules/histogram-bellcurve.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/modules/no-data-to-display.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/modules/wordcloud.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/modules/variable-pie.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/modules/heatmap.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/modules/treemap.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/modules/variwide.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/modules/pattern-fill.js");
    sap.firefly.loadExternalLibrary("https://code.highcharts.com/highcharts-3d.js");
    sap.firefly.loadExternalLibrary("https://blacklabel.github.io/grouped_categories/grouped-categories.js");

    if (!globalThis.Highcharts) {
      sap.firefly.logError("Failed to load highcharts library.");
    }
  }
};
...
sap.firefly.setupHijack = function() {
  if (sap && sap.firefly && sap.firefly.ShellTakeOver && sap.firefly.ShellTakeOver.prototype.doHijack) {
    sap.firefly.ShellTakeOver.prototype.doHijack = sap.firefly.doHijack;
  }
};
sap.firefly.doHijack = function(containerId, program) {
  if (!$ || !sap.firefly.XtWindowManager || !sap.firefly.KernelBoot) {
    // missing dependencies so do nothing
    return;
  }

  containerId = containerId || "content";
  program = program || "FireflyStudio";

  // prepare the window
  var tmpWin = sap.firefly.XtWindowManager.manager().createNewWindow();
  tmpWin.setTitle("Original content!");
  tmpWin.open();

  var windowContent = $("#" + tmpWin.getWindowId() + " > .window-content");
  var content = $("#" + containerId);
  var hijackContainer;

  // get the elements
  if (content && content.length > 0 && windowContent && windowContent.length > 0) {
    setTimeout(function() {
      windowContent.html("<div id='hijack-container' style='position:relative;width:100%;height:100%'></div>");
      hijackContainer = windowContent.find("#hijack-container");
      content.children(":not(:last)").appendTo(hijackContainer);
    }, 0);

    // on window close return all to previous state
    tmpWin.attachOnClosePress(function() {
      content.empty();
      hijackContainer.children().appendTo(content);
      hijackContainer.remove();
      tmpWin.close();
    });
  }
};
...
sap.ui.core.Control.extend("sap.firefly.XtUi5Highcharts", {
  m_isResizing: false,
  m_resizingStartX: 0,
  m_initialAreaSize: 0,
  m_jsonDataManifest: null,
  m_contentHeight: null,
  m_contentWidth: null,
  ...
  init: function() {
    ...
    if (!window.Highcharts) {
      sap.firefly.logError("Missing Highcharts library. Did you load the library?");
      return;
    }
  },
  ...
  renderer: function(oRm, oControl) {
    var hasError = false;
    var errorText = "";
    var dataManifest = oControl.m_jsonDataManifest;

    if (dataManifest !== null) {
      hasError = dataManifest.getBooleanByKeyExt("HasError", false);
      if (hasError === true) {
        errorText = dataManifest.getStringByKey("ErrorText");
      }
    }
  ...
  onAfterRendering: function() {
    if (this.options) {
      ...
      var dataManifest = this.m_jsonDataManifest;
      if (dataManifest !== null) {
        hasError = dataManifest.getBooleanByKeyExt("HasError", false);
      }

      if (hasError === false) {
        // update highchart json size
        this.addSizeToHighchartJson();

        if (this.options.istimeSeries === true) {
          var chart1 = new Highcharts.stockChart(this.getId(), this.options);
        } else {
          var chartOptions = new Highcharts.setOptions({
            lang: {
              "thousandsSep": ","
            }
          });
          var chart1 = new Highcharts.Chart(this.getId(), this.options);
        }
  ...
  setupControl: function() {
    // this.setDataManifest( null );
    this.setJsonModel(null);

    // this.jsonDataManifest = null;
    // this.jsonModel = null;
    // this.options = null;
  },

  // required for realtive sizing calculation (%)
  calculateContentSize: function(nativeHookId) {
    var container = $("#" + nativeHookId);
    if (container !== null) {
      this.m_contentHeight = container.height();
      this.m_contentWidth = container.width();
    } else {
      this.m_contentHeight = null;
      this.m_contentWidth = null;
    }
  },

  setModelJson: function(model) {
    if (model !== null) {
      // var jsonString = sap.firefly.PrUtils.serialize( model, false,
      // false, 0 );
      // var nativeModel = JSON.parse( jsonString );
      var nativeModel = model.convertToNative();

      // test function, used to develop and test functions
      /*
       * nativeModel["chart"]["events"] = { load: function () {
       *
      ...
      // add on load event, converts from string passed in chart json
      if ((((nativeModel || {}).chart || {}).events || {}).load) {
        var tmpLoadFunc = nativeModel["chart"]["events"]["load"];
        eval("var tmpFunc = " + tmpLoadFunc);
        nativeModel["chart"]["events"] = {
          load: tmpFunc
        };
      }

      this.setJsonModel(nativeModel);
      this.options = nativeModel;

      // add size to highchart json
      this.addSizeToHighchartJson();

    }
  },

  addSizeToHighchartJson: function() {
    //width
    var width = this.getWidth();
    if (width !== undefined && width !== null) {
      var widthWithNoDigits = width.replace(new RegExp("[0-9]", "g"), "");
      if (widthWithNoDigits === "px") {
        this.options.chart.width = parseInt(width);
      } else if (widthWithNoDigits === "%") {
        if (this.m_contentWidth !== null) {
          var relativeWidth = parseInt(width);
          relativeWidth = (relativeWidth / 100) * this.m_contentWidth;
          this.options.chart.width = relativeWidth;
        }
      }
    }
    ...
    if (height !== undefined && height !== null) {
      var heightWithNoDigits = height.replace(new RegExp("[0-9]", "g"), "");
      if (heightWithNoDigits === "px") {
        this.options.chart.height = parseInt(height);
      } else if (heightWithNoDigits === "%") {
        if (this.m_contentHeight !== null) {
          var relativeHeight = parseInt(height);
          relativeHeight = (relativeHeight / 100) * this.m_contentHeight;
          this.options.chart.height = relativeHeight;
        }
      }
    }
  ...
  setDataManifest: function(dataManifest) {
    this.m_jsonDataManifest = dataManifest;
    this.invalidate();
  },
...
sap.ui.core.Control.extend("sap.firefly.XtUi5AppIcon", {
  ...
  renderer: function(oRm, oControl) {
    ...
    if (iconTooltip !== undefined && iconTooltip !== null) {
      oRm.writeAttribute("title", iconTooltip);
    }
...
sap.ui.core.Control.extend("sap.firefly.XtUi5TaskBar", {
  ...
  renderer: function(oRm, oControl) {
    ...
    if (tooltip !== undefined && tooltip !== null) {
      oRm.writeAttribute("title", tooltip);
    }
...
sap.ui.core.Control.extend("sap.firefly.XtUi5SacTableGrid", {
  ...
  m_lastCellRightClickParams: null,
  ...
  init: function() {
    // make sure that the library is loaded
    if (!window.sactable) {
      sap.firefly.logError("Missing sactable library. Did you load the library?");
      return;
    }
    ...
    this.m_widgetContainer.className = "reactTableComponent";
    ...
    sap.ui.core.ResizeHandler.register(this.m_widgetContainer, function(params) {
      ...
      myself.m_containerSize = params.size;
      myself.rerender();
    });

    // register for the on context menu handler
    this.m_widgetContainer.addEventListener('contextmenu', function(e) {
      if (myself.m_lastCellRightClickParams && myself.mEventRegistry.onCellContextMenu) {
        // first fire the selection change event, since the selection area changed but the table does not send an event
        var params = {};
        params.selectionArea = myself.m_sacTable.table.selectionArea;
        myself.fireOnSelectionChange(params);
        // then the context menu event
        myself.m_lastCellRightClickParams.selectionArea = myself.m_sacTable.table.selectionArea; // fill the params with the selection are
        myself.fireOnCellContextMenu(myself.m_lastCellRightClickParams); // fire the event
        myself.m_lastCellRightClickParams = null; // reset the variable
        myself.m_sacTable.table.selecting = false; // stop selection
  ...
  setModelJson: function(modelJson) {
    return this._setTableModelInternal(modelJson, false);
  },

  //custom methods
  // ======================================

  _setTableModelInternal: function(modelJson, supressInvalidate) {
    var convertedModel = null;
    if (modelJson) {
      convertedModel = sactable.Converter.convert(modelJson);
    }
    return this.setProperty("modelJson", convertedModel, supressInvalidate);
  },
  ...
  renderTableData: function() {
    ...
    if (this.getModelJson()) {
      ...
      this.createTableInstanceIfNeeded();
      if (this.m_sacTable) {
        this.m_sacTable.updateTableData(this.getModelJson());
      ...
      //clear all children
      this.m_widgetContainer.innerHTML = "";
      // no data div
      ...
      noDataDiv.innerHTML = "No table data";
      this.m_widgetContainer.appendChild(noDataDiv);
    }
  },

  createTableInstanceIfNeeded: function() {
    if (!this.m_sacTable && this.m_widgetContainer) {
      ...
      var tableCallbacks = {
        onCellSelected: function() {
          ...
          //  console.log(myself.m_sacTable.table.selectionArea);
        },
        onUpdateNewLineMemberCell: function() {
          //  console.log("onUpdateNewLineMemberCell")
        },
        onCellMouseDown: function(params) {
          //console.log("onCellMouseDown");
          if (params.event.button === 2) { // 2 === right click
            // save the params for the context menu event
            myself.m_lastCellRightClickParams = params;
          } else {
            // generate cell id of clicked cell combination of button row and col
            myself.m_clickEventId = params.event.button + params.row + params.col;
            myself.m_lastCellRightClickParams = null;
          }
        },
        onCellMouseUp: function(params) {
          ...
          if (myself.m_clickEventId === tmpCellId && params.event && params.event.button === 0) { // 0 === left click
            params.selectionArea = myself.m_sacTable.table.selectionArea;
            myself.fireOnCellClick(params);
            myself.m_lastCellRightClickParams = null;
          } else if (params.event.button === 2) { // 2 === right click
            myself.m_lastCellRightClickParams = params;
          }
          myself.m_clickEventId = null;
        },
        ...
        onCellKeyUp: function() {
          //      console.log("onCellKeyUp")
        },
        onDrillIconClicked: function(params) {
          //    console.log("onDrillIconClicked")
          myself.fireOnDrillIconClick(params);
        },
        onCellEdit: function() {
          //      console.log("onCellEdit")
        },
        onTitleSelected: function() {
          //        console.log("onTitleSelected")
        },
        onSelectedRegionChanged: function(params) {
          // fires twice on mouse down and on mouse up, for that reason i use the onCellSelected callback above
          //myself.fireOnSelectionChange(params);
      ...
      this.m_sacTable = new sactable.Table(initialModel, this.m_widgetContainer, tableCallbacks);
    }
  },

  // update the table model width dynamic information
  adjustTableModel: function() {
    var tableModel = this.getModelJson();
    if (this.m_widgetContainer && tableModel) {
      //size
      tableModel.widgetHeight = this.m_containerSize.height;
      tableModel.widgetWidth = this.m_containerSize.width;
      //id
      ...
      //other props
      tableModel.showGrid = this.getShowGrid() || false;
      //feature toggles
      ...
      //update the model
      this._setTableModelInternal(tableModel, true);
    }
  },
...
sap.firefly.UxGeneric.prototype.setNativeControl = function(nativeControl) {
  ...
  if (nativeControl !== null) {
    ...
    nativeControl.data("ffItem", this);
  ...
  if (nativeControl && nativeControl.getMetadata && nativeControl.getMetadata().dnd && nativeControl.addDragDropConfig) {
    ...
    //  targetAggregation: "items",
      drop: function(oEvent) {
        ...
        var dropPositionStr = oEvent.getParameters().dropPosition;
        // at least dragged control needs to be there!
        if (nativeDraggedControl) {
          myself._fireOnDropEventIfPossible(nativeDraggedControl, nativeDroppedControl, dropPositionStr);
        }
      },
      dragOver: function(oEvent) {
      ...
      dragEnter: function(oEvent) {
        ...
        var oDragSession = oEvent.getParameter("dragSession");
        var oDraggedControl = oDragSession.getDragControl();
        // if no drag control present then it is probably a file drag event
        if (!oDraggedControl) {
          oEvent.preventDefault();
        }
      }
...
sap.firefly.UxGeneric.prototype._fireOnDropEventIfPossible = function(nativeDraggedControl, nativeDroppedControl, dropPositionStr) {
  if (this.getListenerOnDrop() !== null) {
    ...
    var newDropEvent = sap.firefly.UiDropEvent.createDrop(this, null, draggedControl, droppedControl, dropPositionStr);
...
sap.firefly.UxGeneric.prototype.setIcon = function(value) {
  ...
  var iconUri = sap.ui.core.IconPool.getIconURI(value);
  if (!iconUri) {
    iconUri = value;
  }
...
sap.firefly.UxGeneric.prototype.getMatrix = function() {
  return sap.firefly.DfUiContext.prototype.getMatrix.call(this);
};

sap.firefly.UxGeneric.prototype.setMatrix = function(value) {
  sap.firefly.DfUiContext.prototype.setMatrix.call(this, value);
...
sap.firefly.UxGeneric.prototype.setTag = function(tag) {
  ...
  // add firefly tag property as class and data-sap-help-id attribute to the sapui5 control
  if (this.getNativeControl() !== null && this.getNativeControl().addStyleClass !== undefined && tag && tag.length > 0) {
    this.getNativeControl().addStyleClass(tag);
    this.getNativeControl().data("sap-help-id", tag, true);
  }
  return this;
};
...
sap.firefly.UxGeneric.prototype._addDraggable = function() {
  ...
  if (nativeControl && nativeControl.getMetadata && nativeControl.getMetadata().dnd && nativeControl.addDragDropConfig) {
    ...
    nativeControl.addDragDropConfig(new sap.ui.core.dnd.DragInfo({
...
sap.firefly.UxGeneric.prototype._removeDraggable = function() {
  ...
  if (nativeControl && nativeControl.getMetadata && nativeControl.getMetadata().dnd && nativeControl.removeAllDragDropConfig) {
    ...
    nativeControl.removeAllDragDropConfig();
  }
};
...
sap.firefly.UxGeneric.prototype.setBorderSize = function(borderSize) {
  sap.firefly.DfUiContext.prototype.setBorderSize.call(this, borderSize);
  var borderStyle = this._calculateBorderStyleCss();
  this._updateControlProperty(this.applyBorderStyleCss, null, borderStyle, "borderSize");
  return this;
};
...
sap.firefly.UxGeneric.prototype.setBorderColor = function(color) {
  ...
  var borderStyle = this._calculateBorderStyleCss();
  this._updateControlProperty(this.applyBorderStyleCss, null, borderStyle, "borderColor");
  return this;
};
...
sap.firefly.UxGeneric.prototype.calculateHeightCss = function() {
  var heightCss = this.getHeightCss();
  return heightCss;
};
...
sap.firefly.UxGeneric.prototype.calculatePaddingCss = function() {
  var paddingCss = this.getPaddingCss();
  return paddingCss;
};
...
sap.firefly.UxGeneric.prototype.calculateMarginCss = function() {
  var marginCss = this.getMarginCss();
  return marginCss;
};
...
sap.firefly.UxGeneric.prototype.applyBorderStyleCss = function(element, borderCss) {
...
sap.firefly.UxGeneric.prototype._calculateBorderStyleCss = function() {
  ...
  var borderStyleCss = "solid";

  if (borderColor !== null) {
    borderStyleCss = borderStyleCss + " " + borderColor.getRgbaColor();
  }

  if (borderSize !== null) {
    borderStyleCss = borderStyleCss + " " + borderSize.getCssValue();
  }

  // if no size or color specified then simply remove the border styling
  if (borderStyleCss === "solid") {
    borderStyleCss = null;
  }
...
sap.firefly.UxGeneric.prototype._calculateCornerRadiusCss = function() {
  var cornerRadiusCss = this.getCornerRadiusCss();
  return cornerRadiusCss;
};
...
sap.firefly.UxGeneric.prototype._calculateAlignSelfCss = function() {
  var alignSelfConst = this.getAlignSelf();
  var alignSelfCss = null;
  if (alignSelfConst) {
    alignSelfCss = alignSelfConst.getCssValue();
  }
  return alignSelfCss;
};
...
sap.firefly.UxGeneric.prototype._calculateFontColorCss = function() {
  ...
  var fontColorCss = null;
  if (fontColor) {
    fontColorCss = fontColor.getRgbaColor();
  }
  return fontColorCss;
};

sap.firefly.UxGeneric.prototype._calculateFontSizeCss = function() {
  var fontSizeCss = this.getFontSizeCss();
  return fontSizeCss;
};

sap.firefly.UxGeneric.prototype._calculateFontStyleCss = function() {
  var fontStyleConst = this.getFontStyle();
  var fontStyleCss = null;
  if (fontStyleConst) {
    fontStyleCss = fontStyleConst.getCssValue();
  }
  return fontStyleCss;
};

sap.firefly.UxGeneric.prototype._calculateFontWeightCss = function() {
  var fontWeightConst = this.getFontWeight();
  var fontWeightCss = null;
  if (fontWeightConst) {
    fontWeightCss = fontWeightConst.getCssValue();
  }
  return fontWeightCss;
};

sap.firefly.UxGeneric.prototype._calculateOverflowCss = function() {
  var overflowConst = this.getOverflow();
  var overflowCss = null;
  if (overflowConst) {
    overflowCss = overflowConst.getCssValue();
  }
  return overflowCss;
};
...
sap.firefly.UxIcon.prototype.setIcon = function(value) {
  sap.firefly.UxGeneric.prototype.setIcon.call(this, value);
  var iconUri = sap.ui.core.IconPool.getIconURI(value);
  this.getNativeControl().setSrc(iconUri);
  return this;
};

sap.firefly.UxIcon.prototype.getIcon = function() {
  return sap.firefly.UxGeneric.prototype.getIcon.call(this);
};
...
sap.firefly.UxIcon.prototype.setFontColor = function(fontColor) {
  sap.firefly.UxGeneric.prototype.setFontColor.call(this, fontColor);
  return this;
};

sap.firefly.UxIcon.prototype.getFontColor = function() {
  return sap.firefly.UxGeneric.prototype.getFontColor.call(this);
};

sap.firefly.UxIcon.prototype.setFontSize = function(fontSize) {
  sap.firefly.UxGeneric.prototype.setFontSize.call(this, fontSize);
  return this;
};

sap.firefly.UxIcon.prototype.getFontSize = function() {
...
sap.firefly.UxInput.prototype._addEvents = function(nativeControl) {
  ...
  nativeControl.attachSuggestionItemSelected(function(oEvent) {
    if (myself.getListenerOnSuggestionSelect() !== null) {
      ...
      var selectedItem = selectedNativeItem.data("ffItem");
...
// special border styling handling
sap.firefly.UxInput.prototype.applyBorderStyleCss = function(element, borderCss) {
  $(element).find(".sapMInputBaseContentWrapper").css("border", borderCss);
};
...
sap.firefly.UxSearchField.prototype._addEvents = function(nativeControl) {
  ...
  nativeControl.attachSearch(function(oEvent) {
    ...
    if (selectedSuggestionItem != null) {
      if (myself.getListenerOnSuggestionSelect() !== null) {
        var selectedSuggestionItem = selectedSuggestionItem.data("ffItem");
...
sap.firefly.UxLabel.prototype.getFontWeight = function() {
  return sap.firefly.UxGeneric.prototype.getFontWeight.call(this);
};
...
sap.firefly.UxRadioButtonGroup.prototype.getSelectedItem = function() {
  ...
  if (selectedItem != null) {
    return selectedItem.data("ffItem");
  }
  return null;
};
  ...
  nativeControl.attachExpand(function(oControlEvent) {
    ...
    if (isExpand === true) {
      if (myself.getListenerOnExpand() !== null) {
        myself.getListenerOnExpand().onExpand(sap.firefly.UiControlEvent.create(myself));
      }
    } else {
      if (myself.getListenerOnCollapse() !== null) {
        myself.getListenerOnCollapse().onCollapse(sap.firefly.UiControlEvent.create(myself));
      }
    }
  });
};
...
sap.firefly.UxSegmentedButton.prototype._addEvents = function(nativeControl) {
  ...
  nativeControl.attachSelectionChange(function(oControlEvent) {
    ...
    if (nativeItem !== null) {
      var selectedItem = nativeItem.data("ffItem");
    }
...
sap.firefly.UxTree.prototype._addEvents = function(nativeControl) {
  ...
  nativeControl.attachToggleOpenState(function(oControlEvent) {
    ...
    var mobileTreeItem = nativeItem.data("ffItem");
    ...
    if (isExpanded) {
      ...
      // tree control event
      if (myself.getListenerOnExpand() !== null) {
        var uiEventSelection = sap.firefly.UiSelectionEvent.createSingleSelection(myself, null, mobileTreeItem);
        myself.getListenerOnExpand().onExpand(uiEventSelection);
      }
    } else {
      // items event
      ...
      if (myself.getListenerOnCollapse() !== null) {
        var uiEventSelection = sap.firefly.UiSelectionEvent.createSingleSelection(myself, null, mobileTreeItem);
        myself.getListenerOnCollapse().onCollapse(uiEventSelection);
      }
    }
  });
  ...
  nativeControl.attachSelectionChange(function(oControlEvent) {
    ...
    if (isSelect === true) {
      if (myself.getListenerOnSelect() !== null) {
        ...
        var selectedItem = listItem.data("ffItem");
  ...
  nativeControl.attachDelete(function(oControlEvent) {
    if (myself.getListenerOnDelete() !== null) {
      ...
      var deletedItem = nativeTreeItem.data("ffItem");
      myself.getListenerOnDelete().onDelete(sap.firefly.UiSelectionEvent.createSingleSelection(myself, null, deletedItem));
    }
  });
};
...
sap.firefly.UxTree.prototype.getSelectedItem = function() {
  ...
  if (nativeSelectedItem) {
    return nativeSelectedItem.data("ffItem");
  }
  return null;
};
...
sap.firefly.UxTree.prototype.getSelectedItems = function() {
  ...
  for (var i = 0; i < nativeSelectedItems.length; i++) {
    ...
    selectedItems.add(tmpNativeTreeItem.data("ffItem"));
  }
...
sap.firefly.UxTree.prototype.createTreeModel = function() {
  ...
  if (this.getNativeControl().getModel().getJSON() == null || this.getNativeControl().getModel().getJSON().length <= 2) {
    ...
    this.getNativeControl().bindItems("/", function(sId, oContext) {
      ...
      if (mobileTreeItem) {
        var iconUri = sap.ui.core.IconPool.getIconURI(mobileTreeItem.getIcon());

        var nativeTreeItem = new sap.m.StandardTreeItem({
          title: mobileTreeItem.getText(),
          highlight: mobileTreeItem.getHighlight() != null ? mobileTreeItem.getHighlight().getName() : null,
          selected: mobileTreeItem.isSelected(),
          icon: iconUri,
          visible: mobileTreeItem.isVisible(),
          busy: mobileTreeItem.isBusy(),
          type: mobileTreeItem.getListItemType() != null ? mobileTreeItem.getListItemType().getName() : null,
          counter: mobileTreeItem.getCounter(),
        });

        nativeTreeItem.addEventDelegate({
          onclick: function() {
            if (mobileTreeItem) {
              mobileTreeItem.itemClicked();
            }
          },
          ondblclick: function() {
            if (mobileTreeItem) {
              mobileTreeItem.itemDblClicked();
            }
          }
        });

        nativeTreeItem.attachDetailPress(function(oControlEvent) {
          if (mobileTreeItem) {
            mobileTreeItem.itemDetailPressed();
          }
        });

        nativeTreeItem.oncontextmenu = function(oControlEvent) {
          if (mobileTreeItem) {
            mobileTreeItem.itemContextMenuClick(oControlEvent);
          }
        };

        mobileTreeItem.setNativeControl(nativeTreeItem);

        return nativeTreeItem
      }
      return null;
    });
  } else {
...
sap.firefly.UxTreeItem.prototype = new sap.firefly.UxGeneric();
...
sap.firefly.UxTreeItem.prototype._addEvents = function(nativeControl) {
  var myself = this;
};

// ======================================

sap.firefly.UxTreeItem.prototype.addItem = function(item) {
  sap.firefly.UxGeneric.prototype.addItem.call(this, item);
  this.createTreeModel();
  return this;
};

sap.firefly.UxTreeItem.prototype.insertItem = function(item, index) {
  sap.firefly.UxGeneric.prototype.insertItem.call(this, item, index);
  this.createTreeModel();
  return this;
};

sap.firefly.UxTreeItem.prototype.removeItem = function(item) {
  sap.firefly.UxGeneric.prototype.removeItem.call(this, item);
  this.createTreeModel();
  return this;
};

sap.firefly.UxTreeItem.prototype.clearItems = function() {
  sap.firefly.UxGeneric.prototype.clearItems.call(this);
  this.createTreeModel();
  return this;
};
...
sap.firefly.UxTreeItem.prototype.setEnabled = function(enabled) {
  sap.firefly.DfUiContext.prototype.setEnabled.call(this, enabled); // skip superclass implementation
  if (this.getNativeControl()) {
    this.getNativeControl().setBlocked(!enabled);
  } else {
    this.setProperty("blocked", !enabled);
  }
  return this;
};

sap.firefly.UxTreeItem.prototype.isEnabled = function() {
  if (this.getNativeControl()) {
...
sap.firefly.UxTreeItem.prototype.setSelected = function(selected) {
  sap.firefly.UxGeneric.prototype.setSelected.call(this, selected);
  if (this.getNativeControl()) {
    this.getNativeControl().setSelected(selected);
  }
...
sap.firefly.UxTreeItem.prototype.isSelected = function() {
  if (this.getNativeControl()) {
...
sap.firefly.UxTreeItem.prototype.setBusy = function(busy) {
  sap.firefly.UxGeneric.prototype.setBusy.call(this, busy);
  return this;
};

sap.firefly.UxTreeItem.prototype.isBusy = function() {
  return sap.firefly.UxGeneric.prototype.isBusy.call(this);
};
...
sap.firefly.UxTreeItem.prototype.setCounter = function(counter) {
  sap.firefly.UxGeneric.prototype.setCounter.call(this, counter);
  if (this.getNativeControl()) {
    this.getNativeControl().setCounter(value);
  }
  return this;
};

sap.firefly.UxTreeItem.prototype.getCounter = function() {
  return sap.firefly.UxGeneric.prototype.getCounter.call(this);
};

sap.firefly.UxTreeItem.prototype.setHighlight = function(messageType) {
...
sap.firefly.UxTreeItem.prototype.getHighlight = function() {
  return sap.firefly.UxGeneric.prototype.getHighlight.call(this);
};

sap.firefly.UxTreeItem.prototype.setListItemType = function(listItemType) {
...
sap.firefly.UxTreeItem.prototype.getListItemType = function() {
  return sap.firefly.UxGeneric.prototype.getListItemType.call(this);
};

sap.firefly.UxTreeItem.prototype.setExpanded = function(expanded) {
  sap.firefly.UxGeneric.prototype.setExpanded.call(this, expanded);
  if (expanded === true) {
...
sap.firefly.UxTreeItem.prototype.isExpanded = function() {
  if (this.getNativeControl()) {
    return this.getNativeControl().getExpanded();
  }
  return sap.firefly.UxGeneric.prototype.isExpanded.call(this);
...
sap.firefly.UxTreeItem.prototype.setNode = function(node) {
  sap.firefly.UxGeneric.prototype.setNode.call(this, node);
  if (this.getNativeControl()) {
...
sap.firefly.UxTreeItem.prototype.isNode = function() {
  if (this.getNativeControl()) {
    return !this.getNativeControl().isLeaf();
  }
  return sap.firefly.UxGeneric.prototype.isNode.call(this);
};
...
sap.firefly.UxTreeItem.prototype.createTreeModel = function() {
  if (this.getParent()) {
    this.getParent().createTreeModel();
  }
};

sap.firefly.UxTreeItem.prototype.expandNativeItem = function(item) {
  if (this.getParent()) {
    this.getParent().expandNativeItem(item);
  }
};

sap.firefly.UxTreeItem.prototype.collapseNativeItem = function(item) {
  if (this.getParent()) {
    this.getParent().collapseNativeItem(item);
  }
};

sap.firefly.UxTreeItem.prototype.itemClicked = function() {
  if (this.getListenerOnClick() !== null) {
    this.getListenerOnClick().onClick(sap.firefly.UiControlEvent.create(this));
  }
};

sap.firefly.UxTreeItem.prototype.itemDblClicked = function() {
  if (this.getListenerOnDoubleClick() !== null) {
    this.getListenerOnDoubleClick().onDoubleClick(sap.firefly.UiControlEvent.create(this));
  }
};

sap.firefly.UxTreeItem.prototype.itemDetailPressed = function() {
  if (this.getListenerOnDetailPress() !== null) {
    this.getListenerOnDetailPress().onDetailPress(sap.firefly.UiControlEvent.create(this));
  }
};

sap.firefly.UxTreeItem.prototype.itemExpanded = function() {
  if (this.getListenerOnExpand() !== null) {
    var uiEventSelection = sap.firefly.UiSelectionEvent.createSingleSelection(this, null, null);
    this.getListenerOnExpand().onExpand(uiEventSelection);
  }
};

sap.firefly.UxTreeItem.prototype.itemCollapsed = function() {
  if (this.getListenerOnCollapse() !== null) {
    var uiEventSelection = sap.firefly.UiSelectionEvent.createSingleSelection(this, null, null);
    this.getListenerOnCollapse().onCollapse(uiEventSelection);
  }
};

sap.firefly.UxTreeItem.prototype.itemContextMenuClick = function(oControlEvent) {
...
sap.firefly.UxResponsiveTable.prototype._addEvents = function(nativeControl) {
  ...
  nativeControl.attachSelectionChange(function(oControlEvent) {
    ...
    if (isSelect === true) {
      if (myself.getListenerOnSelect() !== null) {
        ...
        var selectedRow = row.data("ffItem");
  ...
  nativeControl.attachDelete(function(oControlEvent) {
    if (myself.getListenerOnDelete() !== null) {
      ...
      var deletedItem = nativeItem.data("ffItem");
      myself.getListenerOnDelete().onDelete(sap.firefly.UiSelectionEvent.createSingleSelection(myself, null, deletedItem));
    }
  });
...
sap.firefly.UxResponsiveTable.prototype.getSelectedItems = function() {
  ...
  for (var i = 0; i < aSelectedItems.length; i++) {
    oList.add(aSelectedItems[i].data("ffItem"));
  }
  return oList;
};
...
sap.firefly.UxTreeTable.prototype._addEvents = function(nativeControl) {
  ...
  nativeControl.attachToggleOpenState(function(oControlEvent) {
    ...
    if (isExpanded) {
      ...
      // tree control event
      if (myself.getListenerOnExpand() !== null) {
        var uiEventSelection = sap.firefly.UiSelectionEvent.createSingleSelection(myself, null, treeTableRow);
        myself.getListenerOnExpand().onExpand(uiEventSelection);
      }
    } else {
      // items event
...
sap.firefly.UxTreeTableRow.prototype.rowExpanded = function() {
  if (this.getListenerOnExpand() !== null) {
    var newEvent = sap.firefly.UiControlEvent.create(this);
...
sap.firefly.UxDropDown.prototype._addEvents = function(nativeControl) {
  ...
  nativeControl.attachChange(function(oEvent) {
    ...
    if (nativeNode !== null) {
      selectedItem = nativeNode.data("ffItem");
...
sap.firefly.UxDropDown.prototype.setValueStateText = function(valueStateText) {
  sap.firefly.UxGeneric.prototype.setValueStateText.call(this, requvalueStateTextired);
  this.getNativeControl().setValueStateText(valueStateText);
  return this;
};
...
sap.firefly.UxComboBox.prototype = new sap.firefly.UxGeneric();
...
sap.firefly.UxComboBox.prototype._addEvents = function(nativeControl) {
  var myself = this;

  // onSelectionChange event
  nativeControl.attachSelectionChange(function(oEvent) {
    if (myself.getListenerOnSelect() !== null && oEvent.getParameters().selectedItem != null) {
      var nativeNode = oEvent.getParameters().selectedItem;
      if (nativeNode !== null) {
        var selectedItem = nativeNode.data("ffItem");
        var ffEvent = sap.firefly.UiSelectionEvent.createSingleSelection(myself, null, selectedItem);
        myself.getListenerOnSelect().onSelect(ffEvent);
      }
    }
...
sap.firefly.UxComboBox.prototype.addItem = function(item) {
  sap.firefly.UxGeneric.prototype.addItem.call(this, item);
  var nativeItem = item.getNativeControl();
  this.getNativeControl().addItem(nativeItem);
  return this;
...
sap.firefly.UxComboBox.prototype.insertItem = function(item, index) {
  sap.firefly.UxGeneric.prototype.insertItem.call(this, item, index);
  var nativeItem = item.getNativeControl();
  this.getNativeControl().insertItem(nativeItem, index);
  return this;
...
sap.firefly.UxComboBox.prototype.removeItem = function(item) {
  var nativeItem = item.getNativeControl();
  this.getNativeControl().removeItem(nativeItem);
  sap.firefly.UxGeneric.prototype.removeItem.call(this, item);
  return this;
...
sap.firefly.UxComboBox.prototype.clearItems = function() {
  sap.firefly.UxGeneric.prototype.clearItems.call(this);
  this.getNativeControl().removeAllItems();
  return this;
};
...
sap.firefly.UxComboBox.prototype.open = function() {
  sap.firefly.UxGeneric.prototype.open.call(this);
  this.getNativeControl().open();
  return this;
};

sap.firefly.UxComboBox.prototype.close = function() {
  sap.firefly.UxGeneric.prototype.close.call(this);
  this.getNativeControl().close();
  return this;
};

sap.firefly.UxComboBox.prototype.isOpen = function() {
  return this.getNativeControl().isOpen();
};

// ======================================

sap.firefly.UxComboBox.prototype.setText = function(text) {
  sap.firefly.DfUiContext.prototype.setText.call(this, text); // skip superclass implementation
  this.getNativeControl().setValue(text);
  return this;
};

sap.firefly.UxComboBox.prototype.getText = function() {
  return this.getNativeControl().getValue();
};

sap.firefly.UxComboBox.prototype.setPlaceholder = function(placeholder) {
  sap.firefly.UxGeneric.prototype.setPlaceholder.call(this, placeholder);
  this.getNativeControl().setPlaceholder(placeholder);
  return this;
};

sap.firefly.UxComboBox.prototype.getPlaceholder = function() {
  return this.getNativeControl().getPlaceholder();
};

sap.firefly.UxComboBox.prototype.setRequired = function(required) {
  sap.firefly.UxGeneric.prototype.setRequired.call(this, required);
  return this;
};

sap.firefly.UxComboBox.prototype.isRequired = function() {
  return sap.firefly.UxGeneric.prototype.isRequired.call(this);
};

sap.firefly.UxComboBox.prototype.setValueState = function(valueState) {
  sap.firefly.UxGeneric.prototype.setValueState.call(this, valueState);
  var newValueState = sap.ui.core.ValueState.None;
  if (valueState === sap.firefly.UiValueState.NONE) {
    newValueState = sap.ui.core.ValueState.None;
...
sap.firefly.UxComboBox.prototype.getValueState = function() {
  return sap.firefly.UxGeneric.prototype.getValueState.call(this);
};

sap.firefly.UxComboBox.prototype.setValueStateText = function(valueStateText) {
  sap.firefly.UxGeneric.prototype.setValueStateText.call(this, valueStateText);
  this.getNativeControl().setValueStateText(valueStateText);
  return this;
};

sap.firefly.UxComboBox.prototype.getValueStateText = function() {
  return this.getNativeControl().getValueStateText();
};
...
sap.firefly.UxComboBox.prototype.setHeight = function(height) {
  // remove height from the object
  // don't change the Dropdown height on JavaScript, it should only be done on iOS
  sap.firefly.UxGeneric.prototype.setHeight.call(this, null);
  return this;
...
sap.firefly.UxMultiComboBox.prototype = new sap.firefly.UxGeneric();
...
sap.firefly.UxMultiComboBox.prototype._addEvents = function(nativeControl) {
  var myself = this;


  // onSelectionChange event
  nativeControl.attachSelectionChange(function(oEvent) {
    if (myself.getListenerOnSelect() !== null && oEvent.getParameters().selected === true) {
    ...
    if (myself.getListenerOnSelectionChange() !== null) {
      myself.getListenerOnSelectionChange().onSelectionChange(sap.firefly.UiSelectionEvent.createEmptySelection(myself, null));
    }
  });

  // onSelectionFinish event
  nativeControl.attachSelectionFinish(function(oEvent) {
    if (myself.getListenerOnSelectionFinish() !== null) {
      ...
      for (var i = 0; i < nativeSelectedItems.length; i++) {
        oSelectedItemsList.add(nativeSelectedItems[i].data("ffItem"));
      }

      myself.getListenerOnSelectionFinish().onSelectionFinish(sap.firefly.UiSelectionEvent.createMultiSelection(myself, null, oSelectedItemsList));
    }
  });
...
sap.firefly.UxMultiComboBox.prototype.addItem = function(item) {
  sap.firefly.UxGeneric.prototype.addItem.call(this, item);
  var nativeItem = item.getNativeControl();
  this.getNativeControl().addItem(nativeItem);
  return this;
...
sap.firefly.UxMultiComboBox.prototype.insertItem = function(item, index) {
  sap.firefly.UxGeneric.prototype.insertItem.call(this, item, index);
  var nativeItem = item.getNativeControl();
  this.getNativeControl().insertItem(nativeItem, index);
  return this;
...
sap.firefly.UxMultiComboBox.prototype.removeItem = function(item) {
  var nativeItem = item.getNativeControl();
  this.getNativeControl().removeItem(nativeItem);
  sap.firefly.UxGeneric.prototype.removeItem.call(this, item);
  return this;
...
sap.firefly.UxMultiComboBox.prototype.clearItems = function() {
  sap.firefly.UxGeneric.prototype.clearItems.call(this);
  this.getNativeControl().removeAllItems();
  return this;
};
...
sap.firefly.UxMultiComboBox.prototype.openAt = function(control) {
  sap.firefly.UxGeneric.prototype.openAt.call(this, control);
  this.open();
  return this;
};

sap.firefly.UxMultiComboBox.prototype.open = function() {
  sap.firefly.UxGeneric.prototype.open.call(this);
  this.getNativeControl().open();
  return this;
};

sap.firefly.UxMultiComboBox.prototype.close = function() {
  sap.firefly.UxGeneric.prototype.close.call(this);
  this.getNativeControl().close();
  return this;
};

sap.firefly.UxMultiComboBox.prototype.isOpen = function() {
  return this.getNativeControl().isOpen();
};

// ======================================

sap.firefly.UxMultiComboBox.prototype.setText = function(text) {
  sap.firefly.DfUiContext.prototype.setText.call(this, text); // skip superclass implementation
  this.getNativeControl().setValue(text);
  return this;
};

sap.firefly.UxMultiComboBox.prototype.getText = function() {
  return this.getNativeControl().getValue();
};

sap.firefly.UxMultiComboBox.prototype.setPlaceholder = function(placeholder) {
  sap.firefly.UxGeneric.prototype.setPlaceholder.call(this, placeholder);
  this.getNativeControl().setPlaceholder(placeholder);
  return this;
};

sap.firefly.UxMultiComboBox.prototype.getPlaceholder = function() {
  return this.getNativeControl().getPlaceholder();
};

sap.firefly.UxMultiComboBox.prototype.setRequired = function(required) {
  sap.firefly.UxGeneric.prototype.setRequired.call(this, required);
  return this;
};

sap.firefly.UxMultiComboBox.prototype.isRequired = function() {
  return sap.firefly.UxGeneric.prototype.isRequired.call(this);
};
...
sap.firefly.UxMultiComboBox.prototype.setHeight = function(height) {
  // remove height from the object
  // don't change the Dropdown height on JavaScript, it should only be done on iOS
  sap.firefly.UxGeneric.prototype.setHeight.call(this, null);
  return this;
...
sap.firefly.UxListItemBase.prototype.newInstance = function() {
  var object = new sap.firefly.UxListItemBase();
  object.setup();
  return object;
};
...
sap.firefly.UxHtml.prototype.setValue = function(value) {
  ...
  if (value && value.length > 0) {
    if (this._isURL(value)) {
      this.getNativeControl().setContent("<div><iframe style='width:100%;height:100%' src='" + value + "'></iframe></div>");
    } else {
...
sap.firefly.UxTabStrip = function() {
   sap.firefly.UxGeneric.call(this);
  this._ff_c = "UxTabStrip";
};
sap.firefly.UxTabStrip.prototype = new sap.firefly.UxGeneric();

sap.firefly.UxTabStrip.prototype.newInstance = function() {
  var object = new sap.firefly.UxTabStrip();
  object.setup();
  return object;
};

sap.firefly.UxTabStrip.prototype.initializeNative = function() {
  sap.firefly.UxGeneric.prototype.initializeNative.call(this);
  var myself = this;
...
sap.firefly.UxTabStrip.prototype.releaseObject = function() {
  sap.firefly.UxGeneric.prototype.releaseObject.call(this);
};

// ======================================

sap.firefly.UxTabStrip.prototype._addEvents = function(nativeControl) {
  var myself = this;
...
sap.firefly.UxTabStrip.prototype.addItem = function(item) {
  sap.firefly.UxGeneric.prototype.addItem.call(this, item);
  var nativeItem = item.getNativeControl();
  this.getNativeControl().addItem(nativeItem);
  return this;
...
sap.firefly.UxTabStrip.prototype.insertItem = function(item, index) {
  sap.firefly.UxGeneric.prototype.insertItem.call(this, item, index);
  var nativeItem = item.getNativeControl();
  this.getNativeControl().insertItem(nativeItem, index);
  return this;
...
sap.firefly.UxTabStrip.prototype.removeItem = function(item) {
  var nativeItem = item.getNativeControl();
  this.getNativeControl().removeItem(nativeItem);
  sap.firefly.UxGeneric.prototype.removeItem.call(this, item);
  return this;
...
sap.firefly.UxTabStrip.prototype.clearItems = function() {
  sap.firefly.UxGeneric.prototype.clearItems.call(this);
  this.getNativeControl().removeAllItems();
  return this;
};
...
sap.firefly.UxTabStrip.prototype.setSelectedItem = function(selectedItem) {
  sap.firefly.UxGeneric.prototype.setSelectedItem.call(this, selectedItem);
  if (selectedItem !== null) {
...
sap.firefly.UxTabStrip.prototype.getSelectedItem = function() {
...
sap.firefly.UxTabStrip.prototype.setBusy = function(busy) {
  sap.firefly.UxGeneric.prototype.setBusy.call(this, busy);
  return this;
};

sap.firefly.UxTabStrip.prototype.isBusy = function() {
  return sap.firefly.UxGeneric.prototype.isBusy.call(this);
};
...
sap.firefly.UxTabStrip.prototype.applyCustomCssStyling = function(element) {
...
sap.firefly.UxTabStripItem = function() {
   sap.firefly.UxGeneric.call(this);
  this._ff_c = "UxTabStripItem";
};
sap.firefly.UxTabStripItem.prototype = new sap.firefly.UxGeneric();

sap.firefly.UxTabStripItem.prototype.newInstance = function() {
  var object = new sap.firefly.UxTabStripItem();
  object.setup();
  return object;
};

sap.firefly.UxTabStripItem.prototype.initializeNative = function() {
  sap.firefly.UxGeneric.prototype.initializeNative.call(this);
  var myself = this;
...
sap.firefly.UxTabStripItem.prototype.releaseObject = function() {
  sap.firefly.UxGeneric.prototype.releaseObject.call(this);
};

// ======================================

sap.firefly.UxTabStripItem.prototype._addEvents = function(nativeControl) {
  var myself = this;
};

// ======================================

sap.firefly.UxTabStripItem.prototype.setContent = function(content) {
  sap.firefly.UxGeneric.prototype.setContent.call(this, content);
  this.getNativeControl().removeAllContent();
  if (content !== null) {
...
sap.firefly.UxTabStripItem.prototype.getContent = function() {
  return sap.firefly.UxGeneric.prototype.getContent.call(this);
};

sap.firefly.UxTabStripItem.prototype.clearContent = function() {
  sap.firefly.UxGeneric.prototype.clearContent.call(this);
  this.getNativeControl().removeAllContent();
  return this;
};
...
sap.firefly.UxTabStripItem.prototype.setText = function(text) {
  sap.firefly.UxGeneric.prototype.setText.call(this, text);
  return this;
};

sap.firefly.UxTabStripItem.prototype.getText = function() {
  return sap.firefly.UxGeneric.prototype.getText.call(this);
};

sap.firefly.UxTabStripItem.prototype.setIcon = function(icon) {
  sap.firefly.UxGeneric.prototype.setIcon.call(this, icon);
  return this;
};

sap.firefly.UxTabStripItem.prototype.getIcon = function() {
  return sap.firefly.UxGeneric.prototype.getIcon.call(this);
};
...
sap.firefly.UxDialog.prototype.initializeNative = function() {
  ...
  nativeControl.setResizable(true);
  nativeControl.setDraggable(true);
...
sap.firefly.UxDialog.prototype.applyCustomCssStyling = function(element) {
  // scroll content should always have 100% (default is auto), this makes dynamic laouting better
...
sap.firefly.UxTileContainer.prototype.clickEventHandler = function(oControlEvent) {
  if (oControlEvent && oControlEvent.srcControl && oControlEvent.srcControl != this.getNativeControl()) {
    ...
    var ffTileItemControl = parent.data("ffItem");
    ...
    if (ffTileItemControl && ffTileItemControl.getUiType() === sap.firefly.UiType.TILE_CONTAINER) {
      ffTileItemControl = control.data("ffItem");
    }
...
sap.firefly.UxTileContainer.prototype.fireOnContextMenuClickEvent = function(oControlEvent) {
  var control = oControlEvent.srcControl;
  var ffControl = control.data("ffItem");
...
sap.firefly.UxHighChart = function() {
   sap.firefly.UxGeneric.call(this);
  this._ff_c = "UxHighChart";
};
sap.firefly.UxHighChart.prototype = new sap.firefly.UxGeneric();

sap.firefly.UxHighChart.prototype.newInstance = function() {
  var object = new sap.firefly.UxHighChart();
  object.setup();
  return object;
};

sap.firefly.UxHighChart.prototype.initializeNative = function() {
  sap.firefly.UxGeneric.prototype.initializeNative.call(this);
  sap.firefly.loadHighchartsIfNeeded();
  var myself = this;
  var nativeControl = new sap.firefly.XtUi5Highcharts(this.getId(), {});
  // var Options = {
  // chart : {
  // renderTo : this.getId()
  // },
  ...
  nativeControl.setupControl();
  // required for realtive sizing calculation (%)
  nativeControl.calculateContentSize(this.getUiManager().getNativeAnchorId());

  this._addEvents(nativeControl);
  this.setNativeControl(nativeControl);
};

sap.firefly.UxHighChart.prototype.releaseObject = function() {
  sap.firefly.UxGeneric.prototype.releaseObject.call(this);
};

// ======================================

sap.firefly.UxHighChart.prototype._addEvents = function(nativeControl) {
  var myself = this;
};

// ======================================

sap.firefly.UxHighChart.prototype.setModelJson = function(jsonModel) {
  sap.firefly.UxGeneric.prototype.setModelJson.call(this, jsonModel);
  this.getNativeControl().setModelJson(jsonModel);
  return this;
};

sap.firefly.UxHighChart.prototype.getModelJson = function() {
  return sap.firefly.UxGeneric.prototype.getModelJson.call(this);
};

sap.firefly.UxHighChart.prototype.setDataManifest = function(dataManifest) {
  sap.firefly.UxGeneric.prototype.setDataManifest.call(this, dataManifest);
  this.getNativeControl().setDataManifest(dataManifest);
  return this;
};

sap.firefly.UxHighChart.prototype.getDataManifest = function() {
  return sap.firefly.UxGeneric.prototype.getDataManifest.call(this);
};
...
sap.firefly.UxHighChart.prototype.applyCornerRadiusCss = function(element, cornerRadiusCss) {
  sap.firefly.UxGeneric.prototype.applyCornerRadiusCss.call(this, element, cornerRadiusCss);
...
sap.firefly.UxWindow.prototype._addEvents = function(nativeControl) {
  ...
  this.m_window.attachOnMove(function(event) {
    if (myself.getListenerOnMove() !== null) {
      var newPosition = sap.firefly.UiPositionValue.create(event.newPosX, event.newPosY);
      var newMoveEvent = sap.firefly.UiMoveEvent.createMove(myself, null, newPosition);
  ...
  this.m_window.attachOnResize(function(event) {
    if (myself.getListenerOnResize() !== null) {
      var newSize = sap.firefly.UiSizeValue.create(event.newWidth, event.newHeight);
      var newResizeEvent = sap.firefly.UiResizeEvent.createResize(myself, null, newSize);
...
sap.firefly.UxAppIcon.prototype._addEvents = function(nativeControl) {
  ...
  nativeControl.attachOnMove(function(oControlEvent) {
    if (myself.getListenerOnMove() !== null) {
      var newPosition = sap.firefly.UiPositionValue.create(oControlEvent.getParameters().newPosX, oControlEvent.getParameters().newPosY);
      var newMoveEvent = sap.firefly.UiMoveEvent.createMove(myself, null, newPosition);
      myself.getListenerOnMove().onMove(newMoveEvent);
    }
  });
...
sap.firefly.UiDriverModule.getInstance = function() {
   if (sap.firefly.UiDriverModule.s_module === null) {
    ...
    sap.firefly.UiType.TAB_STRIP.setFactory(new sap.firefly.UxTabStrip());
    sap.firefly.UiType.TAB_STRIP_ITEM.setFactory(new sap.firefly.UxTabStripItem());
