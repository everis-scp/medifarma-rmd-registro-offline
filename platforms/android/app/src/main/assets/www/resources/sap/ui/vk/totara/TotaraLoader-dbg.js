sap.ui.define([
	...
	TotaraLoader.prototype.processSetSceneCommand = function(jsonContent) {
		...
		if (context) {
			...
			var selectField = context.$select !== undefined ? context.$select : "name,transform,meshId,annotationId,materialId,contentType,visible,opacity,renderOrder,entityId,highlightStyleId,closed";
			var pushViewGroups = context.pushViewGroups !== undefined ? context.pushViewGroups : true;
			var expand = [ "nodes", "bounds" ];

			if (includeAnimation) {
				expand.push("animation");
			}

			if (includeParametric) {
	...
	TotaraLoader.prototype.requestAnnotation = function(sceneVeId, annotationId) {
		...
		var promise = new Promise(function(resolve, reject) {
			...
			materialFinishedCallback = function(newMaterialId) {
				...
				// no texture images/dimensions to load, detach image & imagedetails callback and resolve the promise
				that.onImageFinishedCallbacks.detach(imageFinishedCallback);
				that.onImageDetailsFinishedCallbacks.detach(imageDetailFinishedCallback);
				resolve(fetchedAnnotation);
			};
