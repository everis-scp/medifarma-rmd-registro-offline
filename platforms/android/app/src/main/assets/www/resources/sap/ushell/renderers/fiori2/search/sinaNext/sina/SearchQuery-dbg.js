        ...
        _execute: function (query) {
            ...
            for (var i = 0; i < chartQueries.length; ++i) {
                ...
                if (!dataSourceMetadata) {
                    ...
                    delayedChartQueries.push(chartQuery);
                } else {
                    if (dataSourceMetadata.usage.Facet) {   // eslint-disable-line no-lonely-if
            ...
            var results;
            return Promise.all(requests).then(function (resultsTmp) {
                // execute the delayed chart query calls
                results = resultsTmp;
                var delayedChartQueryRequests = [];
                for (var j = 0; j < delayedChartQueries.length; ++j) {
                    var delayedChartQuery = delayedChartQueries[j];
                    var attributeMetada = query.filter.dataSource.getAttributeMetadata(delayedChartQuery.dimension);
                    if (attributeMetada.usage.Facet) {
                        delayedChartQueryRequests.push(delayedChartQuery.getResultSetAsync());
                    }
                }
                return Promise.all(delayedChartQueryRequests);
            }.bind(this)).then(function (delayedChartQueryResults) {
                // append delayed chart request results to main results
                return results.concat(delayedChartQueryResults);
            }.bind(this)).then(function (resultsTmp) {
                var searchResult = resultsTmp[0];
                var chartResultSets = resultsTmp.slice(1);
