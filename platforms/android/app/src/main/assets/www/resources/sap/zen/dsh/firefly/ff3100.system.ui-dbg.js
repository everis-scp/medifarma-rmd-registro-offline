...
oFF.WeekView.prototype.buildUi = function()
{
	...
	weekNumberLayout.setWidthExt(100, oFF.UiSizeUnit.PERCENT);
	...
	weekNumberButton.setWidthExt(100, oFF.UiSizeUnit.PERCENT);
	weekNumberButton.setHeightExt(100, oFF.UiSizeUnit.PERCENT);
	weekNumberButton.setPaddingValue(0);
	for (var i = 0; i < this.m_weekUiModel.getWeekDays().size(); i++)
	{
		...
		dayButton.setWidthExt(100, oFF.UiSizeUnit.PERCENT);
		dayButton.setHeightExt(100, oFF.UiSizeUnit.PERCENT);
		dayButton.setPaddingValue(0);
		dayButton.setFlex("1");
		...
		{
			dayButton.setBackgroundColor(oFF.UiColor.createByString("#dedede"));
		}
	}
};
...
oFF.FeApolloDialog.prototype.close = function()
{
	if (oFF.notNull(this.m_fileExplorerWindowContainer))
	{
		this.m_fileExplorerWindowContainer.signalExit(0);
	}
};
...
oFF.FeApolloFileExtension.staticSetup = function()
{
	...
	oFF.FeApolloFileExtension.registerNewExtension("qdf", "Gds", "Gds");
	...
	oFF.FeApolloFileExtension.registerNewExtension("js", oFF.TeAthena.DEFAULT_PROGRAM_NAME, "JavaScript Editor").addArgument(oFF.TeAthena.PARAM_TYPE, "js");
	...
	oFF.FeApolloFileExtension.assignAdditionalApolloExtensionToExtension("qdf", "json");
...
oFF.FlexMonthView.prototype.buildUi = function()
{
	...
	this.m_root.setPaddingValue(20);
	if (this.m_displayHeader)
	{
		...
		headerLayout.setWidthExt(100, oFF.UiSizeUnit.PERCENT);
	...
	else
	{
		...
		monthHeaderLayout.setWidthExt(100, oFF.UiSizeUnit.PERCENT);
...
oFF.MonthSelectionView.prototype.buildUi = function()
{
	...
	while (i < this.m_monthNames.size())
	{
		...
		rowLayout.setWidthExt(100, oFF.UiSizeUnit.PERCENT);
		for (var j = 0; j < 3; j++)
		{
...
oFF.UiCredentialsDialog.prototype.showCredentialsDialogInternal = function(title, message, username, password)
{
	...
	this.m_userInput.setWidthCss("100%");
	...
	this.m_passwordInput.setWidthCss("100%");
	...
	this.m_dialog.setWidthCss("400px");
	this.m_dialog.setPaddingCss("15px");
	...
	{
		this.m_dialog.setHeightCss("260px");
	}
...
oFF.SxDlgJavaDoc = function() {};
oFF.SxDlgJavaDoc.prototype = new oFF.XObject();
oFF.SxDlgJavaDoc.prototype._ff_c = "SxDlgJavaDoc";

oFF.SxDlgJavaDoc.JAVA_DOC_URL = "";
oFF.SxDlgJavaDoc.createJavaDocDialog = function(genesis)
{
	oFF.SxDlgJavaDoc.JAVA_DOC_URL = oFF.XStringUtils.concatenate5("https", "://fir", "efly.w", "df.sa", "p.corp/apidoc/");
	if (oFF.isNull(genesis))
	{
		throw oFF.XException.createRuntimeException("Cannot create a Java Doc dialog instance without a genesis. Please sepcify a genesis!");
	}
	var newDialog = new oFF.SxDlgJavaDoc();
	newDialog.setupDialog(genesis);
	return newDialog;
};
oFF.SxDlgJavaDoc.prototype.m_javaDocDialog = null;
oFF.SxDlgJavaDoc.prototype.m_genesis = null;
oFF.SxDlgJavaDoc.prototype.releaseObject = function()
{
	this.m_javaDocDialog = oFF.XObjectExt.release(this.m_javaDocDialog);
	this.m_genesis = null;
	oFF.XObject.prototype.releaseObject.call( this );
};
oFF.SxDlgJavaDoc.prototype.setupDialog = function(genesis)
{
	this.m_genesis = genesis;
	this.buildDialogUi(this.m_genesis);
};
oFF.SxDlgJavaDoc.prototype.buildDialogUi = function(genesis)
{
	this.m_javaDocDialog = this.m_genesis.newControl(oFF.UiType.DIALOG);
	this.m_javaDocDialog.setName("javaDocDialog");
	this.m_javaDocDialog.setTitle("Firefly Javadoc");
	this.m_javaDocDialog.setWidthCss("90vw");
	this.m_javaDocDialog.setHeightCss("80vh");
	this.m_javaDocDialog.setPaddingCss("20px");
	var closeJavaDocDialogBtn = this.m_javaDocDialog.addNewDialogButton();
	closeJavaDocDialogBtn.setName("closeJavaDocDialogBtn");
	closeJavaDocDialogBtn.setText("Close");
	closeJavaDocDialogBtn.registerOnPress(this);
	var javaDocHtml = this.m_javaDocDialog.setNewContent(oFF.UiType.HTML);
	javaDocHtml.setName("javaDocHtmlFrame");
	javaDocHtml.setWidthCss("100%");
	javaDocHtml.setHeightCss("100%");
	javaDocHtml.setValue(oFF.SxDlgJavaDoc.JAVA_DOC_URL);
};
oFF.SxDlgJavaDoc.prototype.openDialog = function()
{
	this.m_javaDocDialog.open();
};
oFF.SxDlgJavaDoc.prototype.onPress = function(event)
{
	var name = event.getControl().getName();
	if (oFF.XString.isEqual("closeJavaDocDialogBtn", name))
	{
		this.m_javaDocDialog.close();
	}
};

oFF.SxDlgUserProfile = function() {};
oFF.SxDlgUserProfile.prototype = new oFF.XObject();
oFF.SxDlgUserProfile.prototype._ff_c = "SxDlgUserProfile";

oFF.SxDlgUserProfile.createUserProfileDialog = function(genesis, userProfile)
{
	if (oFF.isNull(genesis))
	{
		throw oFF.XException.createRuntimeException("Cannot create a User Profile dialog instance without a genesis. Please sepcify a genesis!");
	}
	var usrProfileDlg = new oFF.SxDlgUserProfile();
	usrProfileDlg.setupDialog(genesis, userProfile);
	return usrProfileDlg;
};
oFF.SxDlgUserProfile.prototype.m_userDialog = null;
oFF.SxDlgUserProfile.prototype.m_userProfile = null;
oFF.SxDlgUserProfile.prototype.m_genesis = null;
oFF.SxDlgUserProfile.prototype.releaseObject = function()
{
	this.m_userDialog = oFF.XObjectExt.release(this.m_userDialog);
	this.m_genesis = null;
	this.m_userProfile = null;
	oFF.XObject.prototype.releaseObject.call( this );
};
oFF.SxDlgUserProfile.prototype.setupDialog = function(genesis, userProfile)
{
	this.m_genesis = genesis;
	this.m_userProfile = userProfile;
	this.buildDialogUi(this.m_genesis, this.m_userProfile);
};
oFF.SxDlgUserProfile.prototype.buildDialogUi = function(genesis, userProfile)
{
	this.m_userDialog = genesis.newControl(oFF.UiType.DIALOG);
	this.m_userDialog.setTitle("Profile Settings");
	...
	if (oFF.notNull(userProfile))
	{
		...
		matrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.IMAGE).setHeightCss("80px").setWidthCss("80px").setCornerRadiusCss("50%").setSrc(userProfile.getThumbnailPhoto());
		matrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getPersonFullName());
		...
		matrixRow2.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getFirstName());
		...
		matrixRow3.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getLastName());
		...
		matrixRow4.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getEmailAddress());
		...
		matrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getPhoneNumber());
		...
		matrixRow6.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getMobilePhoneNumber());
		...
		matrixRow7.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getDepartment());
		...
		matrixRow8.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getRoomNumber());
		...
		loginMatrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getSAPName());
		...
		loginMatrixRow2.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getSamaAccountType());
		...
		loginMatrixRow3.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getManagerPersonNumber());
		...
		loginMatrixRow9.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getPersonNumber());
		...
		loginMatrixRow10.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getCostCenter());
		var headerLabel = genesis.newControl(oFF.UiType.LABEL);
		headerLabel.setName("Person");
		headerLabel.setText("Person");
		headerLabel.setFontSizeExt(14, oFF.UiSizeUnit.PIXEL);
		headerLabel.setFontWeight(oFF.UiFontWeight.BOLD);
		verticalAddressLayout.addItem(headerLabel);
		verticalAddressLayout.addItem(genesis.newControl(oFF.UiType.SPACER).setBackgroundColor(oFF.UiColor.GREY).setHeightCss("1px").setWidthCss("100%"));
		...
		addressPersonMatrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getDescription());
		...
		addressPersonMatrixRow2.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getLastName());
		...
		addressPersonMatrixRow3.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getFirstName());
		...
		addressPersonMatrixRow4.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getTitle());
		...
		addressPersonMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getPersonFullName());
		...
		addressPersonMatrixRow6.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getLanguage());
		...
		headerWorkCenterLabel.setFontSizeExt(14, oFF.UiSizeUnit.PIXEL);
		...
		addressWorkCenterMatrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getDescription());
		...
		addressWorkCenterMatrixRow2.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getDepartment());
		...
		addressWorkCenterMatrixRow3.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getRoomNumber());
		...
		addressWorkCenterMatrixRow4.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(oFF.XString.substring(userProfile.getRoomNumber(), oFF.XString.lastIndexOf(userProfile.getRoomNumber(), ",") + 1, oFF.XString.lastIndexOf(userProfile.getRoomNumber(), ".")));
		...
		addressWorkCenterMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(oFF.XString.substring(userProfile.getRoomNumber(), 0, oFF.XString.lastIndexOf(userProfile.getRoomNumber(), ",")));
		...
		headerCommuncationLabel.setFontSizeExt(14, oFF.UiSizeUnit.PIXEL);
		...
		addressCommunicationMatrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getPhoneNumber());
		...
		addressCommunicationMatrixRow3.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getMobilePhoneNumber());
		...
		addressCommunicationMatrixRow4.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getFaxNumber());
		...
		addressCommunicationMatrixRow6.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getEmailAddress());
		...
		addressCommunicationMatrixRow7.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getTelephoneAssistant());
		...
		headerCompanyLabel.setFontSizeExt(14, oFF.UiSizeUnit.PIXEL);
		...
		addressCompanyMatrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getCompany());
		...
		defaultMatrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getLanguage());
		...
		defaultMatrixRow2.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getDataAccessLanguage());
		...
		defaultMatrixRow3.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getDateFormatting());
		...
		defaultMatrixRow4.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getTimeFormatting());
		...
		defaultMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getNumberFormatting());
		...
		defaultMatrixRow6.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getScaleFormatting());
		...
		defaultMatrixRow7.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getCurrencyPosition());
		...
		defaultMatrixRow8.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getDefaultApplication());
		...
		defaultMatrixRow9.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getCleanUpNotification());
		...
		defaultMatrixRow10.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getEmailSystemNotification());
		...
		defaultMatrixRow11.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(userProfile.getEmailProductUpdateNotification());
	...
	else
	{
		...
		headerLabel11.setFontSizeExt(14, oFF.UiSizeUnit.PIXEL);
		...
		headerWorkCenterLabe1_1.setFontSizeExt(14, oFF.UiSizeUnit.PIXEL);
		...
		headerCommuncationLabel1.setFontSizeExt(14, oFF.UiSizeUnit.PIXEL);
		...
		headerCompanyLabel1_1.setFontSizeExt(14, oFF.UiSizeUnit.PIXEL);
	...
	verticalLayout.addItem(userInfoMatrixLayout);
	var defaultTabStrip = genesis.newControl(oFF.UiType.TAB_STRIP);
	defaultTabStrip.setName("defaultTabStrip");
	defaultTabStrip.useMaxWidth();
	defaultTabStrip.setHeightCss("400px");
	...
	this.m_userDialog.setContent(verticalLayout);
	this.m_userDialog.addDialogButton(dlgCloseButton);
};
oFF.SxDlgUserProfile.prototype.openDialog = function()
{
	this.m_userDialog.open();
};
oFF.SxDlgUserProfile.prototype.onPress = function(event)
{
	var name = event.getControl().getName();
	if (oFF.XString.isEqual("closeUserProfileBtn", name))
	{
		this.m_userDialog.close();
	}
};
...
oFF.FeApolloView.prototype.buildView = function(genesis)
{
	...
	this.m_mainLayout.setBackgroundColor(oFF.UiColor.createByString("#f9fafc"));
	if (this.m_directoryManager.getActiveDirectory() === null)
	{
		...
		noActiveFileSystemLbl.setFontSizeCss("19px");
	...
	this.m_hierrarchyTree.setHeightCss("100%");
	this.m_hierrarchyTree.setWidthCss("200px");
	...
	this.m_directoryManager.getRootDirectory().setAssociatedUiControl(rootItem);
	...
	viewSeperator.setWidthCss("1px");
	viewSeperator.setHeightCss("100%");
	viewSeperator.setBackgroundColor(oFF.UiColor.createByString("rgb(204, 204, 204)"));
...
oFF.FeApolloView.prototype.createHeaderToolbar = function(mainLayout)
{
	...
	homePrevSpacer.setWidthCss(oFF.FeApolloView.TOOLBAR_ITEM_SPACING);
	...
	prevNextSpacer.setWidthCss(oFF.FeApolloView.TOOLBAR_ITEM_SPACING);
	...
	nextUpSpacer.setWidthCss(oFF.FeApolloView.TOOLBAR_ITEM_SPACING);
	...
	upSeparatorSpacer.setWidthCss(oFF.FeApolloView.TOOLBAR_ITEM_SPACING);
	var buttonBreadcrumpSectionSeparator = headerToolbarLayout.addNewItemOfType(oFF.UiType.SPACER);
	buttonBreadcrumpSectionSeparator.setWidthCss("1px");
	buttonBreadcrumpSectionSeparator.setHeightCss("60%");
	buttonBreadcrumpSectionSeparator.setBackgroundColor(oFF.UiColor.createByString("#999999"));
	var separatorPathSpacer = headerToolbarLayout.addNewItemOfType(oFF.UiType.SPACER);
	separatorPathSpacer.setWidthCss(oFF.FeApolloView.TOOLBAR_ITEM_SPACING);
	...
	this.m_subHeaderBreadcrumbPathLbl.setWidthCss("100%");
	this.m_subHeaderBreadcrumbPathLbl.setFontSizeCss("15px");
...
{
	this.m_activeHierrarchyTreeItem = dirToRender.getAssociatedUiControl();
...
oFF.FeApolloView.prototype.addNewHierarchyTreeDirectory = function(apolloDir, activeDir)
{
	if (oFF.notNull(apolloDir) && oFF.notNull(activeDir))
	{
		...
		apolloDir.setAssociatedUiControl(newTreeItem);
...
oFF.FeApolloView.prototype.updateActiveDirectoryHierarchyTree = function(apolloItem)
{
	...
	if (oFF.isNull(apolloItem))
	{
		...
		if (lastApolloItem.isDirectory() && lastApolloItem.getAssociatedUiControl() === null)
		{
	...
	{
		if (apolloItem.getAssociatedUiControl() !== null)
		{
			var treeItem = apolloItem.getAssociatedUiControl();
...
oFF.FeApolloView.prototype.openCreateNewItemDialog = function(isFile)
{
	...
	newDirectoryItemDialog.setWidthCss("500px");
	newDirectoryItemDialog.setPaddingCss("20px");
...
oFF.FeApolloView.prototype.openDeleteItemDialog = function(apolloItem)
{
	...
	deleteDirectoryItemDialog.setWidthCss("500px");
	deleteDirectoryItemDialog.setPaddingCss("20px");
...
oFF.FeApolloView.prototype.openRenameItemDialog = function(apolloItem)
{
	...
	renameDirectoryItemDialog.setWidthCss("500px");
	renameDirectoryItemDialog.setPaddingCss("20px");
...
oFF.FeApolloView.prototype.showTileInfo = function(tileItem)
{
	...
	tileInfoPopover.setSizeCss("250px", "250px");
	...
	popoverMainLayout.setPaddingCss("10px");
	...
	popoverTileNameLbl.setFontSizeCss("18px");
	...
	if (isFolder === false && isExecutable === false)
	{
		...
		popoverTypeProgramSpacer.setHeightCss("5px");
		...
		popoverProgramNameLbl.setWidthCss("230px");
...
oFF.FutVulcanUiControlsView.prototype.createView = function(genesis)
{
	...
	systemListWrapper.setHeightCss("100%");
	systemListWrapper.setWidthCss("300px");
	...
	this.m_searchInput.setPaddingCss("5px");
	this.m_searchInput.setHeightCss("40px");
	...
	this.m_controlDetailsContainer.setWidthCss("80%");
...
oFF.FutVulcanUiControlsView.prototype.createOverviewSection = function(uiType)
{
	if (oFF.notNull(this.m_controlDetailsContainer))
	{
		...
		overviewLayout.setHeightCss("160px");
		overviewLayout.setFlex("0 0 160px");
		overviewLayout.setPaddingCss("10px");
		...
		controlNameLbl.setFontSizeCss("20px");
		controlNameLbl.setMarginCss("10px");
		controlNameLbl.setFlex("0 0 23px");
		var titleSpacer = overviewLayout.addNewItemOfType(oFF.UiType.SPACER);
		titleSpacer.setHeightCss("1px");
		titleSpacer.setWidthCss("auto");
...
{
	var controlDetailsTabStrip = this.m_controlDetailsContainer.addNewItemOfType(oFF.UiType.TAB_STRIP);
	controlDetailsTabStrip.setHeightCss("auto");
	controlDetailsTabStrip.setWidthCss("100%");
...
{
	var tmpTabItem = this.m_genesis.newControl(oFF.UiType.TAB_STRIP_ITEM);
	...
	if (oFF.notNull(superType))
	{
		...
		inheritedLbl.setMarginCss("10px");
		inheritedLbl.setFontSizeCss("17px");
...
oFF.FutVulcanUiControlsView.prototype.addUiTypeTabSection = function(tabFlexLayout, uiType, text)
{
	if (oFF.notNull(uiType))
	{
		...
		tmpPanelHeaderLink.setFontSizeCss("16px");
...
oFF.SleMetisFormItem.prototype.setupInternal = function(formLayout, text, inputType, cssWidth, liveChangeListener)
{
	...
	this.m_itemWrapper.setWidthCss(cssWidth);
	this.m_itemWrapper.setFlex(oFF.XStringUtils.concatenate2("1 1 ", cssWidth));
	this.m_itemWrapper.setPaddingCss("10px");
...
oFF.SleMetisFormItem.prototype.setInvalid = function()
{
	...
	this.m_itemInput.setBorderSizeCss("1px");
	this.m_itemInput.setBackgroundColor(oFF.UiColor.create(200, 20, 20, 0.1));
};
oFF.SleMetisFormItem.prototype.setValid = function()
{
	this.m_itemInput.setBorderColor(null);
	this.m_itemInput.setBorderSizeCss(null);
...
oFF.SleMetisImporter.prototype.buildView = function(genesis)
{
	...
	this.m_dialog.setWidthCss("600px");
	this.m_dialog.registerOnAfterOpen(this);
...
oFF.FeApolloItemBase.prototype.m_associatedUiControl = null;
...
oFF.FeApolloItemBase.prototype.setAssociatedUiControl = function(uiControl)
{
	this.m_associatedUiControl = uiControl;
};
oFF.FeApolloItemBase.prototype.getAssociatedUiControl = function()
{
	return this.m_associatedUiControl;
};
...
oFF.FeApolloFileHandler.prototype.handleFileWithApolloExtension = function(apolloFile, apolloExtension)
{
	...
	if (oFF.notNull(openFileExtension))
	{
		if (openFileExtension.isExecutable())
		{
			...
			if (oFF.ProgramRegistration.getAllEntries().containsKey(programName))
			{
				...
				prgTitle = prgManifest.getInitialWindowTitle();
			}
		}
		else
		{
	...
	{
		prgArgs = oFF.ProgramArgs.create();
	}
	if (oFF.notNull(prgManifest))
	{
...
oFF.SuConnectionTestDialog.prototype = new oFF.DfUiDialog();
...
oFF.SuConnectionTestDialog.createNewConnectionTestDialog = function(application, genesis, systemName)
{
	var dialog = new oFF.SuConnectionTestDialog();
	if (oFF.isNull(application))
	{
		throw oFF.XException.createRuntimeException("Cannot create a Metis connection test instance without an application. Please sepcify an application!");
	}
	dialog.setupConnectionTest(application, systemName);
	dialog.setupDialog(genesis);
	return dialog;
};
oFF.SuConnectionTestDialog.prototype.m_application = null;
...
oFF.SuConnectionTestDialog.prototype.setupConnectionTest = function(application, systemName)
{
	this.m_application = application;
	this.m_systemName = systemName;
};
oFF.SuConnectionTestDialog.prototype.setupDialog = function(genesis)
{
	oFF.DfUiDialog.prototype.setupDialog.call( this , genesis);
	this.setDialogName("suConnectionTestDialog");
	this.setDialogTitle(oFF.XStringUtils.concatenate2("Connection test - ", this.m_systemName));
	this.getDialog().setWidthCss("600px");
	this.m_testButton = this.addDialogButton("suConnectionTestDialogTestBtn", oFF.UiButtonType.PRIMARY, "Test", null, this);
	this.m_testButton.setEnabled(false);
	this.addDialogButton("suConnectionTestDialogCloseBtn", oFF.UiButtonType.DEFAULT, "Close", null, this);
	this.prepareTest();
};
oFF.SuConnectionTestDialog.prototype.buildContent = function(genesis)
{
	var mainLayout = genesis.newControl(oFF.UiType.FLEX_LAYOUT);
	mainLayout.setDirection(oFF.UiFlexDirection.COLUMN);
	this.m_statusLbl = mainLayout.addNewItemOfType(oFF.UiType.LABEL);
	this.m_statusLbl.setName("suConnectionTestStatusLbl");
	this.m_statusLbl.setWidthCss("auto");
	this.m_statusLbl.setFontSizeCss("16px");
	this.m_statusLbl.setPaddingCss("10px");
	this.m_statusLbl.setFontColor(oFF.UiColor.WHITE);
	this.m_statusLbl.setWrapping(true);
	this.m_statusLbl.setCornerRadiusCss("5px");
	...
	statusSystemInfoSpacer.setHeightCss("10px");
	...
	this.m_systemInfoText.setWidthCss("auto");
	this.m_systemInfoText.setHeightCss("85px");
	var systemInfoResponseSpacer = this.m_resultLayout.addNewItemOfType(oFF.UiType.SPACER);
	systemInfoResponseSpacer.setHeightCss("10px");
	...
	this.m_serverResponseText.setWidthCss("auto");
	this.m_serverResponseText.setHeightCss("220px");
	var responseMetadataSpacer = this.m_resultLayout.addNewItemOfType(oFF.UiType.SPACER);
	responseMetadataSpacer.setHeightCss("10px");
	...
	this.m_serverMetadataText.setWidthCss("auto");
	this.m_serverMetadataText.setHeightCss("260px");
	return mainLayout;
};
oFF.SuConnectionTestDialog.prototype.startTest = function()
{
	...
	{
		var connectionPool = this.m_application.getConnectionPool();
		var connContainer = connectionPool.getConnection(this.m_systemName);
		if (oFF.notNull(connContainer))
		{
...
{
	this.setStatusLabel(text, oFF.UiColor.createByString(oFF.SuConnectionTestDialog.GREY_COLOR));
};
oFF.SuConnectionTestDialog.prototype.setStatusLabelSuccess = function(text)
{
	this.setStatusLabel(text, oFF.UiColor.createByString(oFF.SuConnectionTestDialog.GREEN_COLOR));
};
oFF.SuConnectionTestDialog.prototype.setStatusLabelError = function(text)
{
	this.setStatusLabel(text, oFF.UiColor.createByString(oFF.SuConnectionTestDialog.RED_COLOR));
};
oFF.SuConnectionTestDialog.prototype.setStatusLabelWarning = function(text)
{
	this.setStatusLabel(text, oFF.UiColor.createByString(oFF.SuConnectionTestDialog.ORANGE_COLOR));
};
...
oFF.SuConnectionTestDialog.prototype.onAfterOpen = function(event)
{
	this.prepareTest();
	this.startTest();
};
oFF.SuConnectionTestDialog.prototype.onAfterClose = function(event)
{
	this.testFinished();
};
...
oFF.SuCalendarDialog.prototype.getDefaultProgramName = function()
{
...
oFF.SuCalendarDialog.prototype.getDefaultWindowSize = function()
{
	return oFF.UiSizeValue.createByCss("330px", "220px");
};
...
oFF.DialogTestProgram.prototype.getDefaultProgramName = function()
{
...
oFF.DialogTestProgram.prototype.buildUi = function(genesis)
{
	...
	tmpLabel.setWidthCss("100%");
	tmpLabel.setHeightCss("50px");
	tmpLabel.setPaddingCss("10px");
...
oFF.SuJavadocDialog.prototype.getDefaultProgramName = function()
{
...
oFF.SuJavadocDialog.prototype.getDefaultWindowSize = function()
{
	return oFF.UiSizeValue.createByCss("90vw", "80vh");
};
...
oFF.SuUserProfileDialog.prototype.getDefaultProgramName = function()
{
...
oFF.SuUserProfileDialog.prototype.getDefaultWindowSize = function()
{
	return oFF.UiSizeValue.createByCss("60%", "60vh");
};
oFF.SuUserProfileDialog.prototype.setupInternal = function()
{
	...
	{
		this.setTitle(oFF.XStringUtils.concatenate2("User profile for ", this.m_userProfile.getPersonFullName()));
	}
	else
	{
...
oFF.SuUserProfileDialog.prototype.buildUi = function(genesis)
{
	...
	{
		var matrixRow1 = userInfoMatrixLayout.addNewMatrixLayoutRow();
		matrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.IMAGE).setHeightCss("80px").setWidthCss("80px").setCornerRadiusCss("50%").setSrc(this.m_userProfile.getThumbnailPhoto());
		matrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(this.m_userProfile.getPersonFullName());
		matrixRow1.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("");
		var matrixRow2 = userInfoMatrixLayout.addNewMatrixLayoutRow();
		matrixRow2.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("");
		matrixRow2.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("First Name");
		...
		var addressPersonMatrixRow5 = addrPersonMatrixLayout.addNewMatrixLayoutRow();
		addressPersonMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("Full Name:");
		addressPersonMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(this.m_userProfile.getPersonFullName());
		var addressPersonMatrixRow6 = addrPersonMatrixLayout.addNewMatrixLayoutRow();
		addressPersonMatrixRow6.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("Language:");
		...
		var addressWorkCenterMatrixRow4 = addrWorkCenterMatrixLayout.addNewMatrixLayoutRow();
		addressWorkCenterMatrixRow4.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("Floor:");
		addressWorkCenterMatrixRow4.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(oFF.XString.substring(this.m_userProfile.getRoomNumber(), oFF.XString.lastIndexOf(this.m_userProfile.getRoomNumber(), ",") + 1, oFF.XString.lastIndexOf(this.m_userProfile.getRoomNumber(), ".")));
		var addressWorkCenterMatrixRow5 = addrWorkCenterMatrixLayout.addNewMatrixLayoutRow();
		addressWorkCenterMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("Building Code:");
		addressWorkCenterMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(oFF.XString.substring(this.m_userProfile.getRoomNumber(), 0, oFF.XString.lastIndexOf(this.m_userProfile.getRoomNumber(), ",")));
		verticalAddressLayout.addItem(addrWorkCenterMatrixLayout);
		var headerCommuncationLabel = genesis.newControl(oFF.UiType.LABEL);
		headerCommuncationLabel.setName("Communication");
		headerCommuncationLabel.setText("Communication");
		...
		var defaultMatrixRow5 = defaultMatrixLayout.addNewMatrixLayoutRow();
		defaultMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("Number Formatting:");
		defaultMatrixRow5.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(this.m_userProfile.getNumberFormatting());
		var defaultMatrixRow6 = defaultMatrixLayout.addNewMatrixLayoutRow();
		defaultMatrixRow6.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("Scale Formatting:");
		...
		var defaultMatrixRow8 = defaultMatrixLayout.addNewMatrixLayoutRow();
		defaultMatrixRow8.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("Default Application:");
		defaultMatrixRow8.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText(this.m_userProfile.getDefaultApplication());
		var defaultMatrixRow9 = defaultMatrixLayout.addNewMatrixLayoutRow();
		defaultMatrixRow9.addNewMatrixLayoutCell().setNewContent(oFF.UiType.LABEL).setText("Clean up notification:");
	...
	userInfoHeaderWrapper.setPadding(oFF.UiUnitValue.createByCss("10px"));
...
oFF.Chronos.prototype.buildUi = function(genesis)
{
	...
	this.m_buttonMonth.setWidthCss("100px");
	...
	this.m_buttonUi5.setWidthCss("100px");
...
oFF.Chronos.prototype.openMonthCalendarDialog = function()
{
	...
	calendarDlgStartCfg.setIsPersistentProgram(true);
...
oFF.CoronaInfo.prototype.getDefaultWindowSize = function()
{
...
oFF.CoronaInfo.prototype.getDefaultProgramName = function()
{
...
oFF.CoronaInfo.prototype.getCoronaData = function(region)
{
	...
	loadingLbl.setFontSizeCss("18px");
	loadingLbl.setMarginCss("20px");
...
oFF.CoronaInfo.prototype.displayCoronaData = function()
{
	...
	if (oFF.notNull(coronaDataStructure))
	{
		...
		else
		{
			...
			coronaInfoLayout.setWidthCss("100%");
			coronaInfoLayout.setHeightCss("45%");
			...
			coronaWeekSpacer.setHeightCss("1px");
			coronaWeekSpacer.setWidthCss("100%");
			...
			weekInfoLayout.setWidthCss("100%");
			weekInfoLayout.setHeightCss("45%");
			...
			weekDataSpacer.setHeightCss("1px");
			weekDataSpacer.setWidthCss("100%");
			...
			metaInfoLayout.setWidthCss("100%");
			metaInfoLayout.setHeightCss("10%");
...
oFF.CoronaInfo.prototype.insertInfoNumber = function(layout, label, total, today, titleSizeCss, totalSizeCss, todaySizeCss)
{
	...
	titleLbl.setFontSizeCss(titleSizeCss);
	...
	totalLbl.setFontSizeCss(totalSizeCss);
	if (today > 0)
	{
		...
		todayLbl.setFontSizeCss(todaySizeCss);
...
oFF.CoronaInfo.prototype.insertInfoText = function(layout, label, text, titleSizeCss, textSizeCss)
{
	...
	textLbl.setFontSizeCss(textSizeCss);
};
...
oFF.WasmDoom1.prototype.getDefaultWindowSize = function()
{
	return oFF.UiSizeValue.createByCss("60vw", "60vh");
};
oFF.WasmDoom1.prototype.getDefaultProgramName = function()
{
...
oFF.FeApollo.prototype.getDefaultProgramName = function()
{
...
oFF.FutVulcan.prototype.getDefaultWindowSize = function()
{
	return oFF.UiSizeValue.createByCss("60vw", "60vh");
};
oFF.FutVulcan.prototype.getDefaultProgramName = function()
{
...
oFF.FutVulcan.prototype.buildUi = function(genesis)
{
	...
	homePageLayout.setBackgroundColor(oFF.UiColor.createByString(oFF.FutVulcan.BG_COLOR));
...
{
	var uiControlsCanvasBtn = this.createCustomTileButton("futVulcanUiControlsBtn", "Ui Controls", "background", oFF.UiColor.createByString("#FA7445"), oFF.UiColor.createByString("#EB5121"));
...
oFF.FutVulcan.prototype.createCustomTileButton = function(name, text, icon, bgColor, borderColor)
{
	...
	wrapperCanvasLayout.setSizeCss("200px", "200px");
	wrapperCanvasLayout.setBackgroundColor(bgColor);
	if (oFF.notNull(borderColor))
	{
		wrapperCanvasLayout.setBorderColor(borderColor);
		wrapperCanvasLayout.setBorderSizeCss("1px");
	}
	var canvasIcon = wrapperCanvasLayout.addNewItemOfType(oFF.UiType.ICON);
	canvasIcon.setIcon(icon);
	canvasIcon.setSizeCss("100%", "100px");
	canvasIcon.setPositionCss("0px", "30px");
	canvasIcon.setFontColor(oFF.UiColor.createByString(oFF.FutVulcan.TILE_TEXT_COLOR));
	canvasIcon.setEnabled(false);
	var canvasLbl = wrapperCanvasLayout.addNewItemOfType(oFF.UiType.LABEL);
	canvasLbl.setText(text);
	canvasLbl.setSizeCss("100%", "50px");
	canvasLbl.setPositionCss("0px", "150px");
	canvasLbl.setWrapping(true);
	canvasLbl.setTextAlign(oFF.UiTextAlign.CENTER);
	canvasLbl.setFontSizeCss("18px");
	canvasLbl.setFontColor(oFF.UiColor.createByString(oFF.FutVulcan.TILE_TEXT_COLOR));
	...
	canvasTransparentButton.setBackgroundColor(oFF.UiColor.createByString("rgba(0,0,0,0)"));
	canvasTransparentButton.setSizeCss("200px", "200px");
	canvasTransparentButton.setPaddingCss("0px");
	canvasTransparentButton.setPositionCss("0px", "0px");
...
oFF.SleMetis.prototype.getDefaultWindowSize = function()
{
	return oFF.UiSizeValue.createByCss("60vw", "60vh");
};
oFF.SleMetis.prototype.getDefaultProgramName = function()
{
...
oFF.SleMetis.prototype.buildUi = function(genesis)
{
	...
	this.m_formToolbar.setHeightCss("40px");
	...
	toolbarLayout.setBackgroundColor(oFF.UiColor.createByString("rgba(128, 128, 128, 0.15)"));
	...
	toolbarLayout.addNewItemOfType(oFF.UiType.SPACER).setWidthCss("10px");
...
oFF.SleMetis.prototype.addNewFormElement = function(text, inputType, cssWidth, breakLine)
{
	...
	if (breakLine === true)
	{
		...
		breakLineSpacer.setWidthCss("100%");
		breakLineSpacer.setHeightCss("0");
...
oFF.SleMetis.prototype.performConnectionTest = function()
{
	...
	else
	{
		...
		var metisConnectionTest = oFF.SuConnectionTestDialog.createNewConnectionTestDialog(this.getApplication(), this.m_genesis, systemDesc.getName());
		metisConnectionTest.open();
	}
};
...
oFF.TeAthena.createNewAthena = function()
{
	var prg = new oFF.TeAthena();
	prg.setup();
	return prg;
};
oFF.TeAthena.createWithFile = function(application, file)
{
	var client = oFF.TeAthena.create(application);
	client.setFile(file);
	client.setValue(null);
	return client;
};
oFF.TeAthena.createWithValue = function(application, value)
{
	var client = oFF.TeAthena.create(application);
	client.setFile(null);
...
oFF.TeAthena.create = function(application)
{
	var client = new oFF.TeAthena();
	client.setup();
	client.setContentType("text");
	client.setApplication(application);
	client.setInitialSystemType(null);
...
oFF.TeAthena.prototype.m_isForceClose = false;
...
oFF.TeAthena.prototype.getDefaultWindowSize = function()
{
	return oFF.UiSizeValue.createByCss("60%", "60%");
};
...
oFF.TeAthena.prototype.getDefaultProgramName = function()
{
...
oFF.TeAthena.prototype.buildUi = function(genesis)
{
	...
	else if (oFF.notNull(this.m_initialValue))
	{
		this.m_codeEditor.setText(this.m_initialValue);
	}
	this.addMenuBarButton("fileToolbarBtn", null, "File", null, this);
	this.addMenuBarButton("editToolbarBtn", null, "Edit", null, this);
};
oFF.TeAthena.prototype.openFileByPath = function(filePath)
{
...
oFF.TeAthena.prototype.openFile = function(file)
{
	if (oFF.isNull(file) || !file.isExisting())
	{
...
oFF.TeAthena.prototype.setValue = function(newString)
{
	this.m_initialValue = newString;
};
...
oFF.TeAthena.prototype.saveContentToCurrentFile = function()
{
	if (oFF.notNull(this.m_file))
	{
		...
		var fileContentToSave = oFF.XByteArray.convertFromString(editorContent);
		this.m_file.saveByteArray(fileContentToSave);
		if (!this.m_file.isValid())
		{
			this.getGenesis().showErrorToast(oFF.XStringUtils.concatenate2("Cannot save: ", this.m_file.getSummary()));
		}
		else
		{
...
oFF.TeAthena.prototype.showUnsavedChangesDialog = function()
{
	var modifiedDialog = oFF.UiConfirmPopup.createNewConfirmDialog(this.getGenesis(), "Unsaved changes!", "You have unsaved changes! Are you sure that you want to quit?");
	modifiedDialog.addButton("modifiedDialogQuitBtn", oFF.UiButtonType.DESTRUCTIVE, "Quit without saving", "delete", this);
	modifiedDialog.open();
};
oFF.TeAthena.prototype.createFileToolbarMenu = function(fileBtn)
{
	var fileToolbarMenu = this.m_genesis.newControl(oFF.UiType.MENU);
	fileToolbarMenu.setName("fileToolbarMenu");
	fileToolbarMenu.addNewItem().setName("athenaToolbarMenuSave").setText("Save").setIcon("save").registerOnPress(this);
	fileToolbarMenu.openAt(fileBtn);
};
oFF.TeAthena.prototype.createEditToolbarMenu = function(editBtn)
{
	var fileToolbarMenu = this.m_genesis.newControl(oFF.UiType.MENU);
	fileToolbarMenu.setName("editToolbarMenu");
	fileToolbarMenu.addNewItem().setName("athenaToolbarMenuDiscardChanges").setText("Discard changes").setIcon("eraser").registerOnPress(this);
	fileToolbarMenu.openAt(editBtn);
};
...
oFF.TeAthena.prototype.onPress = function(event)
{
	...
	if (oFF.notNull(controlParent) && controlParent.getUiType() === oFF.UiType.MENU && oFF.XString.isEqual(controlParent.getName(), "fileToolbarMenu"))
	{
		switch (control.getName())
		{
			case "athenaToolbarMenuSave":
	...
	if (oFF.notNull(controlParent) && controlParent.getUiType() === oFF.UiType.MENU && oFF.XString.isEqual(controlParent.getName(), "editToolbarMenu"))
	{
		switch (control.getName())
		{
			case "athenaToolbarMenuDiscardChanges":
	...
	if (oFF.notNull(controlParent) && controlParent.getUiType() === oFF.UiType.DIALOG)
	{
		var tmpDialog = controlParent;
		switch (control.getName())
		{
			case "modifiedDialogQuitBtn":
				this.m_isForceClose = true;
				this.exitNow(0);
				break;

			case "modifiedDialogCancelBtn":
				controlParent.close();
				break;
...
{
	if (this.isModified() && !this.m_isForceClose)
	{
