sap.ui.define([
	...
	isEmptyObject, FeError, placeholderHelper, Device, semanticDateRangeTypeHelper, controlStateWrapperFactory, testableHelper) {
	"use strict";
	...
	function getMethods(oState, oController, oTemplateUtils) {
		...
		// Create a wrapper around the dynamic page
		// Use this wrapper to store/restore the pinned status of the header.
		var oDynamicPage = oController.byId("page");
		var oDynamicPageWrapper = controlStateWrapperFactory.getControlStateWrapper(oDynamicPage);
		oDynamicPageWrapper.attachStateChanged(function(){
			changeIappState(areDataShownInTable());
		});
		var oSmartTableWrapper;
		// In case of multipleViews with multiple tables (mode "multi"), each tab has an own table or chart, which has an own state, so 
		// multipleViewsHandler has to take care of storing/restoring their states (in its part of the appState stored in genericData.tableTabData)
		// In case of no multipleViews or multipleViews with single table, there is only one table, so its state is handled here and stored in
		// controlStates property of appState
		if (oState.oMultipleViewsHandler.getMode() !== "multi" && oState.oSmartTable) {
			oSmartTableWrapper = controlStateWrapperFactory.getControlStateWrapper(oState.oSmartTable);
			...
			//bDataShown should be the current value of the property
		...
		function getCurrentAppState() {
			...
			var oPageVariantId = oController.byId('template::PageVariant');
			if (oPageVariantId && oPageVariantId.currentVariantGetModified() && oSelectionVariant.getID()) {
		...
		*/
		...
		function fnAdaptToAppStateNavigation(oAppData, oURLParameters){
			...
			if (!oState.oWorklistData.bWorkListEnabled){
				...
				setPlaceHolderValue(bShouldDataBeLoaded);
			}
		...
		function fnAdaptToAppStateStartUpInitial(oURLParameters){
			...
			if (oState.oWorklistData.bWorkListEnabled) {
				...
				if (oState.oSmartFilterbar.isCurrentVariantStandard()) {
					...
					oState.oSmartFilterbar.getSmartVariant().setExecuteOnStandard(getInitialLoadBehaviourSettings(false));
					
					var bApplyAutomaticallyValueInSVM = oState.oSmartFilterbar.getSmartVariant().getExecuteOnStandard(); // holds the actual apply automatically checkbox value set in the app
		...
		function fnAdaptOtherControlsToAppState(oControlsStates) {
			// only one "other" control exists (so far), namely the SmartTable. 
			// in case of multipleViews with multiple tables, no oSmartTableWrapper exists (instead, multipleViewsHandler is caring for the states of SmartTables/SmartCharts)
			if (oSmartTableWrapper){
...
//			}
		...
		function onBeforeSFBVariantFetch(){
			...
			oCurrentAppState.customData['sap.suite.ui.generic.template.genericData'].variantDirty = false;			
		...
		*/
		...
		/* eslint-disable */
		getInitialLoadBehaviourSettings = testableHelper.testableStatic(getInitialLoadBehaviourSettings, "IappStateHandler_getInitialLoadBehaviourSettings");
		/* eslint-disable */
