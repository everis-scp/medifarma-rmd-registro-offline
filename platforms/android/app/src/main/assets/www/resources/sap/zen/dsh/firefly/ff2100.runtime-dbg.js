...
oFF.ApplicationFactory = {
	...
	createApplicationForDragonfly:function(listener)
	{
		...
		env.setVariable(oFF.XEnvironmentConstants.FIREFLY_FEATURE_TOGGLES, "+Olap.AutoVarSubmit");
		return oFF.ApplicationFactory.createApplicationExt2(oFF.SyncType.NON_BLOCKING, listener, null, null, null, -1, null, null, null, null, oFF.ApplicationSystemOption.PATH, true, oFF.DragonflyAppProgram.DEFAULT_PROGRAM_NAME);
	},
...
oFF.BatchRequestManager.prototype.executeBatch = function(syncType, functionFactory, batchModePath, enableRsStreaming)
{
	...
	var request = _function.getRequest();
...
oFF.BatchRequestManager.prototype.removeExecutedFunctions = function()
{
	...
	{
		var rpcBatchFunction = this.m_batchFunctions.get(i);
		var requestStructure = rpcBatchFunction.getRequest().getRequestStructure();
...
oFF.BatchRequestManager.prototype.addQueriesToRequestList = function(requestList)
{
	...
	{
		var rpcBatchFunction = this.m_batchFunctions.get(i);
		var request = rpcBatchFunction.getRequest();
...
oFF.BatchRequestManager.prototype.updateAllBatchFunctions = function(batchList, rootElement)
{
	...
	if (oFF.notNull(batchList))
	{
		...
		for (var k = 0; k < batchFunctionSize; k++)
		{
			...
			var batchFunctionResponse = batchFunction.getResponse();
	...
	else
	{
		...
		{
			this.m_batchFunctions.get(i).getResponse().setRootElement(rootElement, null);
		}
	}
};
...
oFF.BatchRequestManager.prototype.getDataSource = function(structure)
{
	...
	{
		var analytics = structure.getStructureByKey(oFF.ConnectionConstants.INA_ANALYTICS);
		return analytics.getStructureByKey(oFF.ConnectionConstants.INA_DATA_SOURCE);
	}
	if (structure.containsKey(oFF.ConnectionConstants.INA_METADATA))
	{
		return structure.getStructureByKey(oFF.ConnectionConstants.INA_METADATA).getStructureByKey(oFF.ConnectionConstants.INA_DATA_SOURCE);
	}
	if (structure.containsKey(oFF.ConnectionConstants.INA_CUBE))
	{
		return structure.getStructureByKey(oFF.ConnectionConstants.INA_CUBE).getStructureByKey(oFF.ConnectionConstants.INA_DATA_SOURCE);
	}
	if (structure.containsKey(oFF.ConnectionConstants.INA_DS_VALIDATION))
	{
		return structure.getStructureByKey(oFF.ConnectionConstants.INA_DS_VALIDATION).getStructureByKey(oFF.ConnectionConstants.INA_DATA_SOURCE);
	}
	return structure.getStructureByKey(oFF.ConnectionConstants.INA_DATA_SOURCE);
};
oFF.BatchRequestManager.prototype.ensureUniqueInstanceIds = function()
{
	...
	for (var i = 0; i < size; i++)
	{
		...
		var requestStructure = batchFunction.getRequest().getRequestStructure();
...
oFF.DfApplication.prototype.releaseServices = function()
{
	if (oFF.notNull(this.m_serviceRegistry))
	{
		...
		for (var idxKey = 0; idxKey < keys.size(); idxKey++)
		{
			...
			{
				for (var idxService = 0; idxService < services.size(); idxService++)
				{
...
oFF.SubApplication.prototype.setupSubApplication = function(parentApplication, process)
{
	...
	if (oFF.isNull(subSession))
	{
		...
		subSession = parentProcess.newChildProcess();
	}
...
oFF.ApplicationInitializeAction.prototype.processSynchronization = function(syncType)
{
	...
	{
		var kernelBooter = oFF.KernelBoot.createByName(oFF.StandardAppProgram.DEFAULT_PROGRAM_NAME);
...
oFF.DfServiceConfig.prototype.processSynchronization = function(syncType)
{
	...
	if (this.isSystemBoundService())
	{
		...
		if (oFF.isNull(this.m_connectionContainer) || this.getConnectionContainer().isDirty())
		{
...
{
	if (oFF.notNull(this.m_connectionContainer) && this.getConnectionContainer().isDirty())
	{
		this.m_connectionContainer = null;
		this.clearMessages();
		this.resetSyncState();
