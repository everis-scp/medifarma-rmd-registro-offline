...
sap.ui.define([
	...
	Relationship.prototype.calcSRlsPathD = function (x1, y1, x2, y2, nRowHeight, vType,edgePoint) {
		...
		var y3;
		//Alignment of Relationship lines based on AlignShape
		switch (this.mRelatedShapes.predecessor.getAlignShape()) {
			case ShapeAlignment.Top: if (y1 < y2) { //Predecessor is on the top
										//If Predecessor is top Aligned create path on the row edge in between the shapes
										y3 = y1 + nRowHeight - this.mRelatedShapes.predecessor.getDomRef().getBBox().height / 2 - 1;
									} else { //Predecessor is at the bottom
										//If Predecessor is top Aligned create path on the row edge in between the shapes
										y3 = y1 - this.mRelatedShapes.predecessor.getDomRef().getBBox().height / 2 - 1;
									}
									break;
			case ShapeAlignment.Bottom: if (y1 < y2) {//Predecessor is on the top
											//If Predecessor is Bottom Aligned create path on the row edge in between the shapes
											y3 = y1 + this.mRelatedShapes.predecessor.getDomRef().getBBox().height / 2 + 1;
										} else { //Predecessor is at the bottom
											//If Predecessor is Bottom Aligned create path on the row edge in between the shapes
											y3 = y1 - nRowHeight + this.mRelatedShapes.predecessor.getDomRef().getBBox().height / 2 + 1;
										...
										break;
			case ShapeAlignment.Middle: //Default will have the same behavior as Middle
							   default: if (y1 < y2) {//Predecessor is on the top
												//If Predecessor is Top Aligned create path on the row edge in between the shapes
												y3 = y1 + nRowHeight / 2;
											} else { //Predecessor is at the bottom
												//If Predecessor is Bottom Aligned create path on the row edge in between the shapes
		...
		if (this.mRelatedShapes.predecessor.getDomRef("nonLabelGroup")) {
			y3 = y3 - this.mRelatedShapes.predecessor.getDomRef().getBBox().height / 4;
		}
