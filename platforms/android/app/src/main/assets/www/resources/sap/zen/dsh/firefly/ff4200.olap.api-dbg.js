...
oFF.QFactory = {
	...
	createRsDefStructureMemberProperties:function(placeholderAliasMappings, minimumDrillStateMap, unsatisfiedRequiredDimensionNames)
	{
			return oFF.QFactory.s_factory.newRsDefStructureMemberProperties(placeholderAliasMappings, minimumDrillStateMap, unsatisfiedRequiredDimensionNames);
	}
};
...
oFF.QVariableUtils.hasSingleValueFilterOnDimension = function(variables, dimensionName)
{
	...
	for (var i = 0; i < dimensionMemberVariables.size(); i++)
	{
		...
		if (variable.getDimension() !== null && oFF.XString.isEqual(variable.getDimension().getName(), dimensionName))
		{
			var memberNames = variable.getMemberNamesFromFilter(dimensionName);
...
oFF.ReturnedDataSelection.lookupOrCreate = function(name)
{
	...
	var dataSelection = oFF.ReturnedDataSelection.lookup(name);
	if (oFF.isNull(dataSelection))
	{
		dataSelection = oFF.ReturnedDataSelection.createReturnedDataSelectionType(name);
	}
...
oFF.PresentationType.staticSetupPresentation = function()
{
	...
	oFF.PresentationType.DOCUMENT_LINK = oFF.PresentationType.createPresentation("DocumentLink", null, 80);
	oFF.PresentationType.BUSINESS_OBJECT_NODE_IDENTIFIER = oFF.PresentationType.createPresentation("BusinessObjectNodeIdentifier", null, 75);
...
oFF.VariableVariantType.prototype._ff_c = "VariableVariantType";
...
oFF.VariableVariantType.staticSetup = function()
{
	...
	oFF.VariableVariantType.AO_VARIABLE_DEFAULTS = oFF.VariableVariantType.create("analysisOfficeVariableDefaults");
};
oFF.VariableVariantType.create = function(name)
{
	var newVariant = oFF.XConstant.setupName(new oFF.VariableVariantType(), name);
