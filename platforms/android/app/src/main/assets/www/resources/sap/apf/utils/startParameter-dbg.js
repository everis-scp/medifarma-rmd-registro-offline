	...
	var StartParameter = function(component, manifests) {
		...
		this.getAnalyticalConfigurationId = function() {
			...
			if (!analyticalConfigurationId) {
				...
				analyticalConfigurationId = uriParams.get('sap-apf-configuration-id');
			}
			if (analyticalConfigurationId) {
				config = {};
		...
		this.getSteps = function() {
			var uriParams = new UriParameters(window.location.href);
			if (!stepId) {
				stepId = uriParams.get('sap-apf-step-id');
			}
			if (!representationId) {
				representationId = uriParams.get('sap-apf-representation-id');
			}
			if (!stepId) {
				return null;
			}
		...
		this.getParameter = function(parameterName) {
			...
			var uriParams = new UriParameters(window.location.href);
			return uriParams.get(parameterName);
		};
	};
	/*BEGIN_COMPATIBILITY*/
