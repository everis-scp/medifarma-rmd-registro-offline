		...
		function getMethods(oController, oComponentUtils, oServices, oCommonUtils, oBusyHelper) {
			...
			* @return Bool
			...
			function callActionImpl(mParameters, oState, fnResolve, fnReject){
				...
				if (aCurrentContexts.length === 0 && oSourceControl) {
					var oModelForContext = oSourceControl.getModel();
					var sPath = "/";
					var sEntitySetForContext = oSourceControl.getEntitySet ? oSourceControl.getEntitySet() : oSourceControl.getParent().getEntitySet();
				...
				var fnInvokeAction = function (bIsStrict, bWithWarningDialog, aContexts){
					...
					var fnDone = function(aResponses){
						...
						if (bSuccess){
							...
							var oTableBindingInfo = oCommonUtils.getTableOrListBindingInfo(oSourceControl);
			...
			function addEntry(oTable, oPredefinedValues) {
				...
				if (oViewContext) {
					// Detail screen
					sTableBindingPath = oCommonUtils.getTableOrListBindingInfo(oTable).path;
