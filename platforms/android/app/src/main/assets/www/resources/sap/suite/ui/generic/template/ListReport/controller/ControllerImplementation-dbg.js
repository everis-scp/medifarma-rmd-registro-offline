...
sap.ui.define([
	...
	"sap/suite/ui/generic/template/ListReport/controller/SmartTableOrListHandler",
	"sap/base/util/isEmptyObject"
				...
				SmartTableOrListHandler, isEmptyObject) {
	"use strict";
	...
	var oMethods = {
		getMethods: function(oViewProxy, oTemplateUtils, oController) {
			...
			function addEntryImpl(oPredefinedValues, oButton) {
				if (!oButton.data("CrossNavigation")){
					...
					oTemplateUtils.oCommonUtils.fnProcessDataLossOrDraftDiscardConfirmation(function() {
					    oTemplateUtils.oCommonEventHandlers.addEntry(oButton, false, oState.oSmartFilterbar, oPredefinedValues);
						}, Function.prototype, oState, (oTemplateUtils.oComponentUtils.isDraftEnabled() && "NavigateExternal") || undefined);
				}	 
			}
			...
			function addEntryWithFilters(oEvent) {
				...
				if (sIdForCreationDialog && oController.byId(sIdForCreationDialog)) {
					var sIdForCreateButton = oEvent.getSource().getId();
					oController.byId(sIdForCreationDialog).setVisible(true);
					oTemplateUtils.oCommonUtils.executeIfControlReady(oState.oCreateWithDialogHandler.createWithDialog.bind(null, oController.byId(sIdForCreationDialog)), sIdForCreateButton, oPredefinedValues);
				} else {
			...
			function onShareListReportActionButtonPressImpl(oButton) {
				var oFragmentController = {
		            ...
		            getDownloadUrl: function() {
						var oBinding = oState.oSmartTableOrListHandler.getBinding(oState);
			...
			return {
				onInit: function() {
					...
					oState.oSmartTableOrListHandler = new SmartTableOrListHandler(oState, oController, oTemplateUtils);
					...
					if (oState.oWorklistData.bWorkListEnabled && oState.oSmartTable) {
					...
					oViewProxy.refreshBinding = function(bUnconditional, mEntitySets) {
						if (oState.oIappStateHandler.areDataShownInTable()) { // only if data are currently shown a refresh needs to be triggered
							...
							if (bUnconditional || mEntitySets[oState.oSmartTableOrListHandler.getEntitySet()]) {
