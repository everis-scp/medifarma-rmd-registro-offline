	...
	function(
		...
		var Extension = ControllerExtension.extend("sap.fe.core.controllerextensions.EditFlow", {
							...
							function() {
								if (bIsDraft) {
									...
									if (aSemanticKeys && aSemanticKeys.length) {
										var oCurrentSematicMapping = that.base
												.getAppComponent()
												.getRoutingService()
												.getLastSemanticMapping(),
											sCurrentSematicPath = oCurrentSematicMapping && oCurrentSematicMapping.semanticPath;
										var sChangedPath = SemanticKeyHelper.getSemanticPath(oBindingContext);
										if (sCurrentSematicPath !== sChangedPath) {
			...
			toggleDraftActive: function(oContext) {
				...
				var that = this;
				var bEditable, sEditMode, bUseSemanticKey;
				...
				oToggleContext
					...
					.then(function(sCanonicalPath) {
						if (oToggleContext.getPath() !== sCanonicalPath) {
							sCanonicalPath = bUseSemanticKey ? SemanticKeyHelper.getSemanticPath(oContext) : sCanonicalPath;
							oToggleContext = oToggleContext
								.getModel()
					...
					.then(function(oToggleContext) {
						that._setEditMode(sEditMode, false); //switch to edit mode only if a draft is available
