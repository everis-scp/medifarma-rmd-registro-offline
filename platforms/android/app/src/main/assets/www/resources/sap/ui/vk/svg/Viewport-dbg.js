...
sap.ui.define([
	...
	*/
	...
	Viewport.prototype._updateRasterImage = function() {
		if (this._svgElement == null || !this.raster.useRasterMode) {
			return;
		}
			...
			nodes.forEach(function(node) {
				if (node.opacity !== undefined && node.domRef) {
					if (value) {
						...
						node.setTintColor(node.sMask, "rgba(255, 255, 0, .35)");
						node.setOpacity(1);
	...
	Viewport.prototype._getStyleId = function(element) {
		if (element.highlightColor || element.tintColor) {
			return null;
		}
		var key = element._getStyleKey();
		var style = this._styles.get(key);
		if (!style) {
			var attibutes = [];
			element._setStyleAttributes(function(name, value) { attibutes.push(name, value); });
			style = {
				id: "s" + this._styles.size,
				attibutes: attibutes
			};
