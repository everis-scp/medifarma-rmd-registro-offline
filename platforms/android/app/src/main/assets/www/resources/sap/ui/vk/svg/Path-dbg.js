...
sap.ui.define([
	...
	Path.prototype._getParametricShape = function(fillStyles, lineStyles, textStyles) {
		...
		if (this.segments.length === 1 && this.segments[ 0 ].type === "arc" && this.segments[ 0 ].points === undefined) {// this is a parametric arc converted to a path
			var arc = this.segments[ 0 ];
			if (arc.rx === arc.ry) {
				parametric.type = "arc";
				parametric.radius = arc.rx;
			} else {
				parametric.type = "ellipticalArc";
				parametric.major = arc.rx;
				parametric.minor = arc.ry;
			}
			parametric.start = arc.start;
			parametric.end = arc.end;
		} else {
