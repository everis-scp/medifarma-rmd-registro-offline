...
sap.ui.define(
	...
	function(Button, Dialog, mLibrary, MessagePopover, CommonUtils, MessageType, Filter, Sorter, FilterOperator, ObjectPageLayout, Log) {
		...
		function _activeTitlePress(oEvent) {
			...
			var _focusOnMessageTargetControl = function(oMessage, oTargetMdcTable, iRowIndex, oMessageButton) {
				...
				if (aNotTableErroneousControls.length > 0) {
					...
					if (aTargetMdcTableRow.length > 0 && aTargetMdcTableRow[0]) {
						...
						var oTargetCell = _getTargetCell(oTargetMdcTable, oMessageButton, oTargetRow);
						if (oTargetCell) {
			...
			function _getTargetCell(oTargetMdcTable, oMessageButton, oTargetRow) {
				var oRefErrorContext = oMessageButton._getTableRefErrorContext(oTargetMdcTable);
				var sTargetColPropertyName = oRefErrorContext.getProperty(
					oItem
						.getBindingContext("message")
						.getObject()
						.getId()
				)["targetColProperty"];
				var iTargetColNumber = oTargetMdcTable.getColumns().findIndex(function(element) {
					return element.getDataProperty() === sTargetColPropertyName;
				});
				return oTargetRow.getCells()[iTargetColNumber];
			}
							...
							.then(
								function() {
									...
									var _setFocusOnTargetField = function(oTargetMdcTable, iRowIndex, oMessageButton) {
										...
										if (aTargetMdcTableRow.length > 0 && aTargetMdcTableRow[0]) {
											var iFirstVisibleRow = aTargetMdcTableRow[0].getParent().getFirstVisibleRow(),
												oTargetRow = aTargetMdcTableRow[iRowIndex - iFirstVisibleRow],
												oTargetCell = _getTargetCell(oTargetMdcTable, oMessageButton, oTargetRow);
											if (oTargetCell) {
												oTargetField = oTargetCell.isA("sap.fe.macros.FieldAPI")
													? oTargetCell.getContent().getContentEdit()[0]
													: oTargetCell
															.getItems()[0]
															.getContent()
		...
		function _getTableRow(oElement) {
			var oParentElement = oElement.getParent();
			while (oParentElement && !oParentElement.isA("sap.ui.table.Row") && !oParentElement.isA("sap.m.ColumnListItem")) {
				oParentElement = oParentElement.getParent();
			}
			var oTableRow =
				oParentElement && (oParentElement.isA("sap.ui.table.Row") || oParentElement.isA("sap.m.ColumnListItem"))
					? oParentElement
					: undefined;
			return oTableRow;
		}
		...
		function _getTableColumnIndex(oElement) {
			var getTargetFieldAPI = function(oElement) {
				var oParentElement = oElement.getParent();
				while (oParentElement && !oParentElement.isA("sap.fe.macros.FieldAPI")) {
					oParentElement = oParentElement.getParent();
				}
				return oParentElement;
			};
			var oTargetFieldAPI = getTargetFieldAPI(oElement);
			var getTargetVbox = function(oElement) {
				var oParentElement = oElement.getParent();
				while (oParentElement && !oParentElement.isA("sap.m.VBox")) {
					oParentElement = oParentElement.getParent();
				}
				return oParentElement;
			};
			var oTargetVbox = getTargetVbox(oElement),
				oTargetRow = _getTableRow(oElement);
			var oTargetCell =
				oTargetRow &&
				oTargetRow.getCells().find(function(cell) {
					return (
						cell.getId() === (oTargetFieldAPI && oTargetFieldAPI.getId()) ||
						cell.getId() === (oTargetVbox && oTargetVbox.getId())
					);
				});
			var iTargetColumnIndex = oTargetRow && oTargetRow.indexOfCell(oTargetCell);
		...
		function _setMessageSubtitle(
			...
			message.setActiveTitle(
				!sMessageSubtitle || bIsNavigationConfigured || (!!sTableTargetColName ? !!oTableRowContext : !!sTableTargetColName)
			);
		...
		MessageButton.prototype._updateInternalModel = function(
			...
			var oTemp = {
				...
				oModel = oTable.getModel("internal"),
				oContext = this._getTableRefErrorContext(oTable);
		...
		MessageButton.prototype._computeMessageGroupAndSubTitle = function(
			...
			if (bIsBackendMessage) {
				for (l = 0; l < aElements.length; l++) {
					...
					if (oElement.isA("sap.m.Table") || oElement.isA("sap.ui.table.Table")) {
						...
						if (oRowBinding && oRowBinding.isLengthFinal() && oTable.getBindingContext()) {
							...
							this._updateInternalModel(oTableRowContext, iRowIndex, sTableTargetColProperty, oTable, oMessageObject, true);
						}
					} else {
				...
				if (oTable) {
					...
					sTableTargetColProperty = iTargetColumnIndex && oTable.getColumns()[iTargetColumnIndex].getDataProperty();
					sTableTargetProperty = sTableTargetColProperty;
					sTableTargetColName = sTableTargetColProperty && oTable.getColumns()[iTargetColumnIndex].getHeader();
					...
					this._updateInternalModel(oTableRowContext, iRowIndex, sTableTargetColProperty, oTable, oMessageObject, false);
					if (oTableRowContext) {
						var oErrorContext = {
							rowIndex: oTableRowContext ? iRowIndex : "",
							targetColProperty: sTableTargetColProperty ? sTableTargetColProperty : ""
		...
		MessageButton.prototype._checkControlIdInSections = function(aMessages, bEnableBinding) {
			...
			if (aVisibleSections) {
				...
				for (i = aMessages.length - 1; i >= 0; --i) {
					...
					// Checking for hidden manifest switch _disableMessageLazyLoading to turn off this feature until a proper fix is provided
					// Currently a message might not have the controlId from the beginning, in that case we would wrongly enable all bindings
					var oViewDataModel;
					if (
						this.oObjectPageLayout !== undefined &&
						this.oObjectPageLayout.getMetadata().getName() === "sap.uxap.ObjectPageLayout"
					) {
						oViewDataModel = this.oObjectPageLayout.getModel("viewData");
					}
					if (
						!bEnableBinding &&
						(!oTargetedControl || oTargetedControl.isA("sap.m.Table") || oTargetedControl.isA("sap.ui.table.Table")) &&
						this._findTargetForMessage(message) &&
						oViewDataModel &&
						!oViewDataModel.getProperty("/_disableLazyLoadingForMessages")
					) {
						return true;
					}
				}
