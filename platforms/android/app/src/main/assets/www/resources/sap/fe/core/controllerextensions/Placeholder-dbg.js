...
sap.ui.define(["sap/ui/core/mvc/ControllerExtension", "sap/fe/core/BusyLocker", "sap/base/util/UriParameters", "sap/base/Log"], function(
	...
	var Extension = ControllerExtension.extend("sap.fe.core.controllerextensions.Placeholder", {
		...
		getEnumStartingParams: function() {
			return {
			...
			sap.m.NavContainer.addCustomTransition(
				...
				function(oFromPage, oToPage, fCallback) {
					if (oFromPage.getController && oFromPage.getController().isPlaceholder && oFromPage.getController().isPlaceholder()) {
						var sTransition = oFromPage.getController().getOptions("transition");
						if (sTransition !== that.getEnumStartingParams().FE_VIEW_CREATED) {
		...
		isPlaceholderEnabled: function() {
			var bPlaceholderEnabled = false;
			switch (UriParameters.fromQuery(window.location.search).get("sap-ui-xx-placeholder")) {
				case "true":
					bPlaceholderEnabled = true;
					break;
				case "false":
					bPlaceholderEnabled = false;
					break;
				default:
					bPlaceholderEnabled =
