	...
	function getMethods(oComponent, oComponentUtils) {
		...
		return {
			...
			getTemplateSpecificParameters: function(oMetaModel, oSettings, Device, sLeadingEntitySet, oDraftContext, oModel) {
				...
				function getMultiEditSettings(oVariant, oLrSettings, oEntitySet) {
					...
					if (oTableSettings.enableMultiEditDialog && oLrSettings.isResponsiveTable && !oComponentUtils.isDraftEnabled() && !oLrSettings.isWorklist && !oComponent.getAppComponent().getFlexibleColumnLayout()) {
						bMultiEdit = true;
				...
				if (oLrSettings.quickVariantSelectionX) {
					...
					for (var sKey in oVariants) {
						...
						if (!oVariants[sKey].isSmartChart) {
							...
							var oMultiEditSettings = getMultiEditSettings(oVariants[sKey], oLrSettings, oEntitySet);
							oVariants[sKey].tableSettings.multiEdit = oMultiEditSettings.multiEdit;
					...
					var oMultiEditSettings = getMultiEditSettings(undefined, oLrSettings, oEntitySet);
					oLrSettings.multiEdit = oMultiEditSettings.multiEdit;
