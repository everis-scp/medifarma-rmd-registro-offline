sap.ui.define([
	...
	var RequestQueue = function(context, sceneId) {
		...
		this.imageDetails = new ResourceQueue();
	};
	...
	RequestQueue.prototype.generateRequestCommand = function() {
		...
		if (!this.meshes.isEmpty()) {
			...
			var expand = [ "nodes", "bounds" ]; // parametrics?
			...
			command.sceneId = this.sceneId;
			command = Object.assign(command, ids[ 0 ]);
		} else if (!this.imageDetails.isEmpty()) {
			ids = this.imageDetails.requestedList.splice(0, 1);
			Log.info("Requesting image details: " + ids[ 0 ].imageId);
			command = this.createGetContentCommand(Command.getImageDetails, [ ids[ 0 ].imageId ]);
			command.sceneId = this.sceneId;
			command = Object.assign(command, ids[ 0 ]);
		}
