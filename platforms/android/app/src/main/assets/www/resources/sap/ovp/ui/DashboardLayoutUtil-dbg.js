sap.ui.define([
    ...
    DashboardLayoutUtil.prototype.setAriaPos = function() {
        var columnCount = this.dashboardLayoutModel.iColCount;
        var cardsOnColumn = [];
        var ariaPos = {};
        ...
        while (!isEmpty(cardsOnColumn)) {
            var skipCol = Array(columnCount).fill(false);
            var columnNumber = 0;
            while ( columnNumber < columnCount ) {
                if ( cardsOnColumn[columnNumber] && !isEmpty(cardsOnColumn[columnNumber]) && !skipCol[columnNumber]) {
                    var card = cardsOnColumn[columnNumber].shift();
                    count++;
                    ariaPos[card.id] = count;
                    var oCardElement = this.getCardDomId(card.id) && document.getElementById((this.getCardDomId(card.id)));
                    if ( oCardElement ) {
                        oCardElement.ariaPosInSet = count;
                    }
                    skipCol.fill(true, columnNumber, columnNumber + card.dashboardLayout.colSpan);
                    columnNumber = columnNumber + card.dashboardLayout.colSpan;
                } else {
